...
// ** Dec 12th, 08" (v1.01): Fixed Shadow issue when multiple LIs within the
// same UL (level) contain sub menus:
// http://www.dynamicdrive.com/forums/showthread.php?t=39177&highlight=smooth

// ** Feb 11th, 09" (v1.02): The currently active main menu item (LI A) now gets
// a CSS class of ".selected", including sub menu items.
...
// ** Feb 2nd, 10" (v1.4): Adds ability to specify delay before sub menus appear
// and disappear, respectively. See showhidedelay variable below

// ** Dec 17th, 10" (v1.5): Updated menu shadow to use CSS3 box shadows when the
// browser is FF3.5+, IE9+, Opera9.5+, or Safari3+/Chrome. Only .js file
// changed.
...
var ddsmoothmenu = {
	...
	css3support : window.msPerformance
			|| (!document.all && document.querySelector), // detect browsers
															// that support CSS3
															// box shadows (ie9+
															// or FF3.5+,
															// Safari3+, Chrome
	...
	getajaxmenu : function($, setting) { // function to fetch external page
		...
		$.ajax({
					url : setting.contentsource[1], // path to external menu
													// file
					async : true,
					error : function(ajaxrequest) {
						$menucontainer
								.html('Error fetching content. Server Response: '
	...
	buildmenu : function($, setting) {
		...
		$headers.each(function(i) { // loop through each LI header
					...
					// '<img src="'+ (this.istopheader &&
					// setting.orientation!='v'? smoothmenu.arrowimages.down[1]
					// : smoothmenu.arrowimages.right[1])
					// +'" class="' + (this.istopheader &&
					// setting.orientation!='v'? smoothmenu.arrowimages.down[0]
					// : smoothmenu.arrowimages.right[0])
					// + '" style="border:0;" />'
					// )
					if (smoothmenu.shadow.enable && !smoothmenu.css3support) { // if
																				// shadows
																				// enabled
																				// and
																				// browser
						...
						this.$shadow = $('<div class="ddshadow'
								+ (this.istopheader ? ' toplevelshadow' : '')
								+ '"></div>').prependTo($parentshadow).css({
									left : this._shadowoffset.x + 'px',
									top : this._shadowoffset.y + 'px'
								}) // insert shadow DIV and set it to parent
									// node for the next shadow div
					}
					$curobj.hover(function(e) {
						var $targetul = $subul // reference UL to reveal
						var header = $curobj.get(0) // reference header LI as
													// DOM object
						clearTimeout($targetul.data('timers').hidetimer)
						$targetul.data('timers').showtimer = setTimeout(
								function() {
									header._offsets = {
									...
									menuleft = (header._offsets.left + menuleft
											+ header._dimensions.subulw > $(window)
											.width()) ? (header.istopheader
											&& setting.orientation != 'v'
											? -header._dimensions.subulw
													+ header._dimensions.w
											: -header._dimensions.w) : menuleft // calculate
																				// this
																				// sub
																				// menu's
																				// offsets
									...
									if ($targetul.queue().length <= 1) { // if 1
										...
										if (smoothmenu.shadow.enable
												&& !smoothmenu.css3support) {
											var shadowleft = header.istopheader
													? $targetul.offset().left
											...
											header.$shadow.css({
												...
												height : header._dimensions.subulh
														+ 'px'
											}, ddsmoothmenu.transition.overtime)
										}
									}
		...
		if (smoothmenu.shadow.enable && smoothmenu.css3support) { // if
			...
			var css3shadow = parseInt(smoothmenu.shadow.offsetx) + "px "
					+ parseInt(smoothmenu.shadow.offsety) + "px 5px #aaa" // construct
																			// CSS3
																			// box-shadow
																			// value
			var shadowprop = ["boxShadow", "MozBoxShadow", "WebkitBoxShadow",
					"MsBoxShadow"] // possible vendor specific CSS3 shadow
									// properties
	...
	init : function(setting) {
		...
		jQuery(document).ready(function($) { // ajax menu?
					if (typeof setting.contentsource == "object") { // if
																	// external
																	// ajax menu
