 ...
 * 
 * Copyright (c) 2011 John Resig, Jörn Zaefferer Dual licensed under the MIT
 * (MIT-LICENSE.txt) or GPL (GPL-LICENSE.txt) licenses.
 */
...
(function(window) {
	...
	Test.prototype = {
		...
		run : function() {
			...
			try {
				...
				fail(	"Test " + this.testName
								+ " died, exception and test follows", e,
						this.callback);
				QUnit.ok(false, "Died on test #" + (this.assertions.length + 1)
								+ ": " + e.message + " - "
		...
		teardown : function() {
			try {
				...
				QUnit.ok(false, "Teardown failed on " + this.testName + ": "
								+ e.message);
			}
		},
		finish : function() {
			if (this.expected && this.expected != this.assertions.length) {
				QUnit.ok(false, "Expected " + this.expected
								+ " assertions, but " + this.assertions.length
								+ " were run");
			}
			...
			if (tests) {
				...
				for (var i = 0; i < this.assertions.length; i++) {
					...
					li.innerHTML = assertion.message
				...
				if (QUnit.config.reorder && defined.sessionStorage) {
					if (bad) {
						sessionStorage.setItem("qunit-" + this.module + "-"
										+ this.testName, bad);
					} else {
						sessionStorage.removeItem("qunit-" + this.module + "-"
								+ this.testName);
					}
				}
				...
				b.innerHTML = this.name
						...
						+ "</b>, <b class='passed'>" + good + "</b>, "
						+ this.assertions.length + ")</b>";

				var a = document.createElement("a");
				a.innerHTML = "Rerun";
				a.href = QUnit.url({
							filter : getText([b]).replace(/\([^)]+\)$/, "")
									.replace(/(^\s*|\s*$)/g, "")
						});
			...
			try {
				...
				fail(	"reset() failed, following Test " + this.testName
		...
		queue : function() {
			...
			var bad = QUnit.config.reorder
					&& defined.sessionStorage
					&& +sessionStorage.getItem("qunit-" + this.module + "-"
							+ this.testName);
			if (bad) {
				run();
			} else {
	...
	var QUnit = {
		...
		test : function(testName, expected, callback, async) {
			...
			if (config.currentModule) {
				name = '<span class="module-name">' + config.currentModule
						+ "</span>: " + name;
			}
			...
			var test = new Test(name, testName, expected, testEnvironmentArg,
					async, callback);
		...
		/**
		 * Specify the number of expected assertions to gurantee that failed
		...
		/**
		 * Checks that the first two arguments are equal, with an optional
		 * message. Prints out both actual and expected values.
		 * 
		...
		deepEqual : function(actual, expected, message) {
			QUnit
					.push(QUnit.equiv(actual, expected), actual, expected,
							message);
		},
	...
	extend(QUnit, {
		 ...
		 * 
		 * If jQuery is available, uses jQuery's html(), otherwise just
		 * innerHTML.
		 */
		reset : function() {
			if (window.jQuery) {
		...
		triggerEvent : function(elem, type, event) {
			if (document.createEvent) {
				...
				event.initMouseEvent(type, true, true,
						elem.ownerDocument.defaultView, 0, 0, 0, 0, 0, false,
	...
	addEvent(window, "load", function() {
		...
		if (banner) {
			banner.innerHTML = '<a href="' + QUnit.url({
					...
					+ '<label><input name="noglobals" type="checkbox"'
					+ (config.noglobals ? ' checked="checked"' : '')
					+ '>noglobals</label>'
					+ '<label><input name="notrycatch" type="checkbox"'
					+ (config.notrycatch ? ' checked="checked"' : '')
					+ '>notrycatch</label>';
						...
						if (defined.sessionStorage) {
							if (filter.checked) {
								sessionStorage.setItem(
										"qunit-filter-passed-tests", "true");
							} else {
								sessionStorage
	...
	function done() {
		...
		var banner = id("qunit-banner"), tests = id("qunit-tests"), runtime = +new Date
				- config.started, passed = config.stats.all - config.stats.bad, html = [
				'Tests completed in ', runtime, ' milliseconds.<br/>',
				'<span class="passed">', passed,
				'</span> tests of <span class="total">', config.stats.all,
				'</span> passed, <span class="failed">', config.stats.bad,
				'</span> failed.'].join('');
		...
		if (typeof document !== "undefined" && document.title) {
			...
			// non-utf-8-charset
			document.title = (config.stats.bad ? "\u2716" : "\u2714") + " "
					+ document.title;
		}
	...
	function checkPollution(name) {
		...
		if (deletedGlobals.length > 0) {
			ok(false, "Deleted global variable(s): "
							+ deletedGlobals.join(", "));
		}
	}
		...
		innerEquiv = function() { // can take multiple arguments
			...
			return (function(a, b) {
				if (a === b) {
					...
					&& arguments.callee.apply(this, args.splice(1, args.length
											- 1));
		};
	...
	QUnit.jsDump = (function() {
		...
		function join(pre, arr, post) {
			var s = jsDump.separator(), base = jsDump.indent(), inner = jsDump
					.indent(1);
			if (arr.join)
		...
		var jsDump = {
			parse : function(obj, type) { // type is used mostly internally,
											// you can fix a (custom)type in
											// advance
				...
				return type == 'function'
						...
						: type == 'string' ? parser : this.parsers.error;
			},
			typeOf : function(obj) {
				var type;
				if (obj === null) {
			...
			parsers : {
				...
				'function' : function(fn) {
					...
					ret = [ret, QUnit.jsDump.parse(fn, 'functionArgs'), '){']
							.join('');
				...
				object : function(map) {
					...
					QUnit.jsDump.up();
					for (var key in map)
						ret.push(QUnit.jsDump.parse(key, 'key') + ': '
								+ QUnit.jsDump.parse(map[key]));
					QUnit.jsDump.down();
					return join('{', ret, '}');
				},
				node : function(node) {
					var open = QUnit.jsDump.HTML ? '&lt;' : '<', close = QUnit.jsDump.HTML
							? '&gt;'
							: '>';
	...
	/*
	 * Javascript Diff Algorithm By John Resig (http://ejohn.org/) Modified by
	 * Chu Alan "sprite"
	 * 
	 ...
	 * "the quick <del>brown </del> fox <del>jumped </del><ins>jumps </ins>
	 * over"
	 */
	QUnit.diff = (function() {
		function diff(o, n) {
