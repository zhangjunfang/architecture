 ...
 * 
 * Defines the FCKXHtml object, responsible for the XHTML operations. Gecko
 * specific.
 */
	...
	for (var n = 0; n < aAttributes.length; n++) {
		...
		if (oAttribute.specified) {
			...
			if (sAttName.StartsWith('_fck'))
				continue;
			// There is a bug in Mozilla that returns '_moz_xxx' attributes as
			// specified.
			else if (sAttName.indexOf('_moz') == 0)
				continue;
			// There are one cases (on Gecko) when the oAttribute.nodeValue must
			// be used:
			...
			// must be transformed to checked="checked".
				...
				sAttValue = htmlNode.getAttribute(sAttName, 2); // We must use
			// getAttribute
			// to get it
			// exactly as it
			// is defined.
