...
var FCKSpecialCombo = function(caption, fieldWidth, panelWidth, panelMaxHeight,
	...
	// this._Panel.PanelDiv.innerHTML = '<table cellpadding="0" cellspacing="0"
	// width="100%" style="TABLE-LAYOUT: fixed"><tr><td
	// nowrap></td></tr></table>' ;
...
FCKSpecialCombo.prototype.AddItem = function(id, html, label, bgColor) {
	// <div class="SC_Item" onmouseover="this.className='SC_Item SC_ItemOver';"
	// onmouseout="this.className='SC_Item';"><b>Bold 1</b></div>
	var oDiv = this._ItemsHolderEl.appendChild(this._Panel.Document
			.createElement('DIV'));
	...
	// In IE, the width must be set so the borders are shown correctly when the
	// content overflows.
	...
	FCKTools.AddEventListenerEx(oDiv, 'mouseover',
			FCKSpecialCombo_ItemOnMouseOver);
	FCKTools.AddEventListenerEx(oDiv, 'mouseout',
			FCKSpecialCombo_ItemOnMouseOut);
...
FCKSpecialCombo.prototype.SetEnabled = function(isEnabled) {
	...
	// In IE it can happen when the page is reloaded that _OuterTable is null,
	// so check its existence
	if (this._OuterTable)
...
FCKSpecialCombo.prototype.Create = function(targetElement) {
	...
	var eOuterTable = this._OuterTable = targetElement.appendChild(oDoc
			.createElement('TABLE'));
	...
	var oField = FCKTools.AppendElement(eOuterTable.rows[0].insertCell(-1),
			'div');
	if (bShowLabel) {
		...
		this._LabelEl = oField.getElementsByTagName('label')[0]; // Memory
		// Leak
		this._LabelEl.innerHTML = this.Label;
	} else {
		oField.className = 'TB_Button_Off';
		// oField.innerHTML = '<span className="SC_FieldCaption">' +
		// this.Caption + '<table cellpadding="0" cellspacing="0"
		// style="TABLE-LAYOUT: fixed;"><tbody><tr><td class="SC_FieldButton"
		// style="border-left: none;">&nbsp;</td></tr></tbody></table>' ;
		// oField.innerHTML = '<table cellpadding="0" cellspacing="0"
		// style="TABLE-LAYOUT: fixed;"><tbody><tr><td class="SC_FieldButton"
		// style="border-left: none;">&nbsp;</td></tr></tbody></table>' ;

		// Gets the correct CSS class to use for the specified style (param).
		oField.innerHTML = '<table title="'
				+ this.Tooltip
				+ '" class="'
				+ sClass
				...
				+ '<tr>'
				+
				// '<td class="TB_Icon"><img src="' + FCKConfig.SkinPath +
				// 'toolbar/' + this.Command.Name.toLowerCase() + '.gif"
				// width="21" height="21"></td>' +
				'<td><img class="TB_Button_Padding" src="' + FCK_SPACER_PATH
				+ '" /></td>' + '<td class="TB_Text">' + this.Caption + '</td>'
				+ '<td><img class="TB_Button_Padding" src="' + FCK_SPACER_PATH
	...
	FCKTools.AddEventListenerEx(oField, 'mouseover',
			FCKSpecialCombo_OnMouseOver, this);
	FCKTools.AddEventListenerEx(oField, 'mouseout', FCKSpecialCombo_OnMouseOut,
			this);
...
function FCKSpecialCombo_OnClick(e, specialCombo) {
	...
	if (specialCombo.Enabled) {
		...
		// it will not be possible to retrieve "oItemsHolder.offsetHeight" (IE
		// only).
		if (FCKBrowserInfo.IsIE)
 ...
 * 
 * <div class="SC_Field" style="width: 80px;"> <table width="100%"
 * cellpadding="0" cellspacing="0" style="table-layout: fixed;"> <tbody> <tr>
 * <td class="SC_FieldLabel"><label>&nbsp;</label></td>
 * <td class="SC_FieldButton">&nbsp;</td> </tr> </tbody> </table> </div>
 */
