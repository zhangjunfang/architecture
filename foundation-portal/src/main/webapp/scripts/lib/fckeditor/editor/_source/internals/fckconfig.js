 ...
 * another dedicated object and leave FCKConfig as a holder object for settings
 * only).
 */
...
if (document.location.protocol == 'file:') {
	...
	// In Windows it is file:// but in MacOs it is file:/// so let's get it
	// automatically
	var sFullProtocol = document.location.href.match(/^(file\:\/{2,3})/)[1];
	// #945 Opera does strange things with files loaded from the disk, and it
	// fails in Mac to load xml files
...
FCKConfig.ProcessHiddenField = function() {
	...
	for (var i = 0; i < aCouples.length; i++) {
		...
		if (sKey == 'CustomConfigurationsPath') // The Custom Config File path
...
function FCKConfig_PreProcess() {
	...
	// Turn the attributes that will be removed in the RemoveFormat from a
	// string to an array
...
// First of any other protection, we must protect all comments to avoid
		...
		// Script tags will also be forced to be protected, otherwise IE will
		// execute them.
...
FCKConfig.ProtectedSource.Revert = function(html, clearBin) {
	...
	var regex = new RegExp("(<|&lt;)!--\\{" + this._CodeTag
					+ "(\\d+)\\}--(>|&gt;)", "g");
