...
FCKMenuBlock.prototype.AddItem = function(name, label,
	...
	oItem.OnClick = FCKTools.CreateEventListener(FCKMenuBlock_Item_OnClick,
			this);
...
FCKMenuBlock.prototype.Create = function(parentElement) {
	if (!this._ItemsTable) {
		...
		var eItemsTable = this._ItemsTable = oMainElement.appendChild(oDoc
				.createElement('table'));
...
function FCKMenuBlock_Item_OnActivate(menuBlock) {
	...
	if (oActiveItem && oActiveItem != this) {
		// Set the focus to this menu block window (to fire OnBlur on opened
		// panels).
