 ...
 * 
 * Class for working with a selection range, much like the W3C DOM Range, but it
 * is not intended to be an implementation of the W3C interface.
 */
...
FCKDomRange.prototype = {
	...
	_UpdateElementInfo : function() {
		...
		if (!innerRange)
			...
			this.StartNode = eStart.nodeType == 3
					? eStart
					: eStart.childNodes[innerRange.startOffset];
			...
			if (innerRange.collapsed) {
				...
				// usually part of the range, but the marker for the range end.
				// So,
	...
	MoveToElementEditStart : function(targetElement) {
		...
		while (targetElement && targetElement.nodeType == 1) {
			if (FCKDomTools.CheckIsEditable(targetElement))
				...
				break; // If we already found an editable element, stop the
			// loop.
	 ...
	 * selection caret) at the beginning of the block. It means that some things
	 * could be brefore the range, like spaces or empty inline elements, but it
	 * would still be considered at the beginning of the block.
	 */
	CheckStartOfBlock : function() {
		...
		// the offset is zero.
		if (!currentNode)
			currentNode = FCKDomTools.GetPreviousSourceNode(container, true,
					null, block);
		...
		while (currentNode) {
			switch (currentNode.nodeType) {
				case 1 :
					...
					if (!FCKListsLib.InlineChildReqElements[currentNode.nodeName
							.toLowerCase()])
						return cache.IsStartOfBlock = false;
			...
			currentNode = FCKDomTools.GetPreviousSourceNode(currentNode, false,
					null, block);
		}
	 ...
	 * selection caret) at the end of the block. It means that some things could
	 * be after the range, like spaces, empty inline elements, or a single <br>,
	...
	CheckEndOfBlock : function(refreshSelection) {
		...
		if (!currentNode)
			currentNode = FCKDomTools.GetNextSourceNode(container, true, null,
					block);
		...
		while (currentNode) {
			...
			currentNode = FCKDomTools.GetNextSourceNode(currentNode, false,
					null, block);
		}
	...
	CreateBookmark2 : function() {
		...
		// Since we're treating the document tree as normalized, we need to
		// backtrack the text lengths
		...
		// Also note that the node that we use for "address base" would change
		// during backtracking.
		...
		// If the object pointed to by the startOffset and endOffset are text
		// nodes, we need
		// to backtrack and add in the text offset to the bookmark addresses.
		if (addrStart.nodeType == 1 && addrStart.childNodes[bookmark.Start[0]]
				&& addrStart.childNodes[bookmark.Start[0]].nodeType == 3) {
			var curNode = addrStart.childNodes[bookmark.Start[0]];
			var offset = 0;
			while (curNode.previousSibling
					&& curNode.previousSibling.nodeType == 3) {
		...
		if (addrEnd.nodeType == 1 && addrEnd.childNodes[bookmark.End[0]]
				&& addrEnd.childNodes[bookmark.End[0]].nodeType == 3) {
		...
		bookmark.Start = FCKDomTools.GetNodeAddress(addrStart, true)
				.concat(bookmark.Start);
		bookmark.End = FCKDomTools.GetNodeAddress(addrEnd, true)
				.concat(bookmark.End);
		return bookmark;
	},
	...
	MoveToBookmark2 : function(bookmark) {
		...
		var curStart = FCKDomTools.GetNodeFromAddress(this.Window.document,
				bookmark.Start.slice(0, -1), true);
		var curEnd = FCKDomTools.GetNodeFromAddress(this.Window.document,
				bookmark.End.slice(0, -1), true);
	...
	/*
	 * Moves the position of the start boundary of the range to a specific
	 * position relatively to a element. @position: 1 = After Start
	 * <target>^contents</target> 2 = Before End <target>contents^</target> 3 =
	 * Before Start ^<target>contents</target> 4 = After End <target>contents</target>^
	 */
		...
		switch (unit) {
			...
			case 'inline_elements' :
				...
				if (this._Range.startOffset == 0) {
					...
					if (oNode) {
						while (FCKListsLib.InlineNonEmptyElements[oNode.nodeName
								.toLowerCase()]) {
				...
				if ((oNode.nodeType == 3 && offset >= oNode.nodeValue.length)
						|| (oNode.nodeType == 1 && offset >= oNode.childNodes.length)
						|| (oNode.nodeType != 1 && oNode.nodeType != 3)) {
					if (oNode.nodeType != 1)
			...
			case 'list_contents' :
				...
				if (this.StartBlock && FCKConfig.EnterMode != 'br'
					...
					// If it is an element, get the node right before of it (in
					// source order).
					if (oNode.nodeType == 1) {
						var lastNode = oNode.childNodes[this._Range.startOffset];
						if (lastNode)
							oNode = FCKDomTools.GetPreviousSourceNode(lastNode,
									true);
						else
					...
					while (oNode
							&& (oNode.nodeType != 1 || (oNode != this.StartBlockLimit && !boundarySet[oNode.nodeName
									.toLowerCase()]))) {
						this._Range.setStartBefore(oNode);
				...
				if (this.EndBlock && FCKConfig.EnterMode != 'br'
					...
					oNode = this._Range.endContainer;
					if (oNode.nodeType == 1)
						oNode = oNode.childNodes[this._Range.endOffset]
								|| oNode.lastChild;

					// We must look for the right boundary, relative to the
					// range
	...
	/**
	 * Split the block element for the current range. It deletes the contents of
	 * the range and splits the block in the collapsed position, resulting in
	 * two sucessive blocks. The range is then positioned in the middle of them.
	 * 
	 * It returns and object with the following properties: - PreviousBlock : a
	 * reference to the block element that preceeds the range after the split. -
	 * NextBlock : a reference to the block element that follows the range after
	 * the split. - WasStartOfBlock : a boolean indicating that the range was
	 * originaly at the start of the block. - WasEndOfBlock : a boolean
	 * indicating that the range was originaly at the end of the block.
	 * 
	 * If the range was originaly at the start of the block, no split will
	 * happen and the PreviousBlock value will be null. The same is valid for
	 * the NextBlock value if the range was at the end of the block.
	 */
		...
		if (this.StartBlockLimit == this.EndBlockLimit) {
			...
			if (blockTag != 'br') {
				if (!eStartBlock) {
					...
					eEndBlock = this.EndBlock; // FixBlock may have fixed the
					// EndBlock too.
				}
			...
			var bIsStartOfBlock = (eStartBlock != null && this
					.CheckStartOfBlock());
			...
			if (eStartBlock && eEndBlock && eStartBlock == eEndBlock) {
				if (bIsEndOfBlock) {
					...
					// Extract the contents of the block from the selection
					// point to the end of its contents.
	...
	FixBlock : function(isStart, blockTag) {
		...
		if (FCKDomTools.CheckIsEmptyElement(oFixedBlock, function(element) {
