...
if (FCKBrowserInfo.IsIE || FCKBrowserInfo.IsOpera) {
	...
	FCKAnchorsProcessor.ProcessDocument = function(document) {
		...
		while (i >= 0 && (oLink = aLinks[i--])) {
			// If it is anchor. Doesn't matter if it's also a link (even better:
			// we show that it's both a link and an anchor)
			if (oLink.name.length > 0) {
				// if the anchor has some content then we just add a temporary
				// class
				if (oLink.innerHTML !== '') {
...
FCKPageBreaksProcessor.ProcessDocument = function(document) {
	 ...
	 * 
	 * var oCenter ; var i = aCenters.length - 1 ; while ( i >= 0 && ( oCenter =
	 * aCenters[i--] ) ) { if ( oCenter.style.pageBreakAfter == 'always' &&
	 * oCenter.innerHTML.Trim().length == 0 ) { var oFakeImage =
	 * FCKDocumentProcessor_CreateFakeImage( 'FCK__PageBreak',
	 * oCenter.cloneNode(true) ) ;
	 * 
...
FCKEmbedAndObjectProcessor = (function() {
	...
	var processElement = function(el) {
		...
		var replaceElement;
		var fakeImg = replaceElement = FCKDocumentProcessor_CreateFakeImage(
				'FCK__UnknownObject', clone);
		FCKEmbedAndObjectProcessor.RefreshView(fakeImg, el);

		for (var i = 0; i < customProcessors.length; i++)
			replaceElement = customProcessors[i](el, replaceElement)
					|| replaceElement;
	...
	return FCKTools.Merge(FCKDocumentProcessor.AppendNew(), {
		ProcessDocument : function(doc) {
			// Firefox 3 would sometimes throw an unknown exception while
			// accessing EMBEDs and OBJECTs
...
FCK.GetRealElement = function(fakeElement) {
	...
	if (fakeElement.getAttribute('_fckflash')) {
		if (fakeElement.style.width.length > 0)
			e.width = FCKTools.ConvertStyleSizetoView(fakeElement.style.width);

		if (fakeElement.style.height.length > 0)
			e.height = FCKTools
					.ConvertStyleSizetoView(fakeElement.style.height);
	}
...
if (FCKBrowserInfo.IsIE) {
	FCKDocumentProcessor.AppendNew().ProcessDocument = function(document) {
		...
		while (i >= 0 && (eHR = aHRs[i--])) {
			...
			// We must insert the new one after it. insertBefore will not work
			// in all cases.
...
FCKDocumentProcessor.AppendNew().ProcessDocument = function(document) {
	...
	while (i >= 0 && (oInput = aInputs[i--])) {
		if (oInput.type == 'hidden') {
			var oImg = FCKDocumentProcessor_CreateFakeImage('FCK__InputHidden',
					oInput.cloneNode(true));
...
FCKEmbedAndObjectProcessor.AddCustomHandler(function(el, fakeImg) {
	if (!(el.nodeName.IEquals('embed') && (el.type == 'application/x-shockwave-flash' || /\.swf($|#|\?)/i
			.test(el.src))))
		return;
...
if (FCKBrowserInfo.IsSafari) {
	FCKDocumentProcessor.AppendNew().ProcessDocument = function(doc) {
		var spans = doc.getElementsByClassName
				...
				: Array.prototype.filter.call(doc.getElementsByTagName('span'),
						function(item) {
