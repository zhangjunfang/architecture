...
if (FCKBrowserInfo.IsAIR) {
	var FCKAdobeAIR = (function() {
		...
		var getDocumentHead = function(doc) {
			...
			if (heads && heads[0])
				...
				doc.documentElement.insertBefore(head,
		...
		return {
			...
			ToolbarSet_GetOutElement : function(win, outMatch) {
				...
				toolbarTarget = toolbarTarget.document
						.getElementById(outMatch[2]);
			},
	...
	(function() {
		...
		FCKPanel_Window_OnBlur = function(e, panel) {
			...
			panel._focusTimer = FCKTools.SetTimeout(
					_Original_FCKPanel_Window_OnBlur, 100, this, [e, panel]);
		}

		FCK.StartEditor = function() {
			// Force pointing to the CSS files instead of using the inline CSS
			// cached styles.
			window.FCK_InternalCSS = FCKConfig.BasePath
					+ 'css/fck_internal.css';
			window.FCK_ShowTableBordersCSS = FCKConfig.BasePath
