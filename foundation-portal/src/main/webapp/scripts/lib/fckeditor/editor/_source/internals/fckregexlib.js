...
var FCKRegexLib = {
	// This is the Regular expression used by the SetData method for the
	// "&apos;" entity.
	AposEntity : /&apos;/gi,

	// Used by the Styles combo to identify styles that can't be applied to
	// text.
	...
	// Temporary classes (Tables without border, Anchors with content) used in
	// IE
	FCK_Class : /\s*FCK__[^ ]*(?=\s+|$)/,
	...
	// Used in conjunction with the FCKConfig.ForceSimpleAmpersand configuration
	// option.
	ForceSimpleAmpersand : /___FCKAmp___/g,

	// Get the closing parts of the tags with no closing tags, like <br/>...
	// gets the "/>" part.
	SpaceNoClose : /\/>/g,
	...
	// We look for the "src" and href attribute with the " or ' or without one
	// of
	...
	// All variables defined in a style attribute or style definition. The
	// variable
