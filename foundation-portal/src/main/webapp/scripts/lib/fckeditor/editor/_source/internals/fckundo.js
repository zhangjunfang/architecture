...
FCKUndo.Changed = false; // Is the document changed in respect to its initial
// image?
FCKUndo.MaxTypes = 25;
...
FCKUndo._GetBookmark = function() {
	...
	try {
		// There are some tricky cases where this might fail (e.g. having a lone
		// empty table in IE)
...
FCKUndo._SelectBookmark = function(bookmark) {
	...
	if (bookmark instanceof Object) {
		...
		try {
			// this does not always succeed, there are still some tricky cases
			// where it fails
			// e.g. add a special character at end of document, undo, redo ->
			// error
			range.Select();
		} catch (e) {
...
FCKUndo.SaveUndoStep = function() {
	...
	// Assume the editor content is changed when SaveUndoStep() is called after
	// the first time.
	...
	if (this.CurrentIndex > 0
			...
			&& this._CheckIsBookmarksEqual(bookmark,
					this.SavedData[this.CurrentIndex][1]))
		return;
	// Save the selection and caret position in the first undo level for the
	// first change.
	else if (this.CurrentIndex == 0 && this.SavedData.length
			&& sHtml == this.SavedData[0][0]) {
