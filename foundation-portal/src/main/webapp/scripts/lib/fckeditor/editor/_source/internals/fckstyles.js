...
var FCKStyles = FCK.Styles = {
	...
	CheckSelectionChanges : function() {
		...
		// Walks the start node parents path, checking all styles that are being
		// listened.
			...
			if (callbacks) {
				...
				if (state != (style._LastState || null)) {
					...
					for (var i = 0; i < callbacks.length; i++) {
						...
						callback
								.call(callbackOwner || window, styleName, state);
					}
				}
			}
		}
	...
	/**
	 * Remove all styles from the current selection. TODO: - This is almost a
	 * duplication of FCKStyle.RemoveFromRange. We should try to merge things.
	 */
	RemoveAll : function() {
		...
		while (currentNode) {
			...
			if (tagsRegex.test(currentNode.nodeName))
				...
				FCKDomTools.RemoveAttributes(currentNode,
						FCKConfig.RemoveAttributesArray);
	...
	GetStyles : function() {
		...
		if (!styles) {
			styles = this._GetStyles = FCKTools.Merge(this._LoadStylesCore(),
					this._LoadStylesCustom(), this._LoadStylesXml());
		}
		return styles;
	},
	...
	_LoadStylesXml : function() {
		...
		var stylesXmlObj = FCKXml.TransformToObject(xml
				.SelectSingleNode('Styles'));
		...
		for (var i = 0; i < styleNodes.length; i++) {
			...
			if (element.length == 0)
				throw ('The element name is required. Error loading "'
						+ stylesXmlPath + '"');
				...
				for (j = 0; j < cssStyleOverrideNodes.length; j++) {
					...
					if (overrideAttNode) {
						...
						for (var k = 0; k < overrideAttNode.length; k++) {
							var overrideAttValue = overrideAttNode[k].value
									|| null;
							if (overrideAttValue) {
								// Check if the override attribute value is a
								// regular expression.
								var regexMatch = overrideAttValue
										&& FCKRegexLib.RegExp
												.exec(overrideAttValue);
								if (regexMatch)
									overrideAttValue = new RegExp(
			...
			// Add the style to the "Styles" collection using it's name as the
			// key.
