...
public class CZipInputStream extends InflaterInputStream implements
	...
	private static final int STORED = CZipEntry.STORED;
	private static final int DEFLATED = CZipEntry.DEFLATED;
	 ...
	 */
	public CZipEntry getNextEntry() throws IOException {
		ensureOpen();
		if (entry != null) {
			closeEntry();
		}
	 ...
	 */
	private CZipEntry readLOC() throws IOException {
		try {
	 ...
	 */
	protected CZipEntry createZipEntry(String name) {
		return new CZipEntry(name);
	}

	/*
	 * Reads end of deflated entry as well as EXT descriptor if present.
	 */
	private void readEnd(CZipEntry e) throws IOException {
	...
	// String name;
	// try {
	// if (fileEncoding == null || fileEncoding.equals("") || this instanceof
	// JarInputStream) {
