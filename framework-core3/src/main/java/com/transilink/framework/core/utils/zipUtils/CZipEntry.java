...
public class CZipEntry implements ZipConstants, Cloneable {
	 ...
	 */
	CZipEntry(String name, long jzentry) {
		this.name = name;
		initFields(jzentry);
	}
	...
	public void setComment(String comment) {
		if (comment != null && comment.length() > 0xffff / 3
				&& CZipOutputStream.getUTF8Length(comment) > 0xffff) {
	...
	public Object clone() {
		try {
			CZipEntry e = (CZipEntry) super.clone();
