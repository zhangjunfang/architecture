...
FCK.InitializeBehaviors = function() {
	...
	if (FCKBrowserInfo.IsGecko) {
		...
		this.EditorDocument.addEventListener('dragenter', N, true);
		this.EditorDocument.addEventListener('dragover', N, true);
		this.EditorDocument.addEventListener('drop', this._ExecDrop, true);
		this.EditorDocument.addEventListener('mousedown', function(ev) {
			var O = ev.srcElement;
			if (O.nodeName.IEquals('IMG', 'HR', 'INPUT', 'TEXTAREA', 'SELECT')) {
				FCKSelection.SelectNode(O);
			}
		}, true);
		this.EditorDocument.addEventListener('mouseup', function(ev) {
					if (ev.srcElement.nodeName.IEquals('INPUT', 'TEXTAREA',
							'SELECT'))
						ev.preventDefault()
				}, true);
...
FCKTools.CreateXmlObject = function(A) {
	switch (A) {
		...
		case 'DOMDocument' :
			var B = (new DOMParser())
					.parseFromString('<tmp></tmp>', 'text/xml');
