 ...
 * 
 * Defines the FCKToolbarSet object that is used to load and draw the toolbar.
 */
		...
		default :
			...
			if (oOutMatch) {
				if (FCKBrowserInfo.IsAIR)
					...
					eToolbarTarget = eval('parent.' + oOutMatch[1]).document
							.getElementById(oOutMatch[2]);
			} else {
				// Out:[TargetId]
				oOutMatch = sLocation.match(/^Out:(\w+)$/);
				if (oOutMatch)
					eToolbarTarget = parent.document
							.getElementById(oOutMatch[1]);
			}
			...
			// If it is a shared toolbar, it may be already available in the
			// target element.
			...
			// element.
			var eToolbarIFrame = FCKTools.GetElementDocument(eToolbarTarget)
			...
			eTargetDocument
					.write('<html><head>'
							+ sBase
							+ '<script type="text/javascript"> var adjust = function() { window.frameElement.height = document.body.scrollHeight ; }; '
							+ 'window.onresize = window.onload = '
							+ 'function(){' // poll scrollHeight until it no
							// longer changes for 1 sec.
							+ 'var timer = null;'
							+ 'var lastHeight = -1;'
							+ 'var lastChange = 0;'
							+ 'var poller = function(){'
			...
			FCKTools.AddEventListener(eTargetDocument, 'contextmenu',
					FCKTools.CancelEvent);

			// Load external resources (must be done here, otherwise Firefox
			// will not
			...
			if (FCK.IECleanup)
				FCK.IECleanup.AddItem(eToolbarTarget,
...
function FCK_OnFocus(editorInstance) {
	...
	oToolbarset.CurrentInstance.FocusManager
			.RemoveWindow(oToolbarset._IFrame.contentWindow);
...
var FCKToolbarSet = function(targetDocument) {
	...
	FCKTools.AddEventListener(eExpandHandle, 'click',
			FCKToolbarSet_Expand_OnClick);

	eCollapseHandle.title = FCKLang.ToolbarCollapse;
	FCKTools.AddEventListener(eCollapseHandle, 'click',
...
FCKToolbarSet.prototype._ChangeVisibility = function(collapse) {
	this._Document.getElementById('xCollapsed').style.display = collapse
			? ''
			: 'none';
	this._Document.getElementById('xExpanded').style.display = collapse
			? 'none'
			: '';
...
FCKToolbarSet.prototype.Load = function(toolbarSetName) {
	...
	for (var x = 0; x < ToolbarSet.length; x++) {
		...
		// If the configuration for the toolbar is missing some element or has
		// any extra comma
