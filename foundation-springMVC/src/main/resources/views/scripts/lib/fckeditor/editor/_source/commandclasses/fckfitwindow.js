...
FCKFitWindow.prototype.Execute = function() {
	...
	if (!this.IsMaximized) {
		...
		// Save and reset the styles for the entire node tree. They could
		// interfere in the result.
		eParent = eEditorFrame;
		// The extra () is to avoid a warning with strict error checking. This
		// is ok.
		...
		eEditorFrame.offsetLeft; // Kludge for Safari 3.1 browser bug, do not
		// remove. See #2066.
	...
	} else // Resize to original size.
	{
		...
		if (FCKBrowserInfo.IsIE)
			...
			eMainWindow
					.removeEventListener("resize", FCKFitWindow_Resize, true);
