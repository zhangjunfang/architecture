 ...
 * 
 * This file has been compressed for better performance. The original source can
 * be found at "editor/_source".
 */
...
function FCKIECleanup_Cleanup() {
	if (!this._FCKCleanupObj
			|| (FCKConfig.MsWebBrowserControlCompat && !window.FCKUnloadFlag))
		return;
...
var FCK = {
	...
	GetData : function(A) {
		...
		var D = FCK.DataProcessor.ConvertToDataFormat(C
						? B.documentElement
	...
	SetData : function(A, B) {
		...
		if (FCK.EditMode == 0) {
			...
			A = FCK.DataProcessor.ConverttoView(A);
...
var FCKFocusManager = FCK.FocusManager = {
	...
	AddWindow : function(A, B) {
		var C;
		if (FCKBrowserInfo.IsIE)
			C = A.nodeType == 1 ? A : A.frameElement
					? A.frameElement
					: A.document;
...
function Doc_OnPaste() {
	...
	var B = FCK.Paste(!FCKConfig.ForcePasteAsPlainText
...
FCK.InsertHtml = function(A) {
	...
	if (B.type.toLowerCase() == 'control')
		B.clear();
	A = '<span id="__fakeFCKRemove__" style="display:none;">fakeFCKRemove</span>'
			+ A;
...
FCKConfig.ProtectedSource.Revert = function(A, B) {
	function _Replace(m, opener, index) {
		var C = B
				? FCKTempBin.RemoveElement(index)
	...
	CheckAndRemovePaddingNode : function(A, B, C) {
		...
		try {
			if (D.parentNode != A.body
					...
					|| (D.firstChild && D.firstChild.nodeValue != '\xa0' && String(D.firstChild.tagName)
							.toLowerCase() != 'br')) {
...
FCKTools.ConvertStyleSizetoView = function(A) {
...
FCKTools.IsStrictMode = function(A) {
	return ('CSS1Compat' == (A.compatMode || (FCKBrowserInfo.IsSafari
			? 'CSS1Compat'
			: null)));
};
...
FCKXHtml._AppendNode = function(A, B) {
	...
	switch (B.nodeType) {
		case 1 :
			...
			if (FCKBrowserInfo.IsIE) {
				if (B.scopeName && B.scopeName != 'HTML'
						&& B.scopeName != 'FCK')
		...
		case 3 :
			...
			return this
					._AppendTextNode(A, B.nodeValue.ReplaceNewLineChars(' '));
		case 8 :
		...
		default :
			A.appendChild(this.XML
					.createComment("Element not supported - Type: "
...
FCKXHtml._AppendAttributes = function(A, B, C, D) {
	...
	for (var n = 0; n < E.length; n++) {
		...
		if (F.specified) {
			...
			if (G.StartsWith('_fck'))
				...
				if (H.length == 0)
					continue;
			} else if (G.indexOf('on') == 0)
				H = F.nodeValue;
			else if (D == 'body' && G == 'contenteditable')
				continue;
			else if (F.nodeValue === true)
				H = G;
			else {
				try {
	...
	if (bHasStyle || B.style.cssText.length > 0) {
		...
		var J = B.style.cssText.replace(FCKRegexLib.StyleProperties,
				FCKTools.ToLowerCase);
...
FCKXHtml.TagProcessors['form'] = function(A, B) {
	if (B.acceptCharset && B.acceptCharset.length > 0
			&& B.acceptCharset != 'UNKNOWN')
...
FCKXHtml.TagProcessors['label'] = function(A, B) {
	if (B.htmlFor.length > 0)
		FCKXHtml._AppendAttribute(A, 'for', B.htmlFor);
	A = FCKXHtml._AppendChildNodes(A, B);
	return A;
};
...
FCKXHtml.TagProcessors['textarea'] = FCKXHtml.TagProcessors['select'] = function(
		A, B) {
	if (B.name)
		FCKXHtml._AppendAttribute(A, 'name', B.name);
	A = FCKXHtml._AppendChildNodes(A, B);
...
FCKStyle.prototype = {
								...
								if (FCKDomTools.HasAttribute(O, Q)) {
									switch (Q) {
										case 'style' :
											...
											break;
										case 'class' :
											if (FCKDomTools.GetAttributeValue(
													O, Q) != this
	...
	CheckActive : function(A) {
		switch (this.GetType()) {
			case 0 :
				return this
						.CheckElementRemovable(A.Block || A.BlockLimit, true);
			case 1 :
				var B = A.Elements;
				for (var i = 0; i < B.length; i++) {
					var C = B[i];
	...
	_GetStyleText : function() {
		...
		var B = (this._StyleDesc.Attributes
				? this._StyleDesc.Attributes['style'] || ''
				: '');
		if (B.length > 0)
			B += ';';
		for (var C in A)
...
var FCKStyles = FCK.Styles = {
	...
	_GetRemoveFormatTagsRegex : function() {
		var A = new RegExp('^(?:'
						+ FCKConfig.RemoveFormatTags.replace(/,/g, '|') + ')$',
				'i');
	...
	OutdentListItem : function(A) {
		...
		if (B.tagName.toUpperCase().Equals('UL', 'OL')) {
			...
			if (L || K.Equals('UL', 'OL')) {
				...
				var O = C.createElement(FCKConfig.EnterMode == 'p'
						? 'p'
						: 'div');
...
FCKDomRange.prototype = {
	...
	CreateBookmark2 : function() {
		...
		while (B && D.nodeType == 3) {
			A.Start[0] += B.length;
			D = B;
			B = B.previousSibling;
		}
		while (C && E.nodeType == 3) {
			A.End[0] += C.length;
			E = C;
			C = C.previousSibling;
		};
...
FCKDomRange.prototype.SelectBookmark = function(A, B) {
	...
	if (G) {
		...
		D = (B || !F.previousSibling || F.previousSibling.nodeName
				.toLowerCase() == 'br')
				&& !F.nextSibing;
		...
		if (D) {
			F.parentNode.insertBefore(this.Window.document
							.createTextNode('\ufeff'), F);
		}
	};
	if (!this._Range)
...
FCKDomRange.prototype._GetSelectionMarkerTag = function(A) {
	...
	if (D.parentElement().document != B)
		return null;
	D.collapse(A === true);
	var E = 'fck_dom_range_temp_' + (new Date()).valueOf() + '_'
...
FCKDomRangeIterator.prototype = {
	GetNextParagraph : function() {
		...
		var D;
		var E;
		var F = this.ForceBrBreak
				? FCKListsLib.ListBoundaries
...
var FCKEnterKey = function(A, B, C, D) {
	...
	E.SetKeystrokes([[13, 'Enter'], [SHIFT + 13, 'ShiftEnter'],
			[8, 'Backspace'], [CTRL + 8, 'CtrlBackspace'], [46, 'Delete']]);
	this.TabText = '';
	...
	if (D) {
		...
		if (!G && !H) {
			if (F.nodeName.IEquals('li') && F.firstChild
				...
				F.insertBefore(FCKTools.GetElementDocument(F)
								.createTextNode('\xa0'), F.firstChild);
			if (F)
...
FCKEnterKey.prototype._ExecuteEnterBr = function(A) {
	...
	if (B.StartBlockLimit == B.EndBlockLimit) {
		...
		if (!F && D && (/^H[1-6]$/).test(E)) {
			...
			G = E.IEquals('pre');
			if (G)
				H = this.Window.document.createTextNode(FCKBrowserInfo.IsIE
						? '\r'
						: '\n');
			else
...
FCKEnterKey.prototype._CheckIsAllContentsIncluded = function(A, B) {
	...
	if (A.EndContainer == B || A.EndContainer == B.lastChild) {
		var E = A.EndContainer.nodeType == 3
		...
		RefreshView : function(placeHolder, original) {
			if (original.getAttribute('width') > 0)
				placeHolder.style.width = FCKTools
						.ConvertHtmlSizeToStyle(original.getAttribute('width'));
			if (original.getAttribute('height') > 0)
				placeHolder.style.height = FCKTools
						.ConvertHtmlSizeToStyle(original.getAttribute('height'));
		},
...
FCK.GetRealElement = function(A) {
	...
	if (A.getAttribute('_fckflash')) {
		if (A.style.width.length > 0)
			e.width = FCKTools.ConvertStyleSizetoView(A.style.width);
		if (A.style.height.length > 0)
			e.height = FCKTools.ConvertStyleSizetoView(A.style.height);
	};
	return e;
};
if (FCKBrowserInfo.IsIE) {
...
if (FCKBrowserInfo.IsSafari) {
	FCKDocumentProcessor.AppendNew().ProcessDocument = function(A) {
		var B = A.getElementsByClassName
				? A.getElementsByClassName('Apple-style-span')
...
FCKSelection.Save = function(A) {
	...
	if (C) {
		...
		if (D) {
			if (D.parentElement
					&& FCKTools.GetElementDocument(D.parentElement()) != B)
				D = null;
			else if (D.item && FCKTools.GetElementDocument(D.item(0)) != B)
				D = null;
		}
	};
...
FCKSelection._GetSelectionDocument = function(A) {
	...
	if (!B)
		...
		return FCKTools.GetElementDocument(B.item(0));
	else
		return FCKTools.GetElementDocument(B.parentElement());
};
...
FCKTableHandler.DeleteRows = function(A) {
	if (!A) {
		...
		var C = [];
		for (var i = 0; i < B.length; i++) {
			var D = FCKTools.GetElementAscensor(B[i], 'TR');
			C[D.rowIndex] = D;
		};
...
FCKTableHandler.InsertColumn = function(A) {
	...
	for (var i = 0; i < D.rows.length; i++) {
		...
		if (A)
			F.insertBefore(B, G);
		else if (G.nextSibling)
			F.insertBefore(B, G.nextSibling);
		else
			F.appendChild(B);
	}
};
...
FCKTableHandler.MergeCells = function() {
	...
	if (A.length < 2)
		return;
	var B = A[0];
	var C = this._CreateTableMap(B.parentNode.parentNode);
	...
	for (var i = 0; i < F.height; i++) {
		...
		if (J > 0)
			I.appendChild(FCKTools.GetElementDocument(B).createElement('br'));
	};
	this._ReplaceCellsByMarker(C, '_SelectedCells', B);
	this._UnmarkCells(A, '_SelectedCells');
	this._InstallTableMap(C, B.parentNode.parentNode);
	B.appendChild(I);
...
FCKTableHandler.MergeDown = function() {
	...
	while (D && D.childNodes && D.childNodes.length > 0)
		E.appendChild(D.removeChild(D.firstChild));
	if (E.firstChild)
		E.insertBefore(FCKTools.GetElementDocument(D).createElement('br'),
				E.firstChild);
	B.appendChild(E);
	this._MarkCells([D], '_Replace');
	this._ReplaceCellsByMarker(C, '_Replace', B);
...
FCKTableHandler.HorizontalSplitCell = function() {
	...
	if (F > 1) {
		var G = Math.ceil(F / 2);
		var H = FCKTools.GetElementDocument(B).createElement('td');
		if (FCKBrowserInfo.IsGeckoLike)
		...
		for (var i = 0; i < C.length; i++) {
			...
			if (C[i][E] == B) {
				M.push(B);
				M.push(FCKTools.GetElementDocument(B).createElement('td'));
				if (FCKBrowserInfo.IsGeckoLike)
...
FCKTableHandler.VerticalSplitCell = function() {
	...
	var B = A[0];
	var C = this._CreateTableMap(B.parentNode.parentNode);
	var D = FCKTableHandler._GetCellIndexSpan(C, B.parentNode.rowIndex, B);
	var E = B.rowSpan;
	var F = B.parentNode.rowIndex;
	if (isNaN(E))
		E = 1;
	if (E > 1) {
		B.rowSpan = Math.ceil(E / 2);
		var G = F + Math.ceil(E / 2);
		var H = null;
		for (var i = D + 1; i < C[G].length; i++) {
			if (C[G][i].parentNode.rowIndex == G) {
				H = C[G][i];
				break;
			}
		};
		var I = FCK.EditorDocument.createElement('td');
		I.rowSpan = Math.floor(E / 2);
		if (FCKBrowserInfo.IsGeckoLike)
			FCKTools.AppendBogusBr(I);
		B.parentNode.parentNode.rows[G].insertBefore(I, H);
	} else {
		var G = F + 1;
		var K = FCK.EditorDocument.createElement('tr');
		var L = B.parentNode.parentNode;
		if (L.rows.length > G)
			L.insertBefore(K, L.rows[G]);
		else
			L.appendChild(K);
		for (var i = 0; i < C[F].length;) {
			var M = C[F][i].colSpan;
			if (isNaN(M) || M < 1)
				M = 1;
			if (i == D) {
...
FCKTableHandler._GetColumnCells = function(A, B) {
	var C = [];
...
FCKTableHandler._InstallTableMap = function(A, B) {
	...
	for (var i = 0; i < A.length; i++) {
		for (var j = 0; j < A[i].length; j++) {
			var E = A[i][j];
			if (FCKBrowserInfo.IsIE) {
				E.removeAttribute('_colScanned');
				E.removeAttribute('_rowScanned');
			} else {
				delete E._colScanned;
				delete E._rowScanned;
			}
		}
	};
	for (var i = 0; i < A.length; i++) {
		var J = FCKTools.GetElementDocument(B).createElement('tr');
	...
	if (D && D.tagName.Equals('TD', 'TH'))
		...
		if (D) {
			for (var i = 0; i < D.cells.length; i++) {
				...
				if (C.inRange(E)
						|| (C.compareEndPoints('StartToStart', E) >= 0 && C
								.compareEndPoints('StartToEnd', E) <= 0)
						|| (C.compareEndPoints('EndToStart', E) >= 0 && C
...
FCKFitWindow.prototype.Execute = function() {
	...
	var F = E.style;
	var G;
	var H = new FCKDomRange(FCK.EditorWindow);
	H.MoveToSelection();
	var I = FCKTools.GetScrollPosition(FCK.EditorWindow);
	if (!this.IsMaximized) {
		if (FCKBrowserInfo.IsIE)
	...
	FCK.Focus();
	H.Select();
	FCK.EditorWindow.scrollTo(I.X, I.Y);
};
...
FCKJustifyCommand.prototype = {
	...
	GetState : function() {
		...
		if (FCKBrowserInfo.IsIE)
			...
			C = FCK.EditorWindow.getComputedStyle(B, '')
...
var FCKIndentCommand = function(A, B) {
	this.Name = A;
	this.Offset = B;
	this.IndentCSSProperty = FCKConfig.ContentLangDirection.IEquals('ltr')
			? 'marginLeft'
			: 'marginRight';
};
	...
	_IndentBlock : function(A) {
		...
		while ((E = B.GetNextParagraph())) {
			...
			if (FCKIndentCommand._UseIndentClasses) {
				...
				E.style[this.IndentCSSProperty] = I
						? I + FCKConfig.IndentUnit
						: '';
	...
	Execute : function() {
		...
		if (A == 0) {
			...
			while (O.length > 0) {
				var S = O.shift();
				var N = S.parentNode;
				if (S == S.parentNode.firstChild) {
					N.parentNode.insertBefore(N.removeChild(S), N);
					if (!N.firstChild)
						N.parentNode.removeChild(N);
				} else if (S == S.parentNode.lastChild) {
					N.parentNode.insertBefore(N.removeChild(S), N.nextSibling);
					if (!N.firstChild)
						N.parentNode.removeChild(N);
				} else
					FCKDomTools.BreakParent(S, S.parentNode, B);
				R.push(S);
			};
			if (FCKConfig.EnterMode.IEquals('br')) {
				while (R.length) {
					var S = R.shift();
					var W = true;
					if (S.nodeName.IEquals('div')) {
						var M = FCKTools.GetElementDocument(S)
								.createDocumentFragment();
						var Y = W
								&& S.previousSibling
								&& !FCKListsLib.BlockBoundaries[S.previousSibling.nodeName
										.toLowerCase()];
						if (W && Y)
							M.appendChild(FCKTools.GetElementDocument(S)
									.createElement('br'));
						var Z = S.nextSibling
								&& !FCKListsLib.BlockBoundaries[S.nextSibling.nodeName
										.toLowerCase()];
						while (S.firstChild)
							M.appendChild(S.removeChild(S.firstChild));
						if (Z)
							M.appendChild(FCKTools.GetElementDocument(S)
									.createElement('br'));
						S.parentNode.replaceChild(M, S);
						W = false;
					}
				}
			}
	...
	switch (A) {
		...
		case 'Image' :
			B = new FCKDialogCommand('Image', FCKLang.DlgImgTitle,
					'dialog/fck_image.html', 450, 390);
			break;
		case 'Flash' :
			B = new FCKDialogCommand('Flash', FCKLang.DlgFlashTitle,
					'dialog/fck_flash.html', 450, 390);
			break;
		case 'SpecialChar' :
		...
		case 'ImageButton' :
			B = new FCKDialogCommand('ImageButton', FCKLang.ImageButton,
					'dialog/fck_image.html?ImageButton', 450, 390);
			break;
		case 'SpellCheck' :
...
FCKPanel.prototype.Show = function(x, y, A, B, C) {
	...
	if (this._Popup) {
		...
		var F = FCKTools.GetDocumentPosition(this._Window, A.nodeType == 9
						? (FCKTools.IsStrictMode(A)
								? A.documentElement
								: A.body)
						: A);
...
FCKPanel.prototype.CreateChildPanel = function() {
	var A = this._Popup
			? FCKTools.GetDocumentWindow(this.Document)
			: this._Window;
...
FCKSpecialCombo.prototype.AddItem = function(A, B, C, D) {
	...
	FCKTools.AddEventListenerEx(E, 'click', FCKSpecialCombo_ItemOnClick, [this,
					A]);
...
FCKToolbarSpecialCombo.prototype.Create = function(A) {
	this._Combo = new FCKSpecialCombo(this.GetLabel(), this.FieldWidth,
			this.PanelWidth, this.PanelMaxHeight, FCKBrowserInfo.IsIE
					? window
...
FCKToolbarItems.GetItem = function(A) {
	...
	switch (A) {
		case 'Source' :
			B = new FCKToolbarButton('Source', FCKLang.Source, null, 2, true,
					true, 1);
			break;
		case 'DocProps' :
		...
		case 'Preview' :
			...
			break;
		case 'Templates' :
			B = new FCKToolbarButton('Templates', FCKLang.Templates, null,
					null, null, null, 6);
			break;
		case 'About' :
		...
		case 'Print' :
			...
			break;
		case 'SpellCheck' :
			B = new FCKToolbarButton('SpellCheck', FCKLang.SpellCheck, null,
					null, null, null, 13);
			break;
		case 'Undo' :
		...
		case 'Redo' :
			...
			break;
		case 'SelectAll' :
			B = new FCKToolbarButton('SelectAll', FCKLang.SelectAll, null,
					null, true, null, 18);
			break;
		case 'RemoveFormat' :
			B = new FCKToolbarButton('RemoveFormat', FCKLang.RemoveFormat,
					null, null, false, true, 19);
			break;
		case 'FitWindow' :
			B = new FCKToolbarButton('FitWindow', FCKLang.FitWindow, null,
					null, true, true, 66);
			break;
		case 'Bold' :
		...
		case 'Indent' :
			...
			break;
		case 'Blockquote' :
			B = new FCKToolbarButton('Blockquote', FCKLang.Blockquote, null,
					null, false, true, 73);
			break;
		case 'CreateDiv' :
			B = new FCKToolbarButton('CreateDiv', FCKLang.CreateDiv, null,
					null, false, true, 74);
			break;
		case 'Link' :
		...
		case 'Unlink' :
			...
			break;
		case 'Anchor' :
			B = new FCKToolbarButton('Anchor', FCKLang.Anchor, null, null,
					null, null, 36);
			break;
		case 'Image' :
		...
		case 'TextColor' :
			...
			break;
		case 'BGColor' :
			B = new FCKToolbarPanelButton('BGColor', FCKLang.BGColor, null,
					null, 46);
			break;
		case 'Find' :
			B = new FCKToolbarButton('Find', FCKLang.Find, null, null, null,
					null, 16);
			break;
		case 'Replace' :
			B = new FCKToolbarButton('Replace', FCKLang.Replace, null, null,
					null, null, 17);
			break;
		case 'Form' :
			B = new FCKToolbarButton('Form', FCKLang.Form, null, null, null,
					null, 48);
			break;
		case 'Checkbox' :
		...
		case 'Radio' :
			...
			break;
		case 'TextField' :
			B = new FCKToolbarButton('TextField', FCKLang.TextField, null,
					null, null, null, 51);
			break;
		case 'Textarea' :
		...
		case 'HiddenField' :
			...
			break;
		case 'Button' :
			B = new FCKToolbarButton('Button', FCKLang.Button, null, null,
					null, null, 54);
			break;
		case 'Select' :
			B = new FCKToolbarButton('Select', FCKLang.SelectionField, null,
					null, null, null, 53);
			break;
		case 'ImageButton' :
			...
			break;
		case 'ShowBlocks' :
			B = new FCKToolbarButton('ShowBlocks', FCKLang.ShowBlocks, null,
					null, null, true, 72);
			break;
		default :
...
// var
// FCKToolbar=function(){this.Items=[];};FCKToolbar.prototype.AddItem=function(A){return
...
// this.AddItem(G);};function FCKToolbar_OnItemClick(A){var B=A._FCKToolbar;if
// (B.OnItemClick)
// B.OnItemClick(B,A);};FCKToolbar.prototype.AddSeparator=function(){this.AddItem(new
// FCKToolbarSeparator());};FCKToolbar.prototype.Create=function(A){var
// B=FCKTools.GetElementDocument(A);var
// e=B.createElement('table');e.className='TB_Toolbar';e.style.styleFloat=e.style.cssFloat=(FCKLang.Dir=='ltr'?'left':'right');e.dir=FCKLang.Dir;e.cellPadding=0;e.cellSpacing=0;var
// C=e.insertRow(-1);var D;if
// (!this.HideStart){D=C.insertCell(-1);D.appendChild(B.createElement('div')).className='TB_Start';};for
// (var i=0;i<this.Items.length;i++){this.Items[i].Create(C.insertCell(-1));};if
// (!this.HideEnd){D=C.insertCell(-1);D.appendChild(B.createElement('div')).className='TB_End';};A.appendChild(e);};var
...
FCKToolbar.prototype.Create = function(A) {
	...
	e.style.styleFloat = e.style.cssFloat = (FCKLang.Dir == 'ltr'
			? 'left'
			: 'right');
	e.dir = FCKLang.Dir;
	e.cellPadding = 0;
	e.cellSpacing = 0;
	if (typeof(FCKConfig.EnableAdvanceTable) != "undefined"
			&& FCKConfig.EnableAdvanceTable) {
		FCKConfig.AdvanceTableNum++;
		for (i = 0; i < FCKConfig.AdvanceTable.length; i++) {
			if (FCKConfig.AdvanceTableNum == FCKConfig.AdvanceTable[i]) {
				e.setAttribute('advance', true);
				e.style.display = 'none';
				break;
			};
		};
...
var FCKDialog = (function() {
	...
	var G = function() {
		...
		FCKDomTools.SetElementStyles(C, {
					'width' : Math.max(H.scrollWidth, H.clientWidth,
							E.scrollWidth || 0)
							- 1 + 'px',
		...
		case 'Generic' :
			return {
				AddItems : function(menu, tag, tagName) {
					menu.AddItem('Cut', FCKLang.Cut, 7, FCKCommands
									.GetCommand('Cut').GetState() == -1);
					menu.AddItem('Copy', FCKLang.Copy, 8, FCKCommands
									.GetCommand('Copy').GetState() == -1);
					menu.AddItem('Paste', FCKLang.Paste, 9, FCKCommands
									.GetCommand('Paste').GetState() == -1);
				}
			};
		case 'Table' :
			return {
				AddItems : function(menu, tag, tagName) {
					...
					if (C) {
						...
						D.AddItem('TableInsertCellAfter',
								FCKLang.InsertCellAfter, 58);
						D.AddItem('TableDeleteCells', FCKLang.DeleteCells, 59);
						if (FCKBrowserInfo.IsGecko)
							D.AddItem('TableMergeCells', FCKLang.MergeCells,
									60, FCKCommands
											.GetCommand('TableMergeCells')
											.GetState() == -1);
						else {
							D.AddItem('TableMergeRight', FCKLang.MergeRight,
									60, FCKCommands
											.GetCommand('TableMergeRight')
											.GetState() == -1);
						...
						D.AddItem('TableCellProp', FCKLang.CellProperties, 57,
								FCKCommands.GetCommand('TableCellProp')
										.GetState() == -1);
						menu.AddSeparator();
						D = menu.AddItem('Row', FCKLang.RowCM);
						D.AddItem('TableInsertRowBefore',
								FCKLang.InsertRowBefore, 70);
						D.AddItem('TableInsertRowAfter',
						...
						D.AddItem('TableDeleteColumns', FCKLang.DeleteColumns,
								65);
					};
					if (B || C) {
						menu.AddSeparator();
			...
			return {
				AddItems : function(menu, tag, tagName) {
					if (FCKSelection.HasAncestorNode('UL')) {
						menu.AddSeparator();
						menu.AddItem('BulletedList', FCKLang.BulletedListProp,
								27);
					}
				}
			};
			...
			return {
				AddItems : function(menu, tag, tagName) {
					if (FCKSelection.HasAncestorNode('OL')) {
						menu.AddSeparator();
						menu.AddItem('NumberedList', FCKLang.NumberedListProp,
								26);
					}
				}
			};
