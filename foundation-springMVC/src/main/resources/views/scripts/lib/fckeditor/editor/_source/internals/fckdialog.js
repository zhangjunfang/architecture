...
var FCKDialog = (function() {
	...
	var resizeHandler = function() {
		...
		var relElement = FCKTools.IsStrictMode(topDocument)
				? topDocument.documentElement
				: topDocument.body;

		FCKDomTools.SetElementStyles(cover, {
					'width' : Math.max(relElement.scrollWidth,
							relElement.clientWidth, topDocument.scrollWidth
									|| 0)
							- 1 + 'px',
					'height' : Math.max(relElement.scrollHeight,
							relElement.clientHeight, topDocument.scrollHeight
									|| 0)
							- 1 + 'px'
				});
	}
	...
	return {
		...
		OpenDialog : function(dialogName, dialogTitle, dialogPage, width,
			...
			var useAbsolutePosition = FCKBrowserInfo.IsIE
					&& (!FCKBrowserInfo.IsIE7 || !FCKTools
							.IsStrictMode(topWindow.document));
			if (useAbsolutePosition)
				scrollPosition = FCKTools.GetScrollPosition(topWindow);
			var iTop = Math.max(scrollPosition.Y
							+ (viewSize.Height - height - 20) / 2, 0);
			var iLeft = Math.max(scrollPosition.X
							+ (viewSize.Width - width - 20) / 2, 0);
			...
			// Dummy URL for testing whether the code in fckdialog.js alone
			// leaks memory.
		...
		/**
		 * (For internal use) Called when the top dialog is closed.
		 */
		OnDialogClose : function(dialogWindow) {
			...
			} else // First Dialog.
			{
				...
				this.HideMainCover();
				// Bug #1918: Assigning topDialog = null directly causes IE6 to
				// crash.
				setTimeout(function() {
							topDialog = null;
						}, 0);
		...
		DisplayMainCover : function() {
			...
			var el = FCK.ToolbarSet.CurrentInstance
