...
FCKCodeFormatter._ProtectData = function(outer, opener, data, closer) {
	return opener + '___FCKpd___'
			+ FCKCodeFormatter.ProtectedData.AddItem(data) + closer;
}
...
FCKCodeFormatter.Format = function(html) {
	...
	var sFormatted = html.replace(this.Regex.ProtectedTags,
			FCKCodeFormatter._ProtectData);
	...
	for (var j = 0; j < FCKCodeFormatter.ProtectedData.length; j++) {
		...
		sFormatted = sFormatted.replace(oRegex,
				FCKCodeFormatter.ProtectedData[j].replace(/\$/g, '$$$$'));
	}
