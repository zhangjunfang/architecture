...
FCKSelection.GetParentElement = function() {
	if (this.GetType() == 'Control')
		...
		if (oSel) {
			// if anchorNode == focusNode, see if the selection is text only or
			// including nodes.
			// if text only, return the parent node.
			// if the selection includes DOM nodes, then the anchorNode is the
			// nearest container.
			...
			// looks like we're having a large selection here. To make the
			// behavior same as IE's TextRange.parentElement(),
			// we need to find the nearest ancestor node which encapsulates both
...
FCKSelection.GetBoundaryParentElement = function(startBoundary) {
	...
	if (this.GetType() == 'Control')
		...
		if (oSel && oSel.rangeCount > 0) {
			var range = oSel.getRangeAt(startBoundary
					? 0
					: (oSel.rangeCount - 1));

			var element = startBoundary
