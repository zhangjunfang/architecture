...
FCKTableHandler.DeleteColumns = function(oCell) {
	...
	for (var i = oTable.rows.length - 1; i >= 0; i--) {
		...
		// If the cell to be removed is the first one and the row has just one
		// cell.
...
FCKTableHandler.InsertCell = function(cell, insertBefore) {
	...
	if (!insertBefore && oCell.cellIndex == oCell.parentNode.cells.length - 1)
		...
		oCell.parentNode.insertBefore(oNewCell, insertBefore
						? oCell
						: oCell.nextSibling);
	...
	if (cellSpan > 1) {
		...
		for (var i = 0; i < tableMap.length; i++) {
			...
			if (tableMap[i][colIdx] == refCell) {
				newRow.push(refCell);
				newRow.push(FCKTools.GetElementDocument(refCell)
						.createElement('td'));
				if (FCKBrowserInfo.IsGeckoLike)
...
FCKTableHandler._InstallTableMap = function(tableMap, table) {
	// Workaround for #1917 : MSIE will always report a cell's rowSpan as 1 as
	// long
	// as the cell is not attached to a row. So we'll need an alternative
	// attribute
