 ...
 * represents an editor instance. (Gecko specific implementations)
 */

FCK.Description = "FCKeditor for Gecko Browsers";

FCK.InitializeBehaviors = function() {
	// When calling "SetData", the editing area IFRAME gets a fixed height. So
	...
	this._ExecCheckCaret = function(evt) {
		...
		if (evt.type == 'keypress') {
			...
			// left or up arrow keys need to be processed as well, since <a>
			// links can be expanded in Gecko's editor
		...
		var moveCursor = function() {
			...
			// only perform the patched behavior if we're at the end of a text
			// node.
			if (node.nodeType != 3)
				return;
			...
			if (!(parentTag == 'a'
					|| (!FCKBrowserInfo.IsOpera && String(node.parentNode.contentEditable) == 'false') || (!(FCKListsLib.BlockElements[parentTag] || FCKListsLib.NonEmptyBlockElements[parentTag]) && keyCode == 35)))
				return;

			// our caret has moved to just after the last character of a text
			// node under an unknown tag, how to proceed?
			// first, see if there are other text nodes by DFS walking from this
			// text node.
			// - if the DFS has scanned all nodes under my parent, then go the
			// next step.
			// - if there is a text node after me but still under my parent,
			...
			nextTextNode = FCKTools.GetNextTextNode(node,
					node.parentNode.parentNode, blockEmptyStop);
			if (nextTextNode) {
				// Opera thinks the dummy empty text node we append beyond the
				// end of <a> nodes occupies a caret
				// position. So if the user presses the left key and we reset
				// the caret position here, the user
				// wouldn't be able to go back.
				if (FCKBrowserInfo.IsOpera && keyCode == 37)
					return;

				// now we want to get out of our current parent node, adopt the
				// next parent, and move the caret to
				// the appropriate text node under our new parent.
				// our new parent might be our current parent's siblings if we
				// are lucky.
				range.setStart(nextTextNode, 0);
				range.setEnd(nextTextNode, 0);
			} else {
				// no suitable next siblings under our grandparent! what to do
				// next?
				while (node.parentNode
						&& node.parentNode != FCK.EditorDocument.body
						&& node.parentNode != FCK.EditorDocument.documentElement
						&& node == node.parentNode.lastChild
						&& (!FCKListsLib.BlockElements[node.parentNode.tagName
								.toLowerCase()] && !FCKListsLib.NonEmptyBlockElements[node.parentNode.tagName
								.toLowerCase()]))
				...
				if (FCKListsLib.BlockElements[parentTag]
					...
					// if our parent is a block node, move to the end of our
					// parent.
					range.setStart(node, node.childNodes.length);
					range.setEnd(node, node.childNodes.length);
				} else {
					// things are a little bit more interesting if our parent is
					// not a block node
					// due to the weired ways how Gecko's caret acts...
					var stopNode = node.nextSibling;

					// find out the next block/empty element at our grandparent,
					// we'll
					...
					while (stopNode) {
						...
						if (FCKListsLib.BlockElements[stopTag]
								|| FCKListsLib.EmptyElements[stopTag]
								|| FCKListsLib.NonEmptyBlockElements[stopTag])
							break;
						stopNode = stopNode.nextSibling;
					}

					// note that the dummy marker below is NEEDED, otherwise the
					// caret's behavior will
					// be broken in Gecko.
	...
	// On Gecko, firing the "OnSelectionChange" event on every key press started
	// to be too much
	// slow. So, a timer has been implemented to solve performance issues when
	// typing to quickly.
	...
	if (FCKBrowserInfo.IsGecko) {
		...
		this.EditorDocument.addEventListener('mousedown', function(ev) {
					...
					if (element.nodeName.IEquals('IMG', 'HR', 'INPUT',
							'TEXTAREA', 'SELECT')) {
...
FCK.InsertHtml = function(html) {
	var doc = FCK.EditorDocument, range;
	...
	// For some strange reason the SaveUndoStep() call doesn't activate the undo
	// button at the first InsertHtml() call.
...
FCK.PasteAsPlainText = function() {
	...
	// If the function is called immediately Firefox 2 does automatically paste
	// the contents as soon as the new dialog is created
	...
	/*
	 * var sText = FCKTools.HTMLEncode( clipboardData.getData("Text") ) ; sText =
	 * sText.replace( /\n/g, '<BR>' ) ; this.InsertHtml( sText ) ;
	 */
}
/*
 * FCK.PasteFromWord = function() { // TODO: Implement the "Paste as Plain Text"
 * code.
 * 
 * FCKDialog.OpenDialog( 'FCKDialog_Paste', FCKLang.PasteFromWord,
 * 'dialog/fck_paste.html', 400, 330, 'Word' ) ; // FCK.CleanAndPaste(
...
FCK.CreateLink = function(url, noUndo) {
	// Creates the array that will be returned. It contains one or more created
	// links (see #220).
	...
	if (url.length > 0) {
		...
		var oLinksInteractor = this.EditorDocument.evaluate("//a[@href='"
						+ sTempUrl + "']", this.EditorDocument.body, null,
