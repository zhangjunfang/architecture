...
FCKBlockQuoteCommand.prototype = {
	Execute : function() {
		...
		if (state == FCK_TRISTATE_OFF) {
			...
			if (paragraphs.length < 1) {
				para = range.Window.document.createElement(FCKConfig.EnterMode
						.IEquals('p') ? 'p' : 'div');
				range.InsertNode(para);
				para
						.appendChild(range.Window.document
								.createTextNode('\ufeff'));
			...
			for (var i = 0; i < paragraphs.length; i++) {
				block = paragraphs[i];
				commonParent = FCKDomTools.GetCommonParents(block.parentNode,
						commonParent).pop();
			}
			var lastBlock = null;
			...
			while (tmp.length > 0) {
				...
				if (block.nodeName.IEquals('blockquote')) {
					var docFrag = FCKTools.GetElementDocument(block)
							.createDocumentFragment();
					while (block.firstChild) {
						docFrag
								.appendChild(block
										.removeChild(block.firstChild));
			...
			while (moveOutNodes.length > 0) {
				...
				if (node == node.parentNode.firstChild) {
					bqBlock.parentNode.insertBefore(bqBlock.removeChild(node),
							bqBlock);
			...
			if (FCKConfig.EnterMode.IEquals('br')) {
				while (movedNodes.length) {
					...
					if (node.nodeName.IEquals('div')) {
						var docFrag = FCKTools.GetElementDocument(node)
								.createDocumentFragment();
						var needBeginBr = firstTime
								&& node.previousSibling
								&& !FCKListsLib.BlockBoundaries[node.previousSibling.nodeName
										.toLowerCase()];
						if (firstTime && needBeginBr)
							docFrag.appendChild(FCKTools
									.GetElementDocument(node)
									.createElement('br'));
						var needEndBr = node.nextSibling
								&& !FCKListsLib.BlockBoundaries[node.nextSibling.nodeName
										.toLowerCase()];
	...
	GetState : function() {
		...
		var path = new FCKElementPath(FCKSelection
