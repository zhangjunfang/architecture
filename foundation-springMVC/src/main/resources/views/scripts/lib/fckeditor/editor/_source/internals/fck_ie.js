 ...
 * represents an editor instance. (IE specific implementations)
 */
...
function Doc_OnPaste() {
	...
	var ret = FCK.Paste(!FCKConfig.ForcePasteAsPlainText
...
FCK.InsertHtml = function(html) {
	...
	// be preserved.
	html = '<span id="__fakeFCKRemove__" style="display:none;">fakeFCKRemove</span>'
			+ html;
...
FCK._ExecPaste = function() {
	// As we call ExecuteNamedCommand('Paste'), it would enter in a loop. So,
	...
	if (sHTML === false)
		...
		// Instead of inserting the retrieved HTML, let's leave the OS work for
		// us,
		// by calling FCK.ExecuteNamedCommand( 'Paste' ). It could give better
		// results.
...
FCK.CreateLink = function(url, noUndo) {
	...
	if (url.length > 0) {
		// If there are several images, and you try to link each one, all the
		// images get inside the link:
		// <img><img> -> <a><img></a><img> -> <a><img><img></a> due to the call
		// to 'CreateLink' (bug in IE)
		...
		for (i = 0; i < oLinks.length; i++) {
			...
			// getAttribute must be used. oLink.url may cause problems with IE7
			// (#555).
			if (oLink.getAttribute('href', 2) == sTempUrl) {
				var sInnerHtml = oLink.innerHTML; // Save the innerHTML (IE
				// changes it if it is like
				// an URL).
				oLink.href = url;
				...
				if (oLastChild && oLastChild.nodeName == 'BR') {
					...
					FCKDomTools.InsertAfterNode(oLink, oLink
									.removeChild(oLastChild));
				}
