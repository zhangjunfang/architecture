...
var FCKDomTools = {
	...
	MoveChildren : function(source, target, toTargetStart) {
		...
		if (toTargetStart) {
			while ((eChild = source.lastChild))
				target.insertBefore(source.removeChild(eChild),
						target.firstChild);
		} else {
	...
	MoveNode : function(source, target, toTargetStart) {
		if (toTargetStart)
			target.insertBefore(FCKDomTools.RemoveNode(source),
					target.firstChild);
		else
			target.appendChild(FCKDomTools.RemoveNode(source));
	},

	// Remove blank spaces from the beginning and the end of the contents of a
	// node.
	...
	RTrimNode : function(node) {
		...
		while ((eChildNode = node.lastChild)) {
			if (eChildNode.nodeType == 3) {
				...
				if (sTrimmed.length == 0) {
					...
					// Use "eChildNode.parentNode" instead of "node" to avoid IE
					// bug (#81).
					...
					// Use "node.lastChild.parentNode" instead of "node" to
					// avoid IE bug (#81).
		...
		if (!FCKBrowserInfo.IsIE && !FCKBrowserInfo.IsOpera) {
			...
			if (eChildNode && eChildNode.nodeType == 1
					&& eChildNode.nodeName.toLowerCase() == 'br') {
				// Use "eChildNode.parentNode" instead of "node" to avoid IE bug
				// (#324).
				eChildNode.parentNode.removeChild(eChildNode);
			}
		}
	...
	GetFirstChild : function(node, childNames) {
		...
		while (eChild) {
			if (eChild.nodeType == 1
					&& eChild.tagName.Equals.apply(eChild.tagName, childNames))
				return eChild;
	...
	GetLastChild : function(node, childNames) {
		...
		while (eChild) {
			if (eChild.nodeType == 1
					&& (!childNames || eChild.tagName.Equals(childNames)))
				return eChild;
	 ...
	 * "null" If no element is found. @param {Object} currentNode The node to
	 * start searching from. @param {Boolean} ignoreSpaceTextOnly Sets how text
	 * nodes will be handled. If set to "true", only white spaces text nodes
	 * will be ignored, while non white space text nodes will stop the search,
	 * returning null. If "false" or omitted, all text nodes are ignored. @param
	 * {string[]} stopSearchElements An array of element names that will cause
	 * the search to stop when found, returning null. May be omitted (or null).
	 * @param {string[]} ignoreElements An array of element names that must be
	 * ignored during the search.
	 */
	GetPreviousSourceElement : function(currentNode, ignoreSpaceTextOnly,
			stopSearchElements, ignoreElements) {
		if (!currentNode)
			return null;

		if (stopSearchElements && currentNode.nodeType == 1
				&& currentNode.nodeName.IEquals(stopSearchElements))
			return null;

		if (currentNode.previousSibling)
			currentNode = currentNode.previousSibling;
		else
			return this.GetPreviousSourceElement(currentNode.parentNode,
					ignoreSpaceTextOnly, stopSearchElements, ignoreElements);

		while (currentNode) {
			if (currentNode.nodeType == 1) {
				if (stopSearchElements
						&& currentNode.nodeName.IEquals(stopSearchElements))
					break;

				if (!ignoreElements
						|| !currentNode.nodeName.IEquals(ignoreElements))
					return currentNode;
			} else if (ignoreSpaceTextOnly && currentNode.nodeType == 3
					&& currentNode.nodeValue.RTrim().length > 0)
				break;
			...
			if (currentNode.lastChild)
				...
				return this
						.GetPreviousSourceElement(currentNode,
								ignoreSpaceTextOnly, stopSearchElements,
								ignoreElements);
		}
	...
	/*
	 * Gets the next element (nodeType=1) in the source order. Returns "null" If
	 * no element is found. @param {Object} currentNode The node to start
	 * searching from. @param {Boolean} ignoreSpaceTextOnly Sets how text nodes
	 * will be handled. If set to "true", only white spaces text nodes will be
	 * ignored, while non white space text nodes will stop the search, returning
	 * null. If "false" or omitted, all text nodes are ignored. @param
	 * {string[]} stopSearchElements An array of element names that will cause
	 * the search to stop when found, returning null. May be omitted (or null).
	 * @param {string[]} ignoreElements An array of element names that must be
	 * ignored during the search.
	 */
	GetNextSourceElement : function(currentNode, ignoreSpaceTextOnly,
			stopSearchElements, ignoreElements, startFromSibling) {
		while ((currentNode = this.GetNextSourceNode(currentNode,
				startFromSibling))) // Only one "=".
		{
			if (currentNode.nodeType == 1) {
				if (stopSearchElements
						&& currentNode.nodeName.IEquals(stopSearchElements))
				...
				if (ignoreElements
						&& currentNode.nodeName.IEquals(ignoreElements))
					return this.GetNextSourceElement(currentNode,
							ignoreSpaceTextOnly, stopSearchElements,
							ignoreElements);
	...
	GetNextSourceNode : function(currentNode, startFromSibling, nodeType,
		...
		if (!startFromSibling && currentNode.firstChild)
			...
			if (!node
					&& (!stopSearchNode || stopSearchNode != currentNode.parentNode))
				return this.GetNextSourceNode(currentNode.parentNode, true,
						nodeType, stopSearchNode);
		}

		if (nodeType && node && node.nodeType != nodeType)
			return this
					.GetNextSourceNode(node, false, nodeType, stopSearchNode);
	...
	GetPreviousSourceNode : function(currentNode, startFromSibling, nodeType,
		...
		if (!startFromSibling && currentNode.lastChild)
			...
			if (!node
					&& (!stopSearchNode || stopSearchNode != currentNode.parentNode))
				return this.GetPreviousSourceNode(currentNode.parentNode, true,
						nodeType, stopSearchNode);
		}
	...
	EnforcePaddingNode : function(doc, tagName) {
		// In IE it can happen when the page is reloaded that doc or doc.body is
		// null, so exit here
		try {
			if (!doc || !doc.body)
				return;
		...
		try {
			if (doc.body.lastChild
					&& (doc.body.lastChild.nodeType != 1 || doc.body.lastChild.tagName
		...
		if (FCKBrowserInfo.IsGecko
				&& FCKListsLib.NonEmptyBlockElements[tagName])
		...
		if (doc.body.childNodes.length == 1
				...
				&& (doc.body.firstChild.getAttribute('_moz_dirty') != null || doc.body.firstChild
	...
	CheckAndRemovePaddingNode : function(doc, tagName, dontRemove) {
		...
		try {
			if (paddingNode.parentNode != doc.body
					...
					|| (paddingNode.firstChild
							&& paddingNode.firstChild.nodeValue != '\xa0' && String(paddingNode.firstChild.tagName)
							.toLowerCase() != 'br')) {
	...
	HasAttributes : function(element) {
		...
		for (var i = 0; i < attributes.length; i++) {
			if (FCKBrowserInfo.IsIE && attributes[i].nodeName == 'class') {
				// IE has a strange bug. If calling
				// removeAttribute('className'),
				// the attributes collection will still contain the "class"
				// attribute, which will be marked as "specified", even if the
				// outerHTML of the element is not displaying the class
				// attribute.
	 ...
	 * For example, in the following case: <b>This <i>is some<span /> sample</i>
	 * test text</b> If element = <span />, we have these results: <b>This
	 * <i>is some</i><span /><i> sample</i> test text</b> (If parent = <i>)
	 * <b>This <i>is some</i></b><span /><b><i> sample</i> test text</b>
	 * (If parent = <b>)
	 */
	BreakParent : function(element, parent, reusableRange) {
		var range = reusableRange
				|| new FCKDomRange(FCKTools.GetElementWindow(element));
	...
	/**
	 * Retrieves a uniquely identifiable tree address of a DOM tree node. The
	 * tree address returns is an array of integers, with each integer
	 ...
	 * fourth child of body, the tree address returned would be: [1, 3, 2]
	 * 
	...
	GetNodeAddress : function(node, normalized) {
		var retval = [];
		while (node
				&& node != FCKTools.GetElementDocument(node).documentElement) {
			var parentNode = node.parentNode;
			var currentIndex = -1;
			for (var i = 0; i < parentNode.childNodes.length; i++) {
				var candidate = parentNode.childNodes[i];
				if (normalized === true && candidate.nodeType == 3
	...
	/**
	 * The reverse transformation of FCKDomTools.GetNodeAddress(). This function
	 * returns the DOM node pointed to by its index address.
	 */
	 ...
	 */
	ListToArray : function(listNode, markerObj, baseArray, baseIndentLevel,
			grandparentNode) {
		...
		if (!baseArray)
			baseArray = [];
		// Iterate over all list items to get their contents and look for inner
		// lists.
		for (var i = 0; i < listNode.childNodes.length; i++) {
			...
			if (!grandparentNode) {
				...
				if (itemObj.grandparent
						&& itemObj.grandparent.nodeName.IEquals('li'))
					itemObj.grandparent = itemObj.grandparent.parentNode;
			} else
				itemObj.grandparent = grandparentNode;
			if (markerObj)
				this.SetElementMarker(markerObj, listItem,
						'_FCK_ListArray_Index', baseArray.length);
			...
			for (var j = 0; j < listItem.childNodes.length; j++) {
				...
				if (child.nodeName.IEquals(['ul', 'ol']))
					...
					this.ListToArray(child, markerObj, baseArray,
	...
	ArrayToList : function(listArray, markerObj, baseIndex) {
		...
		while (true) {
			...
			if (item.indent == indentLevel) {
				...
				for (var i = 0; i < item.contents.length; i++)
					currentListItem.appendChild(item.contents[i]
							.cloneNode(true));
				currentIndex++;
				...
				if (item.grandparent.nodeName.IEquals(['ul', 'ol']))
					...
					if (FCKConfig.EnterMode.IEquals(['div', 'p'])
							&& !item.grandparent.nodeName.IEquals('td'))
						currentListItem = doc
				...
				if (currentListItem.nodeName.IEquals(FCKConfig.EnterMode)
						&& currentListItem.firstChild) {
					this.TrimNode(currentListItem);
					if (FCKListsLib.BlockBoundaries[currentListItem.firstChild.nodeName
							.toLowerCase()]) {
						var tmp = doc.createDocumentFragment();
						while (currentListItem.firstChild)
							tmp.appendChild(currentListItem
									.removeChild(currentListItem.firstChild));
						currentListItem = tmp;
					}
				}
				if (FCKBrowserInfo.IsGeckoLike
						&& currentListItem.nodeName.IEquals(['div', 'p']))
		...
		// Clear marker attributes for the new list tree made of cloned nodes,
		// if any.
		if (markerObj) {
	...
	/**
	 * Get the next sibling node for a node. If "includeEmpties" is false, only
	 * element or non empty text nodes are returned.
	 */
	GetNextSibling : function(node, includeEmpties) {
		node = node.nextSibling;

		while (node && !includeEmpties && node.nodeType != 1
				&& (node.nodeType != 3 || node.nodeValue.length == 0))
			node = node.nextSibling;
	...
	/**
	 * Checks if an element has no "useful" content inside of it node tree. No
	 * "useful" content means empty text node or a signle empty inline node.
	...
	CheckIsEmptyElement : function(element, elementCheckCallback) {
		...
		while (child) {
			if (child.nodeType == 1) {
				if (elementChild
						|| !FCKListsLib.InlineNonEmptyElements[child.nodeName
								.toLowerCase()])
					return false;

				if (!elementCheckCallback
						|| elementCheckCallback(child) === true)
					elementChild = child;
		...
		return elementChild ? this.CheckIsEmptyElement(elementChild,
				elementCheckCallback) : true;
	},
	...
	ScrollIntoView : function(element, alignTop) {
		...
		if (alignTop === false) {
			...
			// in other places.
			offset += parseInt(this.GetCurrentElementStyle(element,
							'marginBottom')
							|| 0, 10)
					|| 0;
		}
		...
		if (offset > 0
				&& (offset > currentScroll || offset < currentScroll
						- windowHeight))
	...
	GetSelectedDivContainers : function() {
		...
		while (currentNode && currentNode != endNode) {
			if (currentNode.nodeType != 3
				...
				if (blockLimit && blockLimit.nodeName.IEquals('div')
