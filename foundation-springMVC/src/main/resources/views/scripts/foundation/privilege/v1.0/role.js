...
function reloadRoleList(offset){
	...
	loadDiv('roleListGrid',params,null);
}


// 赋值资源UI
function openDistributionUI(roleid) {
	$.post('openDistributionUI', {roleid:roleid},function(data) {
		$.dialog( {
...
function openDepartmentAuthorizeUI(roleid) {
	$.post('openDepartmentAuthorizeUI', {roleid:roleid},function(data) {
		$.dialog( {
...
function submitCheckboxDepartmentAuthorize(){
	var data = {};
	data["data"] = {};
	var url = "openDepartmentAuthorize";
	var str = "";
	if($(".checkValue").length <=0){
		alert("请选择资源！");
		return false;
