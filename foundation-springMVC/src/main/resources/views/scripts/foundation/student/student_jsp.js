...
function reloadStuList(offset){
	...
	//拼json参数
	var params = {offset:offset,stuname:stuname};
	//return;
						...
						function(data) {
							$.dialog( {
								...
								init: function(){
									...
									$("#className").val(__data.data.className);
									$("#no").val(__data.data.no);
									$(":radio[name='sex'][value='"+__data.data.sex+"']").attr("checked","checked");
									$("#photoimg").attr("src","/upload/"+__data.data.photo);
									$("#birthday").val(__data.data.birthday);
								}
							});
				});
			}else{
...
function updateStu(){
	...
	var photo=$("#photo").val();
	if (!/\.(png|gif|bmp|jpg|jpeg)$/i.test(photo) && photo != "" && photo!= undefined) {
		alert("请上传png、gif、bmp、jpg、jpeg格式的图片！");
		return;
	}
	$("#stuform").ajaxSubmit({
