...
/*
 * JQuery zTree core 3.5.13 http://zTree.me/
 * 
 ...
 * 
 * email: hunter.z@263.net Date: 2013-06-02
 */
(function($) {
	...
	event = {
		...
		bindTree : function(setting) {
			...
			o.bind('selectstart', function(e) {
						var n = e.originalEvent.srcElement.nodeName
								.toLowerCase();
	...
	tools = {
		...
		$ : function(node, exp, setting) {
			...
			if (typeof node == "string") {
				return $(node, setting
								? setting.treeObj.get(0).ownerDocument
								: null);
			} else {
		...
		getNodeMainDom : function(target) {
			return ($(target).parent("li").get(0) || $(target)
					.parentsUntil("li").parent().get(0));
		},
	...
	view = {
		addNodes : function(setting, parentNode, newNodes, isSilent) {
			...
			if (parentNode) {
				var target_switchObj = $$(parentNode, consts.id.SWITCH, setting), target_icoObj = $$(
						parentNode, consts.id.ICON, setting), target_ulObj = $$(
						parentNode, consts.id.UL, setting);
		...
		appendParentULDom : function(setting, node) {
			...
			if (!ulObj.get(0)) {
				var childKey = setting.data.key.children, childHtml = view
						.appendNodes(setting, node.level + 1, node[childKey],
								node, false, true);
		...
		expandCollapseNode : function(setting, node, expandFlag, animateFlag,
			...
			if (!node.open
					&& node.isParent
					&& ((!$$(node, consts.id.UL, setting).get(0)) || (node[childKey]
			...
			var ulObj = $$(node, consts.id.UL, setting), switchObj = $$(node,
					consts.id.SWITCH, setting), icoObj = $$(node,
					consts.id.ICON, setting);
		...
		makeDOMNodeMainBefore : function(html, setting, node) {
			html.push("<li id='", node.tId, "' class='",
		...
		makeDOMNodeNameBefore : function(html, setting, node) {
			...
			html.push("<a id='", node.tId, consts.id.A, "' class='",
					consts.className.LEVEL, node.level, "' treeNode",
					consts.id.A, " onclick=\"", (node.click || ''), "\" ",
		...
		makeUlHtml : function(setting, node, html, content) {
			html.push("<ul id='", node.tId, consts.id.UL, "' class='",
					consts.className.LEVEL, node.level, " ", view
							.makeUlLineClass(setting, node),
		...
		removeChildNodes : function(setting, node) {
			...
			if (!setting.data.keep.parent) {
				node.isParent = false;
				node.open = false;
				var tmp_switchObj = $$(node, consts.id.SWITCH, setting), tmp_icoObj = $$(
						node, consts.id.ICON, setting);
				view
			...
			if (!setting.data.keep.parent && childLength == 0) {
				...
				if (parentNode == root) {
					if (parentNode[childKey].length == 1) {
						...
						var tmp_first_switchObj = $$(parentNode[childKey][0],
								consts.id.SWITCH, setting);
		...
		setNodeFontCss : function(setting, treeNode) {
			var aObj = $$(treeNode, consts.id.A, setting), fontCss = view
		...
		setNodeLineIcos : function(setting, node) {
			if (!node)
				return;
			var switchObj = $$(node, consts.id.SWITCH, setting), ulObj = $$(
					node, consts.id.UL, setting), icoObj = $$(node,
					consts.id.ICON, setting), ulLine = view.makeUlLineClass(
					setting, node);
			if (ulLine.length == 0) {
		...
		setNodeName : function(setting, node) {
			var title = data.getNodeTitle(setting, node), nObj = $$(node,
					consts.id.SPAN, setting);
			nObj.empty();
		...
		setNodeUrl : function(setting, node) {
			var aObj = $$(node, consts.id.A, setting), url = view.makeNodeUrl(
					setting, node);
			...
			var zTreeTools = {
				...
				expandNode : function(node, expandFlag, sonSign, focus,
					...
					if (expandFlag && node.parentTId) {
						view.expandCollapseParentNode(setting, node
					...
					if (!tools.canAsync(setting, node) && sonSign) {
						view.expandCollapseSonNode(setting, node, expandFlag,
								true, function() {
									if (focus !== false) {
										try {
				...
				getNodeByParam : function(key, value, parentNode) {
					if (!key)
						return null;
					return data.getNodeByParam(setting, parentNode
				...
				getNodesByParam : function(key, value, parentNode) {
					if (!key)
						return null;
					return data.getNodesByParam(setting, parentNode
									? parentNode[setting.data.key.children]
									: data.getNodes(setting), key, value);
				},
				getNodesByParamFuzzy : function(key, value, parentNode) {
					if (!key)
						return null;
					return data.getNodesByParamFuzzy(setting, parentNode
									? parentNode[setting.data.key.children]
									: data.getNodes(setting), key, value);
				},
				getNodesByFilter : function(filter, isSingle, parentNode,
					...
					if (!filter || (typeof filter != "function"))
						return (isSingle ? null : []);
					return data.getNodesByFilter(setting, parentNode
									? parentNode[setting.data.key.children]
									: data.getNodes(setting), filter, isSingle,
							invokeParam);
				},
				...
				reAsyncChildNodes : function(parentNode, reloadType, isSilent) {
					...
					if (reloadType == "refresh") {
						...
						for (var i = 0, l = parentNode[childKey]
								? parentNode[childKey].length
								: 0; i < l; i++) {
				...
				refresh : function() {
					...
					data.initCache(setting);
					view.createNodes(setting, 0,
							root[setting.data.key.children]);
				},
...
/*
 * JQuery zTree excheck 3.5.13 http://zTree.me/
 * 
 * Copyright (c) 2010 Hunter.z
 * 
 * Licensed same as jquery - MIT License
...
(function($) {
	...
	_initNode = function(setting, level, n, parentNode, isFirstNode,
		...
		n.chkDisabled = !!n.chkDisabled
				|| (setting.check.chkDisabledInherit && parentNode && !!parentNode.chkDisabled);
	...
	_zTreeTools = function(setting, zTreeTools) {
		zTreeTools.checkNode = function(node, checked, checkTypeFlag,
			...
			if (tools.uCanDo(this.setting) && setting.check.enable
					&& node.nocheck !== true) {
		...
		zTreeTools.getCheckedNodes = function(checked) {
			...
			return data.getTreeCheckedNodes(setting,
					data.getRoot(setting)[childKey], checked);
		}

		zTreeTools.getChangeCheckedNodes = function() {
			var childKey = setting.data.key.children;
			return data.getTreeChangeCheckedNodes(setting, data
				...
				if (node[checkedKey]) {
					if (setting.check.radioType == r.TYPE_ALL) {
						for (i = checkedList.length - 1; i >= 0; i--) {
							...
							view.setChkClass(setting, $$(pNode,
											consts.id.CHECK, setting), pNode);
			...
			if (node.parentTId) {
				...
				if (!value) {
					...
					for (var i = 0, l = pNodes.length; i < l; i++) {
						if ((pNodes[i].nocheck !== true
								&& pNodes[i].chkDisabled !== true && pNodes[i][checkedKey])
...
/*
 * JQuery zTree exedit 3.5.13 http://zTree.me/
 * 
 * Copyright (c) 2010 Hunter.z
 * 
 * Licensed same as jquery - MIT License
...
(function($) {
			...
			if (targetNode && !targetNode.isParent && setting.data.keep.leaf
				...
				return null;
					...
					&& ((node.parentTId == targetNode.tId && moveType == consts.move.TYPE_INNER) || $$(
							node, setting).find("#" + targetNode.tId).length > 0)) {
				return null;
			} else if (!targetNode) {
				targetNode = null;
			}
	...
	_handler = {
		...
		onMousedownNode : function(eventMouseDown, _node) {
			var i, l, setting = data.getSetting(eventMouseDown.data.treeId), root = data
					.getRoot(setting), roots = data.getRoots();
			...
			var doc = $(setting.treeObj.get(0).ownerDocument), body = $(setting.treeObj
					.get(0).ownerDocument.body), curNode, tmpArrow, tmpTarget, isOtherTree = false, targetSetting = setting, sourceSetting = setting, preNode, nextNode, preTmpTargetNodeId = null, preTmpMoveType = null, tmpTargetNodeId = null, moveType = consts.move.TYPE_INNER, mouseDownX = eventMouseDown.clientX, mouseDownY = eventMouseDown.clientY, startTime = (new Date())
					.getTime();
				...
				if (root.dragFlag == 0) {
					...
					for (i = 0, l = nodes.length; i < l; i++) {
						...
						tmpDom = $$("<li id='" + tmpNode.tId + "_tmp'></li>",
								setting);
						tmpDom
								.append($$(tmpNode, consts.id.A, setting)
										.clone());
		...
		showIfameMask : function(setting, showSign) {
			...
			if (showSign) {
				...
				for (var i = 0, l = iframeList.length; i < l; i++) {
					var obj = iframeList.get(i), r = tools.getAbs(obj), dragMask = $$(
							"<div id='zTreeMask_" + i
									+ "' class='zTreeMask' style='top:" + r[1]
									+ "px; left:" + r[0] + "px; width:"
									+ obj.offsetWidth + "px; height:"
	...
	_view = {
		addEditBtn : function(setting, node) {
			if (node.editNameFlag
					|| $$(node, consts.id.EDIT, setting).length > 0) {
				return;
			}
			...
			var aObj = $$(node, consts.id.A, setting), editStr = "<span class='"
		...
		addRemoveBtn : function(setting, node) {
			...
			var aObj = $$(node, consts.id.A, setting), removeStr = "<span class='"
					+ consts.className.BUTTON
					+ " remove' id='"
					+ node.tId
					+ consts.id.REMOVE
		...
		cancelCurEditNode : function(setting, forceName) {
			...
			if (node) {
				var inputObj = root.curEditInput, newName = forceName
						? forceName
						: inputObj.val(), isCancel = !!forceName;
				if (tools.apply(setting.callback.beforeRename, [setting.treeId,
								node, newName, isCancel], true) === false) {
					return false;
				} else {
		...
		moveNode : function(setting, targetNode, node, moveType, animateFlag,
			...
			if (!setting.data.keep.parent && oldParentNode[childKey].length < 1) {
				...
				var tmp_ulObj = $$(oldParentNode, consts.id.UL, setting), tmp_switchObj = $$(
						oldParentNode, consts.id.SWITCH, setting), tmp_icoObj = $$(
						oldParentNode, consts.id.ICON, setting);
		...
		repairNodeLevelClass : function(setting, node, oldLevel) {
			if (oldLevel === node.level)
				return;
			var liObj = $$(node, setting), aObj = $$(node, consts.id.A, setting), ulObj = $$(
					node, consts.id.UL, setting), oldClass = consts.className.LEVEL
					+ oldLevel, newClass = consts.className.LEVEL + node.level;
	...
	view.makeNodeUrl = function(setting, node) {
		return setting.edit.enable ? null : (_makeNodeUrl
				.apply(view, arguments));
	}
