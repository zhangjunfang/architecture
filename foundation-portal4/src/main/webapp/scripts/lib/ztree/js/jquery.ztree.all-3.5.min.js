...
(function(p) {
	var G, H, I, J, K, L, r = {}, u = {}, v = {}, M = {
		...
		if (j.eqs(b.type, "mousedown"))
			...
			if (j.eqs(a.tagName, "span")
					&& a.getAttribute("treeNode" + e.id.SWITCH) !== null)
		...
		addNodesData : function(b, a, c) {
			...
			a[d] || (a[d] = []);
			if (a[d].length > 0)
				a[d][a[d].length - 1].isLastNode = !1, i.setNodeLineIcos(b,
						a[d][a[d].length - 1]);
			a.isParent = !0;
			a[d] = a[d].concat(c)
		},
		...
		getNextNode : function(b, a) {
			...
			for (var c = b.data.key.children, d = a.parentTId ? a
					.getParentNode() : h.getRoot(b), f = 0, g = d[c].length - 1; f <= g; f++)
				if (d[c][f] === a)
		...
		getNodesByParam : function(b, a, c, d) {
			...
			for (var f = b.data.key.children, g = [], l = 0, e = a.length; l < e; l++)
				a[l][c] == d && g.push(a[l]), g = g.concat(h.getNodesByParam(b,
						a[l][f], c, d));
			return g
		},
		getNodesByParamFuzzy : function(b, a, c, d) {
			...
			for (var f = b.data.key.children, g = [], d = d.toLowerCase(), l = 0, e = a.length; l < e; l++)
				typeof a[l][c] == "string"
						&& a[l][c].toLowerCase().indexOf(d) > -1
						&& g.push(a[l]), g = g.concat(h.getNodesByParamFuzzy(b,
						a[l][f], c, d));
			return g
		},
		...
		getPreNode : function(b, a) {
			if (!a)
				return null;
			for (var c = b.data.key.children, d = a.parentTId ? a
					.getParentNode() : h.getRoot(b), f = 0, g = d[c].length; f < g; f++)
				if (d[c][f] === a)
		...
		removeSelectedNode : function(b, a) {
			for (var c = h.getRoot(b), d = 0, f = c.curSelectedList.length; d < f; d++)
				if (a === c.curSelectedList[d]
					...
					c.curSelectedList.splice(d, 1), d--, f--
		},
		...
		unbindTree : function(b) {
			b.treeObj.unbind("click", m.proxy).unbind("dblclick", m.proxy)
					.unbind("mouseover", m.proxy).unbind("mouseout", m.proxy)
					.unbind("mousedown", m.proxy).unbind("mouseup", m.proxy)
					.unbind("contextmenu", m.proxy)
		},
		doProxy : function(b) {
	...
	H = function(b, a) {
		var c = h.getSetting(b.data.treeId), d = c.view.autoCancelSelected
				&& b.ctrlKey && h.isSelectedNode(c, a)
				? 0
				: c.view.autoCancelSelected && b.ctrlKey
	...
	var j = {
		...
		getNodeMainDom : function(b) {
			return p(b).parent("li").get(0)
					|| p(b).parentsUntil("li").parent().get(0)
		},
		uCanDo : function() {
			return !0
		}
		...
		addNodes : function(b, a, c, d) {
			if (!b.data.keep.leaf || !a || a.isParent)
				if (j.isArray(c) || (c = [c]), b.data.simpleData.enable
					...
					var f = k(a, e.id.SWITCH, b), g = k(a, e.id.ICON, b), l = k(
							a, e.id.UL, b);
					if (!a.open)
						i.replaceSwitchClass(a, f, e.folder.CLOSE), i
								.replaceIcoClass(a, g, e.folder.CLOSE), a.open = !1, l
								.css({
											display : "none"
										});
					...
					h.addNodesData(b, h.getRoot(b), c), i.createNodes(b, 0, c,
							null)
		},
		appendNodes : function(b, a, c, d, f, g) {
			...
			for (var e = [], j = b.data.key.children, k = 0, m = c.length; k < m; k++) {
				var o = c[k];
				if (f) {
					var t = (d ? d : h.getRoot(b))[j].length == c.length
							&& k == 0;
				...
				o[j]
						&& o[j].length > 0
						&& (t = i
								.appendNodes(b, a + 1, o[j], o, f, g && o.open));
				g
						&& (i.makeDOMNodeMainBefore(e, b, o), i
								.makeDOMNodeLine(e, b, o), h
								.getBeforeA(b, o, e), i.makeDOMNodeNameBefore(
								e, b, o), h.getInnerBeforeA(b, o, e), i
								.makeDOMNodeIcon(e, b, o), h.getInnerAfterA(b,
								o, e), i.makeDOMNodeNameAfter(e, b, o), h
								.getAfterA(b, o, e), o.isParent && o.open
								&& i.makeUlHtml(b, o, e, t.join("")), i
								.makeDOMNodeMainAfter(e, b, o), h
								.addCreatedNode(b, o))
			}
			return e
		},
		appendParentULDom : function(b, a) {
			...
			if (!k(a, e.id.UL, b).get(0)) {
				var f = i.appendNodes(b, a.level + 1, a[b.data.key.children],
						a, !1, !0);
		...
		asyncNode : function(b, a, c, d) {
			...
			if (a && !a.isParent)
				...
				return j.apply(d), !1;
			if (a)
				a.isAjaxing = !0, k(a, e.id.ICON, b).attr({
					style : "",
					"class" : e.className.BUTTON + " "
							+ e.className.ICO_LOADING
				});
			var l = {};
			...
			if (j.isArray(b.async.otherParam))
				for (f = 0, g = b.async.otherParam.length; f < g; f += 2)
			...
			p.ajax({
				...
				success : function(f) {
					if (o == h.getRoot(b)._ver) {
						var g = [];
						try {
							g = !f || f.length == 0 ? [] : typeof f == "string"
									? eval("(" + f + ")")
									: f
						} catch (l) {
							g = f
						...
						g && g !== "" ? (g = j.apply(b.async.dataFilter, [
										b.treeId, a, g], g), i.addNodes(b, a, g
										? j.clone(g)
										: [], !!c)) : i.addNodes(b, a, [], !!c);
						b.treeObj.trigger(e.event.ASYNC_SUCCESS, [b.treeId, a,
										f]);
						j.apply(d)
					}
				},
				error : function(c, d, f) {
					if (o == h.getRoot(b)._ver) {
						if (a)
							a.isAjaxing = null;
						i.setNodeLineIcos(b, a);
						b.treeObj.trigger(e.event.ASYNC_ERROR, [b.treeId, a, c,
										d, f])
					}
				}
			});
			...
			if (b.callback.onNodeCreated || b.view.addDiyDom)
				for (var a = h.getRoot(b); a.createdNodes.length > 0;) {
					...
					b.callback.onNodeCreated
							&& b.treeObj.trigger(e.event.NODECREATED, [
											b.treeId, c])
				}
		},
		createNodes : function(b, a, c, d) {
			if (c && c.length != 0) {
				var f = h.getRoot(b), g = b.data.key.children, g = !d || d.open
						|| !!k(d[g][0], b).get(0);
				f.createdNodes = [];
				a = i.appendNodes(b, a, c, d, !0, g);
				d
						? (d = k(d, e.id.UL, b), d.get(0)
								&& d.append(a.join("")))
		...
		destroy : function(b) {
			b
					&& (h.initCache(b), h.initRoot(b), m.unbindTree(b), m
							.unbindEvent(b), b.treeObj.empty())
		},
		expandCollapseNode : function(b, a, c, d, f) {
			...
			if (a) {
				if (g.expandTriggerFlag) {
					var q = f, f = function() {
						q && q();
						a.open ? b.treeObj.trigger(e.event.EXPAND,
								[b.treeId, a]) : b.treeObj.trigger(
								e.event.COLLAPSE, [b.treeId, a])
					};
					g.expandTriggerFlag = !1
				}
				if (!a.open
						&& a.isParent
						&& (!k(a, e.id.UL, b).get(0) || a[l] && a[l].length > 0
								&& !k(a[l][0], b).get(0)))
					i.appendParentULDom(b, a), i.createNodeCallback(b);
				if (a.open == c)
					j.apply(f, []);
				else {
					var c = k(a, e.id.UL, b), g = k(a, e.id.SWITCH, b), n = k(
							a, e.id.ICON, b);
					a.isParent
							? (a.open = !a.open, a.iconOpen
									...
									: (i.replaceSwitchClass(a, g,
											e.folder.CLOSE), i.replaceIcoClass(
											a, n, e.folder.CLOSE), d == !1
											|| b.view.expandSpeed == ""
											|| !(a[l] && a[l].length > 0) ? (c
											.hide(), j.apply(f, [])) : c
											.slideUp(b.view.expandSpeed, f)))
							: j.apply(f, [])
				}
			} else
				j.apply(f, [])
		...
		expandCollapseSonNode : function(b, a, c, d, f) {
			var g = h.getRoot(b), e = b.data.key.children, g = a ? a[e] : g[e], e = a
					? !1
		...
		makeDOMNodeIcon : function(b, a, c) {
			...
			b
					.push("<span id='", c.tId, e.id.ICON,
							"' title='' treeNode", e.id.ICON, " class='", i
									.makeNodeIcoClass(a, c), "' style='", i
									.makeNodeIcoStyle(a, c),
							"'></span><span id='", c.tId, e.id.SPAN, "'>", d,
							"</span>")
		},
		makeDOMNodeLine : function(b, a, c) {
			b.push("<span id='", c.tId, e.id.SWITCH, "' title='' class='", i
		...
		makeDOMNodeMainBefore : function(b, a, c) {
			b.push("<li id='", c.tId, "' class='", e.className.LEVEL, c.level,
		...
		makeDOMNodeNameBefore : function(b, a, c) {
			var d = h.getNodeTitle(a, c), f = i.makeNodeUrl(a, c), g = i
					.makeNodeFontCss(a, c), l = [], k;
			for (k in g)
				l.push(k, ":", g[k], ";");
			b.push("<a id='", c.tId, e.id.A, "' class='", e.className.LEVEL,
					c.level, "' treeNode", e.id.A, ' onclick="', c.click || "",
					'" ', f != null && f.length > 0 ? "href='" + f + "'" : "",
					" target='", i.makeNodeTarget(c), "' style='", l.join(""),
					"'");
		...
		makeNodeIcoClass : function(b, a) {
			...
			a.isAjaxing
					|| (c[0] = (a.iconSkin ? a.iconSkin + "_" : "") + c[0], a.isParent
							? c.push(a.open ? e.folder.OPEN : e.folder.CLOSE)
		...
		makeNodeLineClass : function(b, a) {
			...
			b.view.showLine ? a.level == 0 && a.isFirstNode && a.isLastNode ? c
					.push(e.line.ROOT) : a.level == 0 && a.isFirstNode ? c
					.push(e.line.ROOTS) : a.isLastNode
					? c.push(e.line.BOTTOM)
					: c.push(e.line.CENTER) : c.push(e.line.NOLINE);
			a.isParent ? c.push(a.open ? e.folder.OPEN : e.folder.CLOSE) : c
					.push(e.folder.DOCU);
			return i.makeNodeLineClassEx(a) + c.join("_")
		},
		makeNodeLineClassEx : function(b) {
			return e.className.BUTTON + " " + e.className.LEVEL + b.level + " "
					+ e.className.SWITCH + " "
		},
		...
		makeUlHtml : function(b, a, c, d) {
			c.push("<ul id='", a.tId, e.id.UL, "' class='", e.className.LEVEL,
				...
				if (d) {
					...
					b.data.keep.parent
							...
							: (a.isParent = !1, a.open = !1, c = k(a,
									e.id.SWITCH, b), d = k(a, e.id.ICON, b), i
									.replaceSwitchClass(a, c, e.folder.DOCU), i
									.replaceIcoClass(a, d, e.folder.DOCU), k(a,
									e.id.UL, b).remove())
				}
			}
		},
		...
		removeNode : function(b, a) {
			var c = h.getRoot(b), d = b.data.key.children, f = a.parentTId ? a
					.getParentNode() : c;
			a.isFirstNode = !1;
			a.isLastNode = !1;
			...
			if (h.getNodeCache(b, a.tId)) {
				...
				if (!b.data.keep.parent && g == 0)
					f.isParent = !1, f.open = !1, g = k(f, e.id.UL, b), l = k(
							f, e.id.SWITCH, b), j = k(f, e.id.ICON, b), i
							.replaceSwitchClass(f, l, e.folder.DOCU), i
							.replaceIcoClass(f, j, e.folder.DOCU), g.css(
							"display", "none");
				else if (b.view.showLine && g > 0) {
					var n = f[d][g - 1], g = k(n, e.id.UL, b), l = k(n,
							e.id.SWITCH, b);
					j = k(n, e.id.ICON, b);
					f == c ? f[d].length == 1 ? i.replaceSwitchClass(n, l,
							e.line.ROOT) : (c = k(f[d][0], e.id.SWITCH, b), i
							.replaceSwitchClass(f[d][0], c, e.line.ROOTS), i
							.replaceSwitchClass(n, l, e.line.BOTTOM)) : i
							.replaceSwitchClass(n, l, e.line.BOTTOM);
		...
		replaceSwitchClass : function(b, a, c) {
			if (a) {
				...
				if (d != void 0) {
					...
					c !== e.folder.DOCU ? a.removeAttr("disabled") : a.attr(
							"disabled", "disabled")
				}
			}
		},
		...
		setNodeLineIcos : function(b, a) {
			if (a) {
				var c = k(a, e.id.SWITCH, b), d = k(a, e.id.UL, b), f = k(a,
						e.id.ICON, b), g = i.makeUlLineClass(b, a);
		...
		init : function(b, a, c) {
			...
			c = {
				...
				expandNode : function(a, b, c, e, n) {
					...
					else if (a.open = !b, i.switchNode(this.setting, a), e !== !1)
						try {
							k(a, d).focus().blur()
						} catch (m) {
						}
				...
				getNodeIndex : function(a) {
					...
					for (var b = d.data.key.children, c = a.parentTId ? a
							.getParentNode() : h.getRoot(d), e = 0, i = c[b].length; e < i; e++)
						if (c[b][e] == a)
							return e;
					return -1
				},
				...
				reAsyncChildNodes : function(a, b, c) {
					if (this.setting.async.enable) {
						...
						j && (a = h.getRoot(d));
						if (b == "refresh") {
							for (var b = this.setting.data.key.children, m = 0, p = a[b]
									? a[b].length
									: 0; m < p; m++)
								h.removeNodeCache(d, a[b][m]);
							h.removeSelectedNode(d);
							a[b] = [];
							j ? this.setting.treeObj.empty() : k(a, e.id.UL, d)
									.empty()
						}
...
(function(m) {
	...
	p = function(b, a) {
		...
		if (k.apply(c.callback.beforeCheck, [c.treeId, a], !0) == !1)
			return !0;
		a[d] = !a[d];
		...
		view : {
			checkNodeRelation : function(b, a) {
				...
				if (b.check.chkStyle == c.STYLE) {
					...
					if (a[l])
						if (b.check.radioType == c.TYPE_ALL) {
							for (d = g.length - 1; d >= 0; d--)
								c = g[d], c[l] = !1, g.splice(d, 1), e
										.setChkClass(b, n(c, j.id.CHECK, b), c), c.parentTId != a.parentTId
										&& e.repairParentChkClassWithSelf(b, c);
							g.push(a)
						} else {
							g = a.parentTId ? a.getParentNode() : f.getRoot(b);
							for (d = 0, h = g[i].length; d < h; d++)
								c = g[i][d], c[l]
										&& c != a
										&& (c[l] = !1, e.setChkClass(b, n(c,
														j.id.CHECK, b), c))
						}
					...
					a[l]
							&& (!a[i] || a[i].length == 0 || b.check.chkboxType.Y
									.indexOf("s") > -1)
							&& e.setSonNodeCheckBox(b, a, !0), !a[l]
							&& (!a[i] || a[i].length == 0 || b.check.chkboxType.N
									.indexOf("s") > -1)
							&& e.setSonNodeCheckBox(b, a, !1), a[l]
							&& b.check.chkboxType.Y.indexOf("p") > -1
							&& e.setParentNodeCheckBox(b, a, !0), !a[l]
							&& b.check.chkboxType.N.indexOf("p") > -1
							&& e.setParentNodeCheckBox(b, a, !1)
			},
			makeChkClass : function(b, a) {
				var c = b.data.key.checked, d = j.checkbox, h = j.radio, i = "", i = a.chkDisabled === !0
						? d.DISABLED
						: a.halfCheck ? d.PART : b.check.chkStyle == h.STYLE
								? a.check_Child_State < 1 ? d.FULL : d.PART
								: a[c] ? a.check_Child_State === 2
										|| a.check_Child_State === -1
										? d.FULL
										: d.PART : a.check_Child_State < 1
										? d.FULL
										: d.PART, c = b.check.chkStyle + "_"
						+ (a[c] ? d.TRUE : d.FALSE) + "_" + i, c = a.check_Focus
			...
			repairAllChk : function(b, a) {
				if (b.check.enable && b.check.chkStyle === j.checkbox.STYLE)
					for (var c = b.data.key.checked, d = b.data.key.children, h = f
							.getRoot(b), i = 0, l = h[d].length; i < l; i++) {
						var g = h[d][i];
			...
			repairParentChkClassWithSelf : function(b, a) {
				if (a) {
					var c = b.data.key.children;
					a[c] && a[c].length > 0 ? e
							.repairParentChkClass(b, a[c][0]) : e
							.repairParentChkClass(b, a)
				}
			},
			...
			setChkClass : function(b, a, c) {
				a
						&& (c.nocheck === !0 ? a.hide() : a.show(), a
								.removeClass(), a
								.addClass(e.makeChkClass(b, c)))
			},
			setParentNodeCheckBox : function(b, a, c, d) {
				var h = b.data.key.children, i = b.data.key.checked, l = n(a,
						j.id.CHECK, b);
				d || (d = a);
				f.makeChkFlag(b, a);
				a.nocheck !== !0
						&& a.chkDisabled !== !0
						&& (a[i] = c, e.setChkClass(b, l, a), b.check.autoCheckTrigger
								&& a != d
								&& b.treeObj.trigger(j.event.CHECK, [null,
												b.treeId, a]));
				if (a.parentTId) {
					l = !0;
					if (!c)
						for (var h = a.getParentNode()[h], g = 0, k = h.length; g < k; g++)
							if (h[g].nocheck !== !0
									&& h[g].chkDisabled !== !0
									&& h[g][i]
									|| (h[g].nocheck === !0 || h[g].chkDisabled === !0)
									&& h[g].check_Child_State > 0) {
								l = !1;
								break
							}
			...
			setSonNodeCheckBox : function(b, a, c, d) {
				if (a) {
					var h = b.data.key.children, i = b.data.key.checked, l = n(
							a, j.id.CHECK, b);
					d || (d = a);
					var g = !1;
					if (a[h])
						for (var k = 0, m = a[h].length; k < m
								&& a.chkDisabled !== !0; k++) {
							var o = a[h][k];
					...
					if (a != f.getRoot(b) && a.chkDisabled !== !0) {
						...
						if (a.nocheck !== !0 && a.chkDisabled !== !0) {
							if (a[i] = c, !g)
								a.check_Child_State = a[h] && a[h].length > 0
										? c ? 2 : 0
										: -1
						} else
							a.check_Child_State = -1;
						e.setChkClass(b, l, a);
						b.check.autoCheckTrigger
								&& a != d
								&& a.nocheck !== !0
								&& a.chkDisabled !== !0
								&& b.treeObj.trigger(j.event.CHECK, [null,
		...
		data : {
			getRadioCheckedList : function(b) {
				for (var a = f.getRoot(b).radioCheckedList, c = 0, d = a.length; c < d; c++)
					f.getNodeCache(b, a[c].tId) || (a.splice(c, 1), c--, d--);
				return a
			},
			getCheckStatus : function(b, a) {
				...
				return {
					...
					half : a.halfCheck
							? a.halfCheck
							: b.check.chkStyle == j.radio.STYLE
									? a.check_Child_State === 2
									: a[c]
											? a.check_Child_State > -1
													&& a.check_Child_State < 2
											: a.check_Child_State > 0
				}
			},
			getTreeCheckedNodes : function(b, a, c, d) {
				if (!a)
					return [];
				for (var h = b.data.key.children, i = b.data.key.checked, e = c
						&& b.check.chkStyle == j.radio.STYLE
						&& b.check.radioType == j.radio.TYPE_ALL, d = !d
						? []
						: d, g = 0, k = a.length; g < k; g++) {
					if (a[g].nocheck !== !0 && a[g].chkDisabled !== !0
							&& a[g][i] == c && (d.push(a[g]), e))
						break;
			...
			getTreeChangeCheckedNodes : function(b, a, c) {
				if (!a)
					return [];
				for (var d = b.data.key.children, h = b.data.key.checked, c = !c
						? []
						: c, i = 0, e = a.length; i < e; i++)
					a[i].nocheck !== !0 && a[i].chkDisabled !== !0
							&& a[i][h] != a[i].checkedOld && c.push(a[i]), f
							.getTreeChangeCheckedNodes(b, a[i][d], c);
				return c
			},
			makeChkFlag : function(b, a) {
				if (a) {
					var c = b.data.key.children, d = b.data.key.checked, h = -1;
					if (a[c])
	...
	f.addInitNode(function(b, a, c, d) {
				if (c) {
					...
					if (typeof c.nocheck == "string")
						c.nocheck = k.eqs(c.nocheck, "true");
					c.nocheck = !!c.nocheck || b.check.nocheckInherit && d
							&& !!d.nocheck;
					if (typeof c.chkDisabled == "string")
						c.chkDisabled = k.eqs(c.chkDisabled, "true");
					c.chkDisabled = !!c.chkDisabled
							|| b.check.chkDisabledInherit && d
							&& !!d.chkDisabled;
					if (typeof c.halfCheck == "string")
						c.halfCheck = k.eqs(c.halfCheck, "true");
					...
					b.check.chkStyle == j.radio.STYLE
							&& b.check.radioType == j.radio.TYPE_ALL && c[a]
							&& f.getRoot(b).radioCheckedList.push(c)
				}
			});
	f.addInitProxy(function(b) {
		var a = b.target, c = f.getSetting(b.data.treeId), d = "", h = null, e = "", l = null;
		if (k.eqs(b.type, "mouseover")) {
			if (c.check.enable && k.eqs(a.tagName, "span")
					&& a.getAttribute("treeNode" + j.id.CHECK) !== null)
				d = k.getNodeMainDom(a).id, e = "mouseoverCheck"
		} else if (k.eqs(b.type, "mouseout")) {
			if (c.check.enable && k.eqs(a.tagName, "span")
					&& a.getAttribute("treeNode" + j.id.CHECK) !== null)
				d = k.getNodeMainDom(a).id, e = "mouseoutCheck"
		} else if (k.eqs(b.type, "click") && c.check.enable
				&& k.eqs(a.tagName, "span")
				&& a.getAttribute("treeNode" + j.id.CHECK) !== null)
			d = k.getNodeMainDom(a).id, e = "checkNode";
		if (d.length > 0)
			switch (h = f.getNodeCache(c, d), e) {
				case "checkNode" :
					l = p;
					break;
				case "mouseoverCheck" :
	...
	f.addBeforeA(function(b, a, c) {
				b.check.enable
						&& (f.makeChkFlag(b, a), c.push("<span ID='", a.tId,
								j.id.CHECK, "' class='", e.makeChkClass(b, a),
								"' treeNode", j.id.CHECK, a.nocheck === !0
										? " style='display:none;'"
										: "", "></span>"))
			});
	f.addZTreeTools(function(b, a) {
		a.checkNode = function(a, c, i, f) {
			...
			if (a.chkDisabled !== !0
					&& (c !== !0 && c !== !1 && (c = !a[g]), f = !!f, (a[g] !== c || i)
							&& !(f && k.apply(
									this.setting.callback.beforeCheck, [
											b.treeId, a], !0) == !1)
							&& k.uCanDo(this.setting)
							&& b.check.enable
							&& a.nocheck !== !0))
				a[g] = c, c = n(a, j.id.CHECK, b), (i || b.check.chkStyle === j.radio.STYLE)
						&& e.checkNodeRelation(b, a), e.setChkClass(b, c, a), e
						.repairParentChkClassWithSelf(b, a), f
						&& b.treeObj
								.trigger(j.event.CHECK, [null, b.treeId, a])
		};
		...
		a.updateNode = function(d, f) {
			...
			if (d && b.check.enable && n(d, b).get(0) && k.uCanDo(b)) {
				...
				(f == !0 || b.check.chkStyle === j.radio.STYLE)
						&& e.checkNodeRelation(b, d);
				e.setChkClass(b, i, d);
...
(function(u) {
	var H = {
		...
		onHoverOutNode : function(b) {
			var b = m.getSetting(b.data.treeId), a = m.getRoot(b);
			if (a.curHoverNode && !m.isSelectedNode(b, a.curHoverNode))
				f.removeTreeDom(b, a.curHoverNode), a.curHoverNode = null
		},
		onMousedownNode : function(b, a) {
			function c(b) {
				if (A.dragFlag == 0
						&& Math.abs(M - b.clientX) < e.edit.drag.minMoveSize
						&& Math.abs(N - b.clientY) < e.edit.drag.minMoveSize)
					return !0;
				var a, c, g, j, k;
				...
				if (A.dragFlag == 0) {
					...
					for (a = 0, c = l.length; a < c; a++) {
						...
						g = l[a];
						g.isParent && g.open
								? (f.expandCollapseNode(e, g, !g.open), A.dragNodeShowBefore[g.tId] = !0)
								: A.dragNodeShowBefore[g.tId] = !1
					}
					A.dragFlag = 1;
					t.showHoverDom = !1;
					h.showIfameMask(e, !0);
					...
					j = -1;
					if (l.length > 1) {
						var s = l[0].parentTId ? l[0].getParentNode()[k] : m
								.getNodes(e);
						k = [];
						for (a = 0, c = s.length; a < c; a++)
							if (A.dragNodeShowBefore[s[a].tId] !== void 0
									&& (g && j > -1 && j + 1 !== a && (g = !1), k
											.push(s[a]), j = a), l.length === k.length) {
								l = k;
								break
							}
					}
					g
							&& (G = l[0].getPreNode(), Q = l[l.length - 1]
									.getNextNode());
					C = o("<ul class='zTreeDragUL'></ul>", e);
					for (a = 0, c = l.length; a < c; a++)
						if (g = l[a], g.editNameFlag = !1, f.selectNode(e, g,
								a > 0), f.removeTreeDom(e, g), j = o("<li id='"
										+ g.tId + "_tmp'></li>", e), j
								.append(o(g, d.id.A, e).clone()), j.css(
								"padding", "0"), j.children("#" + g.tId
								+ d.id.A).removeClass(d.node.CURSELECTED), C
								.append(j), a == e.edit.drag.maxShowNodeNum - 1) {
							j = o(	"<li id='" + g.tId
											+ "_moretmp'><a>  ...  </a></li>",
									e);
							C.append(j);
				...
				if (A.dragFlag == 1) {
					r
							&& z.attr("id") == b.target.id
							&& v
							&& b.clientX + E.scrollLeft() + 2 > u(
									"#" + v + d.id.A, r).offset().left
							? (g = u("#" + v + d.id.A, r), b.target = g.length > 0
									? g.get(0)
									: b.target)
							: r
									&& (r.removeClass(d.node.TMPTARGET_TREE), v
											&& u("#" + v + d.id.A, r)
													.removeClass(d.node.TMPTARGET_NODE
															+ "_"
															+ d.move.TYPE_PREV)
													.removeClass(d.node.TMPTARGET_NODE
															+ "_"
					...
					for (var D in g)
						if (g[D].treeId
								&& g[D].edit.enable
								&& g[D].treeId != e.treeId
								&& (b.target.id == g[D].treeId || u(b.target)
										.parents("#" + g[D].treeId).length > 0))
							I = !0, i = g[D];
					D = E.scrollTop();
					j = E.scrollLeft();
					k = i.treeObj.offset();
					...
					c = b.clientY + D - k.top;
					var p = i.treeObj.height() + k.top - b.clientY - D, q = b.clientX
							+ j - k.left, x = i.treeObj.width() + k.left
							- b.clientX - j;
					k = c < e.edit.drag.borderMax && c > e.edit.drag.borderMin;
					var s = p < e.edit.drag.borderMax
							&& p > e.edit.drag.borderMin, J = q < e.edit.drag.borderMax
							&& q > e.edit.drag.borderMin, F = x < e.edit.drag.borderMax
							&& x > e.edit.drag.borderMin, p = c > e.edit.drag.borderMin
							&& p > e.edit.drag.borderMin
							&& q > e.edit.drag.borderMin
							&& x > e.edit.drag.borderMin, q = k
							&& i.treeObj.scrollTop() <= 0, x = s
							&& i.treeObj.scrollTop() + i.treeObj.height() + 10 >= a, O = J
							&& i.treeObj.scrollLeft() <= 0, P = F
							&& i.treeObj.scrollLeft() + i.treeObj.width() + 10 >= g;
					if (b.target.id
							&& i.treeObj.find("#" + b.target.id).length > 0) {
						for (var B = b.target; B && B.tagName
								&& !h.eqs(B.tagName, "li") && B.id != i.treeId;)
							B = B.parentNode;
						var R = !0;
						for (a = 0, c = l.length; a < c; a++)
							if (g = l[a], B.id === g.tId) {
						...
						if (R
								&& b.target.id
								&& (b.target.id == B.id + d.id.A || u(b.target)
										.parents("#" + B.id + d.id.A).length > 0))
							r = u(B), v = B.id
					}
					g = l[0];
					if (p
							&& (b.target.id == i.treeId || u(b.target)
									.parents("#" + i.treeId).length > 0)) {
						if (!r && (b.target.id == i.treeId || q || x || O || P)
								&& (I || !I && g.parentTId))
							r = i.treeObj;
						k ? i.treeObj.scrollTop(i.treeObj.scrollTop() - 10) : s
								&& i.treeObj.scrollTop(i.treeObj.scrollTop()
										+ 10);
						J
								? i.treeObj.scrollLeft(i.treeObj.scrollLeft()
										- 10)
								: F
										&& i.treeObj.scrollLeft(i.treeObj
												.scrollLeft()
												+ 10);
						r
								&& r != i.treeObj
								&& r.offset().left < i.treeObj.offset().left
								&& i.treeObj.scrollLeft(i.treeObj.scrollLeft()
										+ r.offset().left
										- i.treeObj.offset().left)
					}
					C.css({
					...
					if (r && r.attr("id") != i.treeId) {
						...
						c = b.ctrlKey && e.edit.drag.isMove
								&& e.edit.drag.isCopy || !e.edit.drag.isMove
								&& e.edit.drag.isCopy;
						a = !!(G && v === G.tId);
						k = !!(Q && v === Q.tId);
						j = g.parentTId && g.parentTId == v;
						g = (c || !k)
								&& h.apply(i.edit.drag.prev, [i.treeId, l, y],
										!!i.edit.drag.prev);
						a = (c || !a)
								&& h.apply(i.edit.drag.next, [i.treeId, l, y],
										!!i.edit.drag.next);
						F = (c || !j)
								&& !(i.data.keep.leaf && !y.isParent)
								&& h.apply(i.edit.drag.inner, [i.treeId, l, y],
										!!i.edit.drag.inner);
						if (!g && !a && !F) {
							...
							(J == 1 || b <= J && b >= -0.2) && g
									? (a = 1 - z.width(), k = s - z.height()
											/ 2, w = d.move.TYPE_PREV)
									: (F == 0 || b >= F && b <= 1.2) && a
											? (a = 1 - z.width(), k = k == null
													|| y.isParent && y.open ? s
													+ c.height() - z.height()
													/ 2 : k.offset().top
													- z.height() / 2, w = d.move.TYPE_NEXT)
											: (a = 5 - z.width(), k = s, w = d.move.TYPE_INNER);
							z.css({
										display : "block",
										top : k + "px",
										left : j + a + "px"
							...
							if (y
									&& y.isParent
									&& w == d.move.TYPE_INNER
									&& (b = !0, window.zTreeMoveTimer
											&& window.zTreeMoveTargetNodeTId !== y.tId
											? (clearTimeout(window.zTreeMoveTimer), window.zTreeMoveTargetNodeTId = null)
											: window.zTreeMoveTimer
													&& window.zTreeMoveTargetNodeTId === y.tId
													&& (b = !1), b))
								window.zTreeMoveTimer = setTimeout(function() {
									w == d.move.TYPE_INNER
											...
											&& y.isParent
											&& !y.open
											&& (new Date).getTime() - K > i.edit.drag.autoOpenTime
											&& h.apply(
													i.callback.beforeDragOpen,
													[i.treeId, y], !0)
											&& (f.switchNode(i, y), i.edit.drag.autoExpandTrigger
													&& i.treeObj.trigger(
															d.event.EXPAND,
															[i.treeId, y]))
								}, i.edit.drag.autoOpenTime + 50), window.zTreeMoveTargetNodeTId = y.tId
						}
					} else if (w = d.move.TYPE_INNER, r
							&& h.apply(i.edit.drag.inner, [i.treeId, l, null],
									!!i.edit.drag.inner) ? r
							.addClass(d.node.TMPTARGET_TREE) : r = null, z.css(
							{
								display : "none"
							}), window.zTreeMoveTimer)
						clearTimeout(window.zTreeMoveTimer), window.zTreeMoveTargetNodeTId = null;
			...
			function n(b) {
				...
				if (A.dragFlag != 0) {
					A.dragFlag = 0;
					var a, k, j;
					for (a = 0, k = l.length; a < k; a++)
						j = l[a], j.isParent
								&& A.dragNodeShowBefore[j.tId]
								&& !j.open
								&& (f.expandCollapseNode(e, j, !j.open), delete A.dragNodeShowBefore[j.tId]);
					C && C.remove();
					z && z.remove();
					var p = b.ctrlKey && e.edit.drag.isMove
							&& e.edit.drag.isCopy || !e.edit.drag.isMove
							&& e.edit.drag.isCopy;
					!p && r && v && l[0].parentTId && v == l[0].parentTId
							&& w == d.move.TYPE_INNER && (r = null);
					if (r) {
						var q = v == null ? null : m.getNodeCache(i, v);
						if (h.apply(e.callback.beforeDrop, [i.treeId, l, q, w,
										p], !0) == !1)
							f.selectNodes(x, l);
						else {
							var s = p ? h.clone(l) : l;
								...
								if (I) {
									if (!p)
										for (var a = 0, c = l.length; a < c; a++)
											f.removeNode(e, l[a]);
									if (w == d.move.TYPE_INNER)
										f.addNodes(i, q, s);
									else if (f
											.addNodes(i, q.getParentNode(), s), w == d.move.TYPE_PREV)
										for (a = 0, c = s.length; a < c; a++)
											f.moveNode(i, q, s[a], w, !1);
									else
										for (a = -1, c = s.length - 1; a < c; c--)
											f.moveNode(i, q, s[c], w, !1)
								} else if (p && w == d.move.TYPE_INNER)
									f.addNodes(i, q, s);
								else if (p
										&& f.addNodes(i, q.getParentNode(), s), w != d.move.TYPE_NEXT)
									for (a = 0, c = s.length; a < c; a++)
										f.moveNode(i, q, s[a], w, !1);
								else
									for (a = -1, c = s.length - 1; a < c; c--)
							...
							w == d.move.TYPE_INNER && h.canAsync(i, q) ? f
									.asyncNode(i, q, !1, a) : a()
						}
					} else
						f.selectNodes(x, l), e.treeObj.trigger(d.event.DROP, [
										b, e.treeId, l, null, null, null])
				}
			}
			function g() {
				return !1
			}
			var k, j, e = m.getSetting(b.data.treeId), A = m.getRoot(e), t = m
					.getRoots();
			if (b.button == 2 || !e.edit.enable || !e.edit.drag.isCopy
					&& !e.edit.drag.isMove)
				return !0;
			var p = b.target, q = m.getRoot(e).curSelectedList, l = [];
			if (m.isSelectedNode(e, a))
				for (k = 0, j = q.length; k < j; k++) {
					if (q[k].editNameFlag && h.eqs(p.tagName, "input")
			...
			var E = u(e.treeObj.get(0).ownerDocument), L = u(e.treeObj.get(0).ownerDocument.body), C, z, r, I = !1, i = e, x = e, G, Q, S = null, T = null, v = null, w = d.move.TYPE_INNER, M = b.clientX, N = b.clientY, K = (new Date)
					.getTime();
	...
	u.extend(!0, u.fn.zTree._z, {
		tools : {
			...
			inputFocus : function(b) {
				b.get(0)
						&& (b.focus(), h.setCursorPosition(b.get(0),
								b.val().length))
			},
			...
			setCursorPosition : function(b, a) {
				if (b.setSelectionRange)
					b.focus(), b.setSelectionRange(a, a);
			...
			showIfameMask : function(b, a) {
				...
				if (a)
					for (var d = o("iframe", b), g = 0, f = d.length; g < f; g++) {
						var j = d.get(g), e = h.getAbs(j), j = o(
								"<div id='zTreeMask_" + g
										+ "' class='zTreeMask' style='top:"
										+ e[1] + "px; left:" + e[0]
										+ "px; width:" + j.offsetWidth
										+ "px; height:" + j.offsetHeight
										+ "px;'></div>", b);
						j.appendTo(body);
		...
		view : {
			addEditBtn : function(b, a) {
				if (!(a.editNameFlag || o(a, d.id.EDIT, b).length > 0)
						&& h.apply(b.edit.showRenameBtn, [b.treeId, a],
								b.edit.showRenameBtn)) {
					var c = o(a, d.id.A, b), n = "<span class='"
							+ d.className.BUTTON
							+ " edit' id='"
							+ a.tId
							+ d.id.EDIT
							+ "' title='"
							+ h.apply(b.edit.renameTitle, [b.treeId, a],
									b.edit.renameTitle) + "' treeNode"
							+ d.id.EDIT + " style='display:none;'></span>";
					c.append(n);
					o(a, d.id.EDIT, b).bind("click", function() {
						if (!h.uCanDo(b)
								|| h.apply(b.callback.beforeEditName, [
												b.treeId, a], !0) == !1)
							return !1;
						f.editNode(b, a);
						return !1
			...
			addRemoveBtn : function(b, a) {
				if (!(a.editNameFlag || o(a, d.id.REMOVE, b).length > 0)
						&& h.apply(b.edit.showRemoveBtn, [b.treeId, a],
					...
					var c = o(a, d.id.A, b), n = "<span class='"
							...
							+ d.id.REMOVE
							+ "' title='"
							+ h.apply(b.edit.removeTitle, [b.treeId, a],
									b.edit.removeTitle) + "' treeNode"
							+ d.id.REMOVE + " style='display:none;'></span>";
					c.append(n);
					o(a, d.id.REMOVE, b).bind("click", function() {
						if (!h.uCanDo(b)
								|| h.apply(b.callback.beforeRemove, [b.treeId,
												a], !0) == !1)
							return !1;
						f.removeNode(b, a);
			...
			addHoverDom : function(b, a) {
				if (m.getRoots().showHoverDom)
					a.isHover = !0, b.edit.enable
							&& (f.addEditBtn(b, a), f.addRemoveBtn(b, a)), h
							.apply(b.view.addHoverDom, [b.treeId, a])
			},
			cancelCurEditNode : function(b, a) {
				var c = m.getRoot(b), n = b.data.key.name, g = c.curEditNode;
				if (g) {
					var k = c.curEditInput, j = a ? a : k.val(), e = !!a;
					if (h.apply(b.callback.beforeRename, [b.treeId, g, j, e],
							!0) === !1)
						return !1;
					else
						g[n] = j ? j : k.val(), b.treeObj.trigger(
								d.event.RENAME, [b.treeId, g, e]);
			...
			editNode : function(b, a) {
				var c = m.getRoot(b);
				f.editNodeBlur = !1;
				if (m.isSelectedNode(b, a) && c.curEditNode == a
						&& a.editNameFlag)
					setTimeout(function() {
					...
					o(a, d.id.SPAN, b)
							.html("<input type=text class='rename' id='"
									+ a.tId + d.id.INPUT + "' treeNode"
									+ d.id.INPUT + " >");
					var g = o(a, d.id.INPUT, b);
					g.attr("value", a[n]);
					b.edit.editNameSelectAll ? h.inputSelect(g) : h
							.inputFocus(g);
					g.bind("blur", function() {
								f.editNodeBlur || f.cancelCurEditNode(b)
							}).bind("keydown", function(c) {
						c.keyCode == "13" ? (f.editNodeBlur = !0, f
								.cancelCurEditNode(b)) : c.keyCode == "27"
								&& f.cancelCurEditNode(b, a[n])
					}).bind("click", function() {
								return !1
			...
			moveNode : function(b, a, c, n, g, k) {
				...
				if (a != c
						&& (!b.data.keep.leaf || !a || a.isParent || n != d.move.TYPE_INNER)) {
					var h = c.parentTId ? c.getParentNode() : j, t = a === null
							|| a == j;
					t && a === null && (a = j);
					if (t)
						n = d.move.TYPE_INNER;
					j = a.parentTId ? a.getParentNode() : j;
					if (n != d.move.TYPE_PREV && n != d.move.TYPE_NEXT)
						n = d.move.TYPE_INNER;
					if (n == d.move.TYPE_INNER)
						if (t)
							...
							if (!a.isParent)
								a.isParent = !0, a.open = !!a.open, f
										.setNodeLineIcos(b, a);
							c.parentTId = a.tId
						}
					var p;
					t ? p = t = b.treeObj : (!k && n == d.move.TYPE_INNER ? f
							.expandCollapseNode(b, a, !0, !1) : k
							|| f.expandCollapseNode(b, a.getParentNode(), !0,
									!1), t = o(a, b), p = o(a, d.id.UL, b), t
							.get(0)
							&& !p.get(0)
							&& (p = [], f.makeUlHtml(b, a, p, ""), t.append(p
									.join(""))), p = o(a, d.id.UL, b));
					var q = o(c, b);
					q.get(0) ? t.get(0) || q.remove() : q = f.appendNodes(b,
							c.level, [c], null, !1, !0).join("");
					p.get(0) && n == d.move.TYPE_INNER ? p.append(q) : t.get(0)
							&& n == d.move.TYPE_PREV ? t.before(q) : t.get(0)
							&& n == d.move.TYPE_NEXT && t.after(q);
					var l = -1, u = 0, x = null, t = null, C = c.level;
					if (c.isFirstNode) {
						if (l = 0, h[e].length > 1)
							x = h[e][1], x.isFirstNode = !0
					} else if (c.isLastNode)
						l = h[e].length - 1, x = h[e][l - 1], x.isLastNode = !0;
					else
						for (p = 0, q = h[e].length; p < q; p++)
							if (h[e][p].tId == c.tId) {
								l = p;
								break
							}
					l >= 0 && h[e].splice(l, 1);
					if (n != d.move.TYPE_INNER)
						for (p = 0, q = j[e].length; p < q; p++)
							j[e][p].tId == a.tId && (u = p);
					if (n == d.move.TYPE_INNER) {
						a[e] || (a[e] = []);
						if (a[e].length > 0)
							t = a[e][a[e].length - 1], t.isLastNode = !1;
						a[e].splice(a[e].length, 0, c);
						c.isLastNode = !0;
						c.isFirstNode = a[e].length == 1
					} else
						a.isFirstNode && n == d.move.TYPE_PREV
								? (j[e].splice(u, 0, c), t = a, t.isFirstNode = !1, c.parentTId = a.parentTId, c.isFirstNode = !0, c.isLastNode = !1)
								: a.isLastNode && n == d.move.TYPE_NEXT
										? (j[e].splice(u + 1, 0, c), t = a, t.isLastNode = !1, c.parentTId = a.parentTId, c.isFirstNode = !1, c.isLastNode = !0)
										: (n == d.move.TYPE_PREV ? j[e].splice(
												u, 0, c) : j[e].splice(u + 1,
												0, c), c.parentTId = a.parentTId, c.isFirstNode = !1, c.isLastNode = !1);
					m.fixPIdKeyValue(b, c);
					m.setSonNodeLevel(b, c.getParentNode(), c);
					f.setNodeLineIcos(b, c);
					f.repairNodeLevelClass(b, c, C);
					!b.data.keep.parent && h[e].length < 1
							? (h.isParent = !1, h.open = !1, a = o(h, d.id.UL,
									b), n = o(h, d.id.SWITCH, b), e = o(h,
									d.id.ICON, b), f.replaceSwitchClass(h, n,
									d.folder.DOCU), f.replaceIcoClass(h, e,
									d.folder.DOCU), a.css("display", "none"))
							: x && f.setNodeLineIcos(b, x);
					t && f.setNodeLineIcos(b, t);
					b.check
							&& b.check.enable
							&& f.repairChkClass
							&& (f.repairChkClass(b, h), f
									.repairParentChkClassWithSelf(b, h), h != c.parent
									&& f.repairParentChkClassWithSelf(b, c));
					k
							|| f.expandCollapseParentNode(b, c.getParentNode(),
									!0, g)
				}
			},
			...
			repairNodeLevelClass : function(b, a, c) {
				if (c !== a.level) {
					var f = o(a, b), g = o(a, d.id.A, b), b = o(a, d.id.UL, b), c = d.className.LEVEL
							+ c, a = d.className.LEVEL + a.level;
					f.removeClass(c);
					f.addClass(a);
					g.removeClass(c);
					g.addClass(a);
	...
	m.addZTreeTools(function(b, a) {
		...
		a.copyNode = function(a, n, g, k) {
			...
			var j = h.clone(n);
			if (!a)
				a = null, g = d.move.TYPE_INNER;
			g == d.move.TYPE_INNER ? (n = function() {
				f.addNodes(b, a, [j], k)
			}, h.canAsync(b, a) ? f.asyncNode(b, a, k, n) : n()) : (f.addNodes(
					b, a.parentNode, [j], k), f.moveNode(b, a, j, g, !1, k));
			return j
		};
		a.editName = function(a) {
			a
					&& a.tId
					&& a === m.getNodeCache(b, a.tId)
					&& (a.parentTId
							&& f.expandCollapseParentNode(b, a.getParentNode(),
									!0), f.editNode(b, a))
		};
		a.moveNode = function(a, n, g, k) {
			...
			if (a && !a.isParent && b.data.keep.leaf && g === d.move.TYPE_INNER)
				return null;
					...
					&& (n.parentTId == a.tId && g == d.move.TYPE_INNER || o(n,
							b).find("#" + a.tId).length > 0))
				return null;
			else
				a || (a = null);
			h.canAsync(b, a) && g === d.move.TYPE_INNER ? f.asyncNode(b, a, k,
					j) : j();
			return n
		};
	...
	f.createNodes = function(b, a, c, d) {
		...
		c && f.repairParentChkClassWithSelf
				&& f.repairParentChkClassWithSelf(b, d)
	};
	var U = f.makeNodeUrl;
	...
	h.uCanDo = function(b, a) {
		var c = m.getRoot(b);
		return a
				&& (h.eqs(a.type, "mouseover") || h.eqs(a.type, "mouseout")
						|| h.eqs(a.type, "mousedown") || h.eqs(a.type,
						"mouseup")) ? !0 : !c.curEditNode
				&& (P ? P.apply(f, arguments) : !0)
	}
})(jQuery);
