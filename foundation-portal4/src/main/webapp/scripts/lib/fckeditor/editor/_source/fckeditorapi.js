 ...
 * 
 * Create the FCKeditorAPI object that is available as a global object in the
 * page where the editor is placed in.
 */
...
function InitializeAPI() {
	...
	if (!(FCKeditorAPI = oParentWindow.FCKeditorAPI)) {
		...
		var sScript = 'window.FCKeditorAPI = {'
				...
				'_FunctionQueue	: {' + 'Functions : new Array(),'
				+ 'IsRunning : false,' +
		...
		if (oParentWindow.execScript)
			...
			if (FCKBrowserInfo.IsGecko10) {
				...
				// oParentWindow.eval in Safari and Gran Paradiso executes in
				// the calling window
