 ...
 * 
 * FCKPastePlainTextCommand Class: represents the "Paste as Plain Text" command.
 */
		...
		switch (this.Name) {
			...
			case 'TableMergeCells' :
				if (FCKTableHandler.CheckIsSelectionRectangular()
					...
					return FCK_TRISTATE_DISABLED;
			case 'TableMergeRight' :
				return FCKTableHandler.GetMergeRightTarget()
						? FCK_TRISTATE_OFF
						: FCK_TRISTATE_DISABLED;
			case 'TableMergeDown' :
				return FCKTableHandler.GetMergeDownTarget()
						? FCK_TRISTATE_OFF
						: FCK_TRISTATE_DISABLED;
			default :
				return FCK_TRISTATE_OFF;
