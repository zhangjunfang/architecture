...
FCKTools.ClearElementAttributes = function(element) {
	...
	for (var i = 0; i < element.attributes.length; i++) {
		...
		element.removeAttribute(element.attributes[i].name, 0); // 0 : Case
		// Insensitive
	}
}
...
FCKTools.CreateXmlObject = function(object) {
	switch (object) {
		...
		case 'DOMDocument' :
			...
			// But that doesn't work if we're running under domain relaxation
			// mode, so we need a workaround.
			// See
			// http://ajaxian.com/archives/xml-messages-with-cross-domain-json
			// about the trick we're using.
			var doc = (new DOMParser()).parseFromString('<tmp></tmp>',
					'text/xml');
...
FCKTools.AddEventListenerEx = function(sourceObject, eventName, listener,
		paramsArray) {
...
FCKTools.RestoreStyles = function(element, savedStyles) {
	...
	if (savedStyles.Inline)
		element.setAttribute('style', savedStyles.Inline, 0); // 0 : Case
	// Insensitive
	else
...
FCKTools.GetElementPosition = function(el, relativeWindow) {
	...
	while (el) {
		...
		// 'FCKConfig.FloatingPanelsZIndex' -- Submenus are under a positioned
		// IFRAME.
		if (sPosition && sPosition != 'static'
				&& el.style.zIndex != FCKConfig.FloatingPanelsZIndex)
			break;

		/*
		 * FCKDebug.Output( el.tagName + ":" + "offset=" + el.offsetLeft + "," +
		 * el.offsetTop + " " + "scroll=" + el.scrollLeft + "," + el.scrollTop ) ;
		 */

		c.X += el.offsetLeft - el.scrollLeft;
		c.Y += el.offsetTop - el.scrollTop;

		// Backtrack due to offsetParent's calculation by the browser ignores
