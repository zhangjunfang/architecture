...
oParser.ParseEMailParams = function(sParams) {
	...
	if (aMatch)
		oEMailParams.Subject = decodeURIComponent(aMatch[2]);
	...
	if (aLinkInfo && aLinkInfo[1] == 'mailto') {
		...
		if (func != null) {
			try {
				...
				aLinkInfo[2] = aLinkInfo[2].replace(/\\'/g,
						'###SINGLE_QUOTE###');
		...
		var aMatch = aLinkInfo[2]
				.match(/^location\.href='mailto:'\+(String\.fromCharCode\([\d,]+\))\+'(.*)'$/);
		if (aMatch) {
			// The link is encoded
...
oParser.CreateEMailUri = function(address, subject, body) {
	...
	switch (FCKConfig.EMailProtection) {
		case 'function' :
			...
			func = func.replace(/NAME/g, "'"
							+ aAddressParts[0].replace(/'/g, '\\\'') + "'");
			func = func.replace(/DOMAIN/g, "'"
							+ aAddressParts[1].replace(/'/g, '\\\'') + "'");
			func = func.replace(/SUBJECT/g, "'"
							+ encodeURIComponent(subject).replace(/'/g, '\\\'')
							+ "'");
			func = func.replace(/BODY/g, "'"
		...
		case 'encode' :
			...
			return 'javascript:location.href=\'mailto:\'+String.fromCharCode('
...
function LoadAnchorNamesAndIds() {
	// Since version 2.0, the anchors are replaced in the DOM by IMGs so the
	// user see the icon
...
function LoadSelection() {
	...
	} else if (sHRef.substr(0, 1) == '#' && sHRef.length > 1) // It is an
	// anchor link.
	{
		sType = 'anchor';
		GetE('cmbAnchorName').value = GetE('cmbAnchorId').value = sHRef
				.substr(1);
...
function SetLinkType(linkType) {
	...
	if (!FCKConfig.LinkDlgHideAdvanced)
		dialog.SetTabVisibility('Advanced',
...
function SetTarget(targetType) {
	...
	GetE('tdPopupName').style.display = GetE('tablePopupFeatures').style.display = (targetType == 'popup'
			? ''
			: 'none');
...
function BuildOnClickPopup() {
	...
	for (var i = 0; i < aChkFeatures.length; i++) {
		if (i > 0)
			sFeatures += ',';
		sFeatures += aChkFeatures[i].value + '='
				+ (aChkFeatures[i].checked ? 'yes' : 'no');
	}

	if (GetE('txtPopupWidth').value.length > 0)
		sFeatures += ',width=' + GetE('txtPopupWidth').value;
	if (GetE('txtPopupHeight').value.length > 0)
		sFeatures += ',height=' + GetE('txtPopupHeight').value;
	if (GetE('txtPopupLeft').value.length > 0)
		sFeatures += ',left=' + GetE('txtPopupLeft').value;
	if (GetE('txtPopupTop').value.length > 0)
		sFeatures += ',top=' + GetE('txtPopupTop').value;
...
function FillPopupFields(windowName, features) {
	...
	if (oFeatures['width'])
		GetE('txtPopupWidth').value = oFeatures['width'];
	if (oFeatures['height'])
		GetE('txtPopupHeight').value = oFeatures['height'];
	if (oFeatures['left'])
		GetE('txtPopupLeft').value = oFeatures['left'];
	if (oFeatures['top'])
		GetE('txtPopupTop').value = oFeatures['top'];
}
...
function Ok() {
	...
	switch (GetE('cmbLinkType').value) {
		...
		case 'email' :
			...
			sUri = oParser.CreateEMailUri(sUri, GetE('txtEMailSubject').value,
		...
		case 'anchor' :
			...
			if (sAnchor.length == 0)
				sAnchor = GetE('cmbAnchorId').value;
	...
	// If no link is selected, create a new one (it may result in more than one
	// link creation - #220).
	...
	for (var i = 0; i < aLinks.length; i++) {
		...
		if (aHasSelection)
			sInnerHtml = oLink.innerHTML; // Save the innerHTML (IE changes it
		// if it is like an URL).
		...
		SetAttribute(oLink, 'tabindex', (GetE('txtAttTabIndex').value > 0
						? GetE('txtAttTabIndex').value
						: null));
...
function BrowseServer() {
	OpenFileBrowser(FCKConfig.LinkBrowserURL, FCKConfig.LinkBrowserWindowWidth,
...
function OnUploadCompleted(errorNumber, fileUrl, fileName, customMsg) {
	...
	switch (errorNumber) {
		...
		case 101 : // Custom warning
			...
			break;
		case 201 :
			alert('A file with the same name is already available. The uploaded file has been renamed to "'
					+ fileName + '"');
			break;
		case 202 :
...
var oUploadAllowedExtRegex = new RegExp(FCKConfig.LinkUploadAllowedExtensions,
		'i');
var oUploadDeniedExtRegex = new RegExp(FCKConfig.LinkUploadDeniedExtensions,
		'i');
...
function CheckUpload() {
	...
	if ((FCKConfig.LinkUploadAllowedExtensions.length > 0 && !oUploadAllowedExtRegex
			.test(sFile))
			|| (FCKConfig.LinkUploadDeniedExtensions.length > 0 && oUploadDeniedExtRegex
					.test(sFile))) {
