...
FCKDataProcessor.prototype = {
	 ...
	 * value will be loaded in the editor. The HTML must be from <html> to
	 * </html>, including <head>, <body> and eventually the DOCTYPE. Note: HTML
	 * comments may already be part of the data because of the pre-processing
	 * made with ProtectedSource. @param {String} data The data to be converted
	 * in the DataProcessor specific format.
	 */
	ConverttoView : function(data) {
		...
		if (FCKConfig.FullPage) {
			...
			if (!FCKRegexLib.HtmlOpener.test(data))
				data = '<html dir="' + FCKConfig.ContentLangDirection + '">'
						+ data + '</html>';
			...
			var html = FCKConfig.DocType + '<html dir="'
			...
			// the horizontal one may appear when the page needs vertical
			// scrolling.
			...
			html += '><head><title></title></head>' + '<body'
					+ FCKConfig.GetBodyAttributes() + '>' + data
					+ '</body></html>';
	...
	/*
	 * Converts a DOM (sub-)tree to a string in the data format. @param {Object}
	 * rootNode The node that contains the DOM tree to be converted to the data
	 * format. @param {Boolean} excludeRoot Indicates that the root node must
	 * not be included in the conversion, only its children. @param {Boolean}
	 * format Indicates that the data must be formatted for human reading. Not
	 * all Data Processors may provide it.
	 */
	 ...
	 * editor selection position. @param {String} html The HTML to be fixed.
	 */
