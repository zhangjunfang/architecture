...
FCKW3CRange.CreateRange = function(parentDocument) {
	...
	/*
	 * // Get the browser implementation of the range, if available. if (
	 * parentDocument.createRange ) { var range = parentDocument.createRange() ;
	 * if ( typeof( range.startContainer ) != 'undefined' ) return range ; }
	 */
...
FCKW3CRange.prototype = {
	...
	// will not need this check for our use of this class so we can ignore it
	// for now.
	...
	selectNodeContents : function(refNode) {
		...
		this.setEnd(refNode, refNode.nodeType == 3
						? refNode.data.length
	...
	insertNode : function(newNode) {
		...
		if (startContainer.nodeType == 3) {
			...
			if (startContainer == this.endContainer)
				this.setEnd(startContainer.nextSibling, this.endOffset
								- this.startOffset);
			...
			startContainer.insertBefore(newNode,
	...
	_ExecContentsAction : function(action, docFrag) {
		...
		// Check the start and end nodes and make the necessary removals or
		// changes.
		...
		if (endNode.nodeType == 3)
			...
			if (endNode.childNodes.length > 0) {
				...
				if (endOffset > endNode.childNodes.length - 1) {
					...
					endNode = FCKDomTools.InsertAfterNode(endNode.lastChild,
		...
		if (startNode.nodeType == 3) {
			...
			if (startOffset == 0) {
				// Let's create a temporary node and mark it for removal.
				startNode = startNode.insertBefore(this._Document
								.createTextNode(''), startNode.firstChild);
				removeStartNode = true;
			} else if (startOffset > startNode.childNodes.length - 1) {
				// Let's create a temporary node and mark it for removal.
				startNode = startNode.appendChild(this._Document
						.createTextNode(''));
				removeStartNode = true;
			} else
		...
		for (var j = i; j < startParents.length; j++) {
			...
			if (clone && levelStartNode != startNode) // action = 0 = Delete
				levelClone = clone.appendChild(levelStartNode
						.cloneNode(levelStartNode == startNode));
		...
		for (var k = i; k < endParents.length; k++) {
			...
			// For Extract and Clone, we must clone this level.
			if (action > 0 && levelStartNode != endNode) // action = 0 =
				// Delete
				levelClone = clone.appendChild(levelStartNode
						.cloneNode(levelStartNode == endNode));

			// The processing of siblings may have already been done by the
			// parent.
			if (!startParents[k]
					|| levelStartNode.parentNode != startParents[k].parentNode) {
				currentNode = levelStartNode.previousSibling;

				while (currentNode) {
					// Stop processing when the current node matches a node in
					// the
					// startParents tree or if it is the startNode.
					if (currentNode == startParents[k]
					...
					if (action == 2) // 2 = Clone
						...
						// When Extracting, mode the removed node to the
						// docFrag.
						if (action == 1) // 1 = Extract
		...
		if (action == 2) // 2 = Clone.
		{
			// No changes in the DOM should be done, so fix the split text (if
			// any).
			...
			if (topStart
					&& topEnd
					&& (startNode.parentNode != topStart.parentNode || endNode.parentNode != topEnd.parentNode)) {
				var endIndex = FCKDomTools.GetIndexOf(topEnd);

				// If the start node is to be removed, we must correct the
				// index to reflect the removal.
				if (removeStartNode
						&& topEnd.parentNode == startNode.parentNode)
					endIndex--;
