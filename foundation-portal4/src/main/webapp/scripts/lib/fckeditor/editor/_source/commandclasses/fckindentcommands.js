...
FCKIndentCommand.prototype = {
	Execute : function() {
		...
		// If we're in a list, then the indent/outdent operations would be done
		// on the list nodes.
		// Otherwise, apply the operation on the nearest block nodes.
		var nearestListBlock = FCKDomTools.GetCommonParentNode(range.StartNode
						|| range.StartContainer, range.EndNode
						|| range.EndContainer, ['ul', 'ol']);
		if (nearestListBlock)
	...
	GetState : function() {
		...
		// If we're not in a list, and the starting block's indentation is zero,
		// and the current
		// command is the outdent command, then we should return
		...
		var listNode = FCKDomTools.GetCommonParentNode(startContainer,
				endContainer, ['ul', 'ol']);
		...
		if (FCKIndentCommand._UseIndentClasses) {
			var indentClass = firstBlock.className
					.match(FCKIndentCommand._ClassNameRegex);
			var indentStep = 0;
			if (indentClass != null) {
	...
	_IndentBlock : function(range) {
		...
		var commonParents = FCKDomTools.GetCommonParents(range.StartContainer,
				range.EndContainer);
		var nearestParent = commonParents[commonParents.length - 1];
		var block;

		while ((block = iterator.GetNextParagraph())) {
			// We don't want to indent subtrees recursively, so only perform the
			// indent operation
			// if the block itself is the nearestParent, or the block's parent
			// is the nearestParent.
			if (!(block == nearestParent || block.parentNode == nearestParent))
				continue;

			if (FCKIndentCommand._UseIndentClasses) {
				// Transform current class name to indent step index.
				var indentClass = block.className
						.match(FCKIndentCommand._ClassNameRegex);
				var indentStep = 0;
				if (indentClass != null) {
					indentClass = indentClass[1];
				...
				if (this.Name.IEquals('outdent'))
					...
					indentStep++;
				indentStep = Math.min(indentStep,
						FCKConfig.IndentClasses.length);
				indentStep = Math.max(indentStep, 0);
				var className = block.className.replace(
						FCKIndentCommand._ClassNameRegex, '');
				if (indentStep < 1)
					block.className = className;
				else
					block.className = (className.length > 0
							? className + ' '
							: '')
				...
				var currentOffset = parseInt(
						block.style[this.IndentCSSProperty], 10);
				if (isNaN(currentOffset))
					currentOffset = 0;
				currentOffset += this.Offset;
				currentOffset = Math.max(currentOffset, 0);
				currentOffset = Math.ceil(currentOffset / this.Offset)
						* this.Offset;
				block.style[this.IndentCSSProperty] = currentOffset
						? currentOffset + FCKConfig.IndentUnit
						: '';
	...
	_IndentList : function(range, listNode) {
		// Our starting and ending points of the range might be inside some
		// blocks under a list item...
		// So before playing with the iterator, we need to expand the block to
		// include the list items.
		...
		// Do indent or outdent operations on the array model of the list, not
		// the list's DOM tree itself.
		// The array model demands that it knows as much as possible about the
		// surrounding lists, we need
		...
		for (var i = lastItem._FCK_ListArray_Index + 1; i < listArray.length
				&& listArray[i].indent > baseIndent; i++)
			listArray[i].indent += indentOffset;

		/*
		 * For debug use only var PrintArray = function( listArray, doc ) { var
		 * s = [] ; for ( var i = 0 ; i < listArray.length ; i++ ) { for ( var j
		 * in listArray[i] ) { if ( j != 'contents' ) s.push( j + ":" +
		 * listArray[i][j] + "; " ) ; else { var docFrag =
		 * doc.createDocumentFragment() ; var tmpNode = doc.createElement(
		 * 'span' ) ; for ( var k = 0 ; k < listArray[i][j].length ; k++ )
		 * docFrag.appendChild( listArray[i][j][k].cloneNode( true ) ) ;
		 * tmpNode.appendChild( docFrag ) ; s.push( j + ":" + tmpNode.innerHTML + "; ") ; } }
		 * s.push( '\n' ) ; } alert( s.join('') ) ; } PrintArray( listArray,
		 * FCK.EditorDocument ) ;
		 */

		// Convert the array back to a DOM forest (yes we might have a few
		// subtrees now).
