 ...
 * 
 * FCKIcon Class: renders an icon from a single image, a strip or even a spacer.
 */

var FCKIcon = function(iconPathOrStripInfoArray) {
	var sTypeOf = iconPathOrStripInfoArray
			? typeof(iconPathOrStripInfoArray)
			: 'undefined';
	switch (sTypeOf) {
		case 'number' :
		...
		default :
			// It is an array in the format [ StripFilePath, IconSize,
			// IconPosition ]
...
FCKIcon.prototype.CreateIconElement = function(document) {
	...
	if (this.Position) // It is using an icons strip image.
	{
		...
		if (FCKBrowserInfo.IsIE) {
			...
			// style="background-position: 0px -16px;background-image:
			// url(strip.gif);">
