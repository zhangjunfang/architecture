...
function FCKTextColorCommand_MoreOnClick(ev, command) {
	...
	FCKDialog.OpenDialog('FCKDialog_Color', FCKLang.DlgColorTitle,
			'dialog/fck_colorselector.html', 410, 320, FCKTools.Bind(command,
					command.SetColor));
}
...
FCKTextColorCommand.prototype._CreatePanelBody = function(targetDocument,
	...
	FCKTools.AddEventListenerEx(oDiv, 'click', FCKTextColorCommand_AutoOnClick,
			this);
	...
	while (iCounter < aColors.length) {
		...
		for (var i = 0; i < 8; i++, iCounter++) {
			...
			oDiv.innerHTML = '<div class="ColorBoxBorder"><div class="ColorBox" style="background-color: '
					+ colorValue + '"></div></div>';

			if (iCounter >= aColors.length)
				oDiv.style.visibility = 'hidden';
			else
				FCKTools.AddEventListenerEx(oDiv, 'click',
						FCKTextColorCommand_OnClick, [this, colorName]);
		}
	}
	...
	if (FCKConfig.EnableMoreFontColors) {
		...
		oDiv.innerHTML = '<table width="100%" cellpadding="0" cellspacing="0" border="0"><tr><td nowrap align="center">'
