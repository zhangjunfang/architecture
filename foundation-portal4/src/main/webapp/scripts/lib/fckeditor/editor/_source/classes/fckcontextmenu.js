...
var FCKContextMenu = function(parentWindow, langDir) {
	...
	// The FCKTools.DisableSelection doesn't seems to work to avoid dragging of
	// the icons in Mozilla
...
/**
 * The customData parameter is just a value that will be send to the command
 * that is executed, so it's possible to reuse the same command for several
 * items just by assigning different data for each one.
 */
FCKContextMenu.prototype.AddItem = function(name, label,
		iconPathOrStripInfoArrayOrIndex, isDisabled, customData) {
...
function FCKContextMenu_Document_OnContextMenu(e) {
	...
	while (el) {
		if (el._FCKContextMenu) {
			...
			FCKContextMenu_AttachedElement_OnContextMenu(e, el._FCKContextMenu,
					el);
			return false;
		}
		el = el.parentNode;
	...
	while (el) {
		if (el._FCKContextMenu) {
			...
			if (!overrideButton) {
				...
				overrideButton = FCKContextMenu_OverrideButton = doc
						.createElement('input');
			...
			overrideButton.style.cssText = 'position:absolute;top:'
					+ (e.clientY - 2) + 'px;left:' + (e.clientX - 2)
