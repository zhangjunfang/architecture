...
FCKToolbarStyleCombo.prototype.CreateItems = function(targetSpecialCombo) {
	...
	// Add the Editor Area CSS to the panel so the style classes are previewed
	// correctly.
	...
	for (var styleName in styles) {
		...
		var caption = style.GetType() == FCK_STYLE_OBJECT
				? styleName
				: FCKToolbarStyleCombo_BuildPreview(style, style.Label
								|| styleName);
...
FCKToolbarStyleCombo.prototype.StyleCombo_OnBeforeClick = function(
	...
	for (var i in targetSpecialCombo.Items) {
		...
		if ((tagName && style.Element == tagName)
				|| (!tagName && style.GetType() != FCK_STYLE_OBJECT)) {
			item.style.display = '';

			if ((path && style.CheckActive(path))
					|| (!path && style
							.CheckElementRemovable(startElement, true)))
