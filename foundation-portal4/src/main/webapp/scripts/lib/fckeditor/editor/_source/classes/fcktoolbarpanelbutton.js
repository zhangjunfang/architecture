 ...
 * 
 * FCKToolbarPanelButton Class: represents a special button in the toolbar that
 * shows a panel when pressed.
 */
...
var FCKToolbarPanelButton = function(commandName, label, tooltip, style, icon) {
	...
	if (icon == null)
		oIcon = FCKConfig.SkinPath + 'toolbar/' + commandName.toLowerCase()
				+ '.gif';
	else if (typeof(icon) == 'number')
		oIcon = [FCKConfig.SkinPath + 'fck_strip.gif', 16, icon];

	var oUIButton = this._UIButton = new FCKToolbarButtonUI(commandName, label,
			tooltip, oIcon, style);
...
FCKToolbarPanelButton.prototype.Create = function(parentElement) {
	...
	var oPanel = FCK.ToolbarSet.CurrentInstance.Commands
...
FCKToolbarPanelButton.prototype.RegisterPanel = function(oPanel) {
	...
	var eLineDiv = oPanel.Document.body.appendChild(oPanel.Document
			.createElement('div'));
	eLineDiv.style.position = 'absolute';
	eLineDiv.style.top = '0px';

	var eLine = oPanel._FCKToolbarPanelButtonLineDiv = eLineDiv
			.appendChild(oPanel.Document.createElement('IMG'));
...
function FCKToolbarPanelButton_OnButtonClick(toolbarButton) {
	...
	var oCommand = FCK.ToolbarSet.CurrentInstance.Commands
			.GetCommand(oButton.CommandName);
	var oPanel = oCommand._Panel;
	oPanel._FCKToolbarPanelButtonLineDiv.style.width = (e.offsetWidth - 2)
			+ 'px';
