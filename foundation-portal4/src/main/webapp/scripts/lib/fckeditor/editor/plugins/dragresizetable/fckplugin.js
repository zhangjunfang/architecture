var FCKDragTableHandler = {
	...
	"_GetBorderCells" : function(w, tableNode, tableMap, mouse) {
		...
		for (var i = 0; i < cells.length; i++) {
			...
			if (minRxDist == null
					|| (Math.abs(rxDist) <= Math.abs(minRxDist) && (minYDist == null || Math
							.abs(yDist) <= Math.abs(minYDist)))) {
				minRxDist = rxDist;
				minYDist = yDist;
				rbCell = cells[i];
			}
		...
		/*
		 * var rowNode = FCKTools.GetElementAscensor( rbCell, "tr" ) ; var
		 * cellIndex = rbCell.cellIndex + 1 ; if ( cellIndex >=
		 * rowNode.cells.length ) return null ; lbCell = rowNode.cells.item(
		 * cellIndex ) ;
		 */
		var rowIdx = rbCell.parentNode.rowIndex;
		var colIdx = FCKTableHandler
				._GetCellIndexSpan(tableMap, rowIdx, rbCell);
	...
	"_ResizeBarMouseDownListener" : function(evt) {
		...
		if (FCKBrowserInfo.IsIE)
			FCKDragTableHandler._ResizeBar.filters
					.item("DXImageTransform.Microsoft.Alpha").opacity = 50;
		else
		...
		var table = FCKTools.GetElementAscensor(FCKDragTableHandler._LeftCell,
				"table");
		var minX = null;
		var maxX = null;
		for (var r = 0; r < FCKDragTableHandler._TableMap.length; r++) {
			...
			var rightPosition = FCKTools.GetWindowPosition(FCK.EditorWindow,
					rightCell);
			var leftPadding = FCKDragTableHandler._GetCellPadding(table,
					leftCell);
			...
			if (maxX == null
					|| rightPosition.x + rightCell.clientWidth - rightPadding < maxX)
		...
		for (var i = 0; i < tableMap.length; i++) {
			for (var j = 0; j < tableMap[i].length; j++) {
				...
				if (tableMap[i][j - 1] != cell)
					...
					cell.width = parseInt(cell.width, 10)
							+ parseInt(colArray[j].width, 10);
				if (tableMap[i][j + 1] != cell) {
	...
	"_GetCellPadding" : function(table, cell) {
		...
		if (typeof(window.getComputedStyle) == "function") {
			...
			cssGuess = parseInt(cell.currentStyle.paddingLeft, 10)
					+ parseInt(cell.currentStyle.paddingRight, 10);
	...
	// The real width of the table cell is the pixel width that you can set to
	// the width attribute of the table cell and after
	...
	"_MouseFindHandler" : function(FCK, evt) {
		...
		if (this._ResizeBar && this._LeftCell) {
			...
			if (lxDist >= 0 && rxDist <= 0)
				...
				inRangeFlag = true;
			if (inRangeFlag) {
				this._ShowResizeBar(FCK.EditorWindow, FCKTools
								.GetElementAscensor(this._LeftCell, "table"), {
							"x" : mouseX,
							"y" : mouseY
						});
				return;
		...
		if (cellTuple == null) {
			...
			this._ShowResizeBar(FCK.EditorWindow, FCKTools.GetElementAscensor(
							this._LeftCell, "table"), {
						"x" : mouseX,
						"y" : mouseY
					});
		}
	...
	"_MouseDragHandler" : function(FCK, evt) {
		...
		// Calculate the mouse position delta and see if we've gone out of
		// range.
	...
	"_ShowResizeBar" : function(w, table, mouse) {
		if (this._ResizeBar == null) {
			...
			// IE doesn't let the tranparent part of the padding block to
			// receive mouse events unless there's something inside.
		...
		var paddingStyles = {
			...
			'height' : barHeight + 'px',
			'width' : barWidth + 'px',
			'left' : (offset.x + mouse.x + FCKTools.GetScrollPosition(w).X - barWidth
					/ 2)
					+ 'px'
		};
		if (FCKBrowserInfo.IsIE)
	...
	"_HideResizeBar" : function() {
		if (this._ResizeBar)
			// IE bug: display : none does not hide the resize bar for some
			// reason.
