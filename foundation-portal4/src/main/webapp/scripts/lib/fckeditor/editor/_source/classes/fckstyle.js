 ...
 * 
 * FCKStyle Class: contains a style definition, and all methods to work with the
 * style in a document.
 */

/**
 * @param {Object}
 *            styleDesc A "style descriptor" object, containing the raw style
 *            definition in the following format: '<style name>' : { Element : '<element
 *            name>', Attributes : { '<att name>' : '<att value>', ... },
 *            Styles : { '<style name>' : '<style value>', ... }, Overrides : '<element
 *            name>'|{ Element : '<element name>', Attributes : { '<att name>' : '<att
 *            value>'|/<att regex>/ }, Styles : { '<style name>' : '<style
 *            value>'|/<style regex>/ }, } }
 */
...
FCKStyle.prototype = {
	/**
	 * Get the style type, based on its element name: - FCK_STYLE_BLOCK (0):
	 * Block Style - FCK_STYLE_INLINE (1): Inline Style - FCK_STYLE_OBJECT (2):
	 * Object Style
	 */
	GetType : function() {
	...
	RemoveFromRange : function(range, selectIt, updateRange) {
		...
		if (range.CheckIsCollapsed()) {
			...
			var isBoundary = isBoundaryRight
					|| !FCKDomTools.GetPreviousSibling(bookmarkStart);
			...
			for (var i = 0; i < path.Elements.length; i++) {
				...
				if (this.CheckElementRemovable(pathElement)) {
					if (isBoundary
						...
						var pathElementName = pathElement.nodeName
								.toLowerCase();

						if (pathElementName == this.Element) {
							// Remove any attribute that conflict with this
							// style, no
							// matter their values.
							for (var att in styleAttribs) {
								if (FCKDomTools.HasAttribute(pathElement, att)) {
									switch (att) {
										case 'style' :
											this
													._RemoveStylesFromElement(pathElement);
											break;

										case 'class' :
											// The 'class' element value must
											// match (#1318).
											if (FCKDomTools.GetAttributeValue(
													pathElement, att) != this
													.GetFinalAttributeValue(att))
												continue;

											/* jsl:fallthru */

										default :
											FCKDomTools.RemoveAttribute(
													pathElement, att);
									}
								}
							}
						...
						this._RemoveOverrides(pathElement,
								styleOverrides[pathElementName]);

						// Remove the element if no more attributes are
						// available and it's an inline style element
				...
				isBoundary = isBoundary
						&& ((isBoundaryRight && !FCKDomTools
								.GetNextSibling(pathElement)) || (!isBoundaryRight && !FCKDomTools
								.GetPreviousSibling(pathElement)));
				...
				if (lastBoundaryElement
						&& (!isBoundary || (i == path.Elements.length - 1))) {
					...
					for (var j = 0; j <= boundaryLimitIndex; j++) {
						var newElement = FCKDomTools
								.CloneElement(boundaryElements[j]);
						newElement.appendChild(currentElement);
						currentElement = newElement;
					}

					// Re-insert the bookmark node (and the collapsed elements)
					// in the DOM, in the new position next to the styled
					// element.
					if (isBoundaryRight)
						FCKDomTools.InsertAfterNode(lastBoundaryElement,
								currentElement);
					else
						lastBoundaryElement.parentNode.insertBefore(
								currentElement, lastBoundaryElement);
		...
		// the code in a way that we can properly remove partially selected
		// nodes.
		...
		while (currentNode) {
			...
			if (currentNode.nodeType == 1) {
					...
					for (var att in styleAttribs) {
						if (FCKDomTools.HasAttribute(currentNode, att)) {
							switch (att) {
								...
								case 'class' :
									// The 'class' element value must match
									// (#1318).
									if (FCKDomTools.GetAttributeValue(
											currentNode, att) != this
											.GetFinalAttributeValue(att))
										continue;
								...
								default :
									FCKDomTools.RemoveAttribute(currentNode,
											att);
							}
						}
					}
				...
				if (mayRemove) {
					// Remove overrides defined to the same element name.
					this._RemoveOverrides(currentNode,
							styleOverrides[elementName]);
	...
	CheckElementRemovable : function(element, fullMatch) {
		...
		if (elementName == this.Element) {
			...
			for (var att in attribs) {
				...
				if (this._CompareAttributeValues(att, FCKDomTools
								.GetAttributeValue(element, att), (this
								.GetFinalAttributeValue(att) || ''))) {
					allMatched = true;
					if (!fullMatch)
						break;
		...
		if (override) {
			...
			for (var i = 0; i < attribs.length; i++) {
				...
				if (FCKDomTools.HasAttribute(element, attName)) {
					...
					if (attValue == null
							|| (typeof attValue == 'string' && FCKDomTools
									.GetAttributeValue(element, attName) == attValue)
							|| attValue.test(FCKDomTools.GetAttributeValue(
									element, attName)))
						return true;
				}
			}
	...
	/**
	 * Get the style state for an element path. Returns "true" if the element is
	 * active in the path.
	 */
	...
	RemoveFromElement : function(element) {
				...
				if (FCKDomTools.HasAttribute(innerElement, att)) {
					switch (att) {
						...
						case 'class' :
							...
							if (FCKDomTools
									.GetAttributeValue(innerElement, att) != this
									.GetFinalAttributeValue(att))
								continue;
		...
		for (var overrideElement in overrides) {
			if (overrideElement != this.Element) {
				...
				for (var i = innerElements.length - 1; i >= 0; i--) {
					var innerElement = innerElements[i];
					this._RemoveOverrides(innerElement,
		...
		if (attributes) {
			for (var i = 0; i < attributes.length; i++) {
				...
				if (FCKDomTools.HasAttribute(element, attName)) {
					...
					// has matches in the attribute value.
					if (attValue == null
							|| (attValue.test && attValue.test(FCKDomTools
									.GetAttributeValue(element, attName)))
							|| (typeof attValue == 'string' && FCKDomTools
									.GetAttributeValue(element, attName) == attValue))
	...
	_CompareAttributeValues : function(attName, valueA, valueB) {
		...
		// Return true if they match or if valueA is null and valueB is an empty
		// string
	...
	GetFinalAttributeValue : function(attName) {
		...
		if (attValue && this._Variables)
			...
			attValue = attValue.Replace(FCKRegexLib.StyleVariableAttName,
	...
	/**
	 * Set the value of a variable attribute or style, to be used when appliying
	 * the style.
	 */
	...
	_FromPre : function(doc, block, newBlock) {
		...
		// Trim the first and last linebreaks immediately after and before
		// <pre>, </pre>,
		// if they exist.
	...
	_CheckAndSplitPre : function(newBlock) {
		...
		while (cursor) {
			...
			// If we have two <BR>s, and they're not at the beginning or the
			// end,
			// then we'll split up the contents following them into another
			// block.
			// Stop processing if we are at the last child couple.
			if (next && next.nextSibling && cursor.nodeName.IEquals('br')
					&& next.nodeName.IEquals('br')) {
				// Remove the first <br>.
				...
				lastNewBlock = FCKDomTools
						.InsertAfterNode(lastNewBlock || newBlock, FCKDomTools
										.CloneElement(newBlock));
	 ...
	 * 
	 * TODO - Implement the "#" style handling. - Properly handle block
	 * containers like <div> and <blockquote>.
	 */
	_ApplyBlockStyle : function(range, selectIt, updateRange) {
		...
		while ((block = iterator.GetNextParagraph())) // Only one =
		{
			...
			if (newBlockIsPre) {
				if (previousPreBlock)
					this._CheckAndMergePre(previousPreBlock, newBlock); // Merge
				// successive
				// <pre>
				// blocks.
				previousPreBlock = newBlock;
			} else if (fromPre)
				this._CheckAndSplitPre(newBlock); // Split <br><br> in
			// successive <pre>s.
		}
	 ...
	 * Apply an inline style to a FCKDomRange.
	 * 
	 * TODO - Merge elements, when applying styles to similar elements that
	 * enclose the entire selection, outputing: <span style="color: #ff0000;
	 * background-color: #ffffff">XYZ</span> instead of: <span style="color:
	 * #ff0000;"><span style="background-color: #ffffff">XYZ</span></span>
	 */
	_ApplyInlineStyle : function(range, selectIt, updateRange) {
		...
		while (currentNode) {
			...
			if (!nodeName || elementDTD[nodeName]) {
				...
				if ((FCK.DTD[currentNode.parentNode.nodeName.toLowerCase()] || FCK.DTD.span)[elementName]
					...
					if (nodeType != 1 || currentNode.childNodes.length == 0) {
						...
						if (includedNode == includedNode.parentNode.lastChild
								&& !elementDTD[includedNode.parentNode.nodeName
										.toLowerCase()])
							applyStyle = true;
					} else {
			...
			if (applyStyle && range.CheckHasRange()
					&& !range.CheckIsCollapsed()) {
				// Build the style element, based on the style object
				// definition.
				...
				if (styleNode.innerHTML.RTrim().length > 0) {
					...
					// Let's merge our new style with its neighbors, if
					// possible.
					this._MergeSiblings(styleNode, this
									._GetAttribsForComparison());

					// As the style system breaks text nodes constantly, let's
					// normalize
					// things for performance.
					// With IE, some paragraphs get broken when calling
					// normalize()
					// repeatedly. Also, for IE, we must normalize body, not
					// documentElement.
					// IE is also known for having a "crash effect" with
					// normalize().
					// We should try to normalize with IE too in some way,
					// somewhere.
					if (!FCKBrowserInfo.IsIE)
						styleNode.normalize();
				}
	...
	_FixBookmarkStart : function(startNode) {
		...
		while ((startSibling = startNode.nextSibling)) // Only one "=".
		{
			if (startSibling.nodeType == 1
					&& FCKListsLib.InlineNonEmptyElements[startSibling.nodeName
							.toLowerCase()]) {
	...
	/**
	 * Merge an element with its similar siblings. "attribs" is and object
	 * computed with _CreateAttribsForComparison.
	 */
	_MergeSiblings : function(element, attribs) {
		if (!element
				|| element.nodeType != 1
				|| !FCKListsLib.InlineNonEmptyElements[element.nodeName
						.toLowerCase()])
			return;
	...
	/**
	 * Merge an element with its similar siblings after it. "attribs" is and
	 * object computed with _CreateAttribsForComparison.
	 */
	_MergeNextSibling : function(element, attribs) {
		// Check the next sibling.
		var sibling = element.nextSibling;

		// Check if the next sibling is a bookmark element. In this case, jump
		// it.
		var hasBookmark = (sibling && sibling.nodeType == 1 && sibling
	...
	/**
	 * Merge an element with its similar siblings before it. "attribs" is and
	 * object computed with _CreateAttribsForComparison.
	 */
	_MergePreviousSibling : function(element, attribs) {
		...
		// Check if the previous sibling is a bookmark element. In this case,
		// jump it.
		var hasBookmark = (sibling && sibling.nodeType == 1 && sibling
				.getAttribute('_fck_bookmark'));
		if (hasBookmark)
		...
		if (sibling && sibling.nodeType == 1
			...
			if (this._CheckAttributesMatch(sibling, attribs)) {
				// Save the first child to be checked too (to merge things like
				// <b><i></i></b><b><i></i></b>).
				var innerSibling = element.firstChild;

				if (hasBookmark)
					FCKDomTools
							.MoveNode(element.previousSibling, element, true);
	...
	_GetStyleText : function() {
		...
		var stylesText = (this._StyleDesc.Attributes
	...
	_GetAttribsForComparison : function() {
		...
		if (styleAttribs) {
			for (var styleAtt in styleAttribs) {
				attribs[styleAtt.toLowerCase()] = styleAttribs[styleAtt]
						.toLowerCase();
			}
		}
		...
		if (overridesDesc) {
			...
			for (var i = 0; i < overridesDesc.length; i++) {
				...
				// Or an object.
					...
					elementName = override.Element ? override.Element
							.toLowerCase() : this.Element;
				...
				if (attrs) {
					...
					for (var attName in attrs) {
						...
						// override value.
						overrideAttrs.push([attName.toLowerCase(),
								attrs[attName]]);
					}
				}
			}
	...
	_CreateElementAttribsForComparison : function(element) {
		...
		for (var i = 0; i < element.attributes.length; i++) {
			...
			if (att.specified) {
				attribs[att.nodeName.toLowerCase()] = FCKDomTools
						.GetAttributeValue(element, att).toLowerCase();
				attribsCount++;
			}
		}
	...
	_CheckAttributesMatch : function(element, styleAttribs) {
		...
		for (var i = 0; i < elementAttrbs.length; i++) {
			...
			if (att.specified) {
				...
				if (styleAtt != FCKDomTools.GetAttributeValue(element, att)
						.toLowerCase())
					break;
