 ...
 * 
 * FCKToolbarSpecialCombo Class: This is a "abstract" base class to be used by
 * the special combo toolbar elements like font name, font size, paragraph
 * format, etc...
 * 
 * The following properties and methods must be implemented when inheriting from
 * this class: - Property: CommandName [ The command name to be executed ] -
 * Method: GetLabel() [ Returns the label ] - CreateItems( targetSpecialCombo ) [
...
FCKToolbarSpecialCombo.prototype.Create = function(targetElement) {
	 ...
	 * this._Combo.PanelMaxHeight = this.PanelMaxHeight != null ?
...
FCKToolbarSpecialCombo.prototype.RefreshState = function() {
	...
	// FCKDebug.Output( 'RefreshState of Special Combo "' + this.TypeOf + '" -
	// State: ' + sValue ) ;
		...
		if (this.RefreshActiveItems)
			...
			if (this._LastValue !== sValue) {
				...
				if (!sValue || sValue.length == 0) {
					...
					FCKToolbarSpecialCombo_RefreshActiveItems(this._Combo,
							sValue);
			}
		}
	} else
