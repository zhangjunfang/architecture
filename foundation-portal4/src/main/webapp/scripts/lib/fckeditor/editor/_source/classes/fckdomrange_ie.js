 ...
 * is not intended to be an implementation of the W3C interface. (IE
 * Implementation)
 */
...
FCKDomRange.prototype.MoveToSelection = function() {
	...
	if (oSel.type != 'Control') {
		...
		this._Range.setStart(eMarkerStart.parentNode, FCKDomTools
						.GetIndexOf(eMarkerStart));
		eMarkerStart.parentNode.removeChild(eMarkerStart);

		// Set the end boundary.
		this._Range.setEnd(eMarkerEnd.parentNode, FCKDomTools
						.GetIndexOf(eMarkerEnd));
...
FCKDomRange.prototype.SelectBookmark = function(bookmark, forceExpand) {
	...
	if (eEndMarker) {
		...
		bIsStartMakerAlone = (forceExpand || !eStartMarker.previousSibling || eStartMarker.previousSibling.nodeName
				.toLowerCase() == 'br')
		...
		dummySpan.innerHTML = '&#65279;'; // Zero Width No-Break Space
		// (U+FEFF). See #1359.
		...
		if (bIsStartMakerAlone) {
			...
			eStartMarker.parentNode.insertBefore(this.Window.document
							.createTextNode('\ufeff'), eStartMarker);
		}
	}
	...
	// Remove the markers (reset the position, because of the changes in the DOM
	// tree).
...
FCKDomRange.prototype._GetSelectionMarkerTag = function(toStart) {
	...
	// IE might take the range object to the main window instead of inside the
	// editor iframe window.
	// This is known to happen when the editor window has not been selected
	// before (See #933).
	...
	var sMarkerId = 'fck_dom_range_temp_' + (new Date()).valueOf() + '_'
