...
var FCKListHandler = {
	OutdentListItem : function(listItem) {
		...
		if (eParent.tagName.toUpperCase().Equals('UL', 'OL')) {
			...
			// All children and successive siblings will be moved to a a
			// DocFrag.
			var eNextSiblings = oDogFrag.RootNode;
			var eHasLiSibling = false;

			// If we have nested lists inside it, let's move it to the list of
			// siblings.
			...
			if (eChildList) {
				...
				var eChild;
				// The extra () is to avoid a warning with strict error
				// checking. This is ok.
			...
			while ((eSibling = listItem.nextSibling)) {
				if (!eHasLiSibling && eSibling.nodeType == 1
						&& eSibling.nodeName.toUpperCase() == 'LI')
					eHasSuccessiveLiSibling = eHasLiSibling = true;

				eNextSiblings.appendChild(eSibling.parentNode
						.removeChild(eSibling));

				// If a sibling is a incorrectly nested UL or OL, consider only
				// its children.
				if (!eHasSuccessiveLiSibling && eSibling.nodeType == 1
			...
			if (bWellNested || sParentParentTag.Equals('UL', 'OL')) {
				...
				// Move the LI after its parent.parentNode (the upper LI in the
				// hierarchy).
				if (bWellNested)
					FCKDomTools.InsertAfterNode(eParent.parentNode, eParent
									.removeChild(listItem));
				else
					FCKDomTools.InsertAfterNode(eParent, eParent
									.removeChild(listItem));
			} else {
				if (eHasLiSibling) {
				...
				var eBlock = oDocument.createElement(FCKConfig.EnterMode == 'p'
						? 'p'
						: 'div');
				...
				if (FCKConfig.EnterMode == 'br') {
					...
					if (FCKBrowserInfo.IsGecko)
						eBlock.parentNode.insertBefore(FCKTools
										.CreateBogusBR(oDocument), eBlock);
					else
						FCKDomTools.InsertAfterNode(eBlock, FCKTools
										.CreateBogusBR(oDocument));
