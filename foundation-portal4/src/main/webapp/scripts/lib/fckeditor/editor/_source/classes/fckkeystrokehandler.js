...
/*
 * Listen to keystroke events in an element or DOM document object. @target: The
 * element or document to listen to keystroke events.
 */
FCKKeystrokeHandler.prototype.AttachToElement = function(target) {
	...
	FCKTools.AddEventListenerEx(target, 'keydown',
			_FCKKeystrokeHandler_OnKeyDown, this);
	if (FCKBrowserInfo.IsGecko10 || FCKBrowserInfo.IsOpera
			|| (FCKBrowserInfo.IsGecko && FCKBrowserInfo.IsMac))
		FCKTools.AddEventListenerEx(target, 'keypress',
				_FCKKeystrokeHandler_OnKeyPress, this);
}

/*
 * Sets a list of keystrokes. It can receive either a single array or "n"
 * arguments, each one being an array of 1 or 2 elemenst. The first element is
 * the keystroke combination, and the second is the value to assign to it. If
 * the second element is missing, the keystroke definition is removed.
 */
FCKKeystrokeHandler.prototype.SetKeystrokes = function() {
	...
	for (var i = 0; i < arguments.length; i++) {
		...
		// If the configuration for the keystrokes is missing some element or
		// has any extra comma
		// this item won't be valid, so skip it and keep on processing.
		if (!keyDef)
			continue;

		if (typeof(keyDef[0]) == 'object') // It is an array with arrays
			// defining the keystrokes.
			this.SetKeystrokes.apply(this, keyDef);
		else {
			if (keyDef.length == 1) // If it has only one element, remove the
				// keystroke.
...
function _FCKKeystrokeHandler_OnKeyDown(ev, keystrokeHandler) {
	...
	// FCKDebug.Output( 'KeyDown: ' + keyCombination + ' - Value: ' +
	// keystrokeValue ) ;
	...
	if (keystrokeValue) {
		...
		if (keystrokeValue === true
				|| !(keystrokeHandler.OnKeystroke && keystrokeHandler.OnKeystroke
						.apply(keystrokeHandler, keystrokeValue)))
			return true;

		cancelIt = true;
	}

	// By default, it will cancel all combinations with the CTRL key only
	// (except positioning keys).
	if (cancelIt
			|| (keystrokeHandler.CancelCtrlDefaults && keyModifiers == CTRL && (keystroke < 33 || keystroke > 40))) {
