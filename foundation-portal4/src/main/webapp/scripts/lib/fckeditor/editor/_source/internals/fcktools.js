...
/**
 * Fixes relative URL entries defined inside CSS styles by appending a prefix to
 * them.
 * 
 * @param (String)
 *            cssStyles The CSS styles definition possibly containing url()
 *            paths.
 * @param (String)
 *            urlFixPrefix The prefix to append to relative URLs.
 */
FCKTools.FixCssUrls = function(urlFixPrefix, cssStyles) {
	if (!urlFixPrefix || urlFixPrefix.length == 0)
		return cssStyles;

	return cssStyles.replace(/url\s*\(([\s'"]*)(.*?)([\s"']*)\)/g, function(
					match, opener, path, closer) {
				if (/^\/|^\w?:/.test(path))
					return match;
				else
 ...
 * 
 * @param (Object)
 *            documentElement The DOM document object to which append the
 *            stylesheet.
 * @param (Variant)
 *            cssFileOrDef A String pointing to the CSS file URL or an Array
 *            with many CSS file URLs or the CSS definitions for the <style>
 *            element.
...
FCKTools.AppendStyleSheet = function(domDocument, cssFileOrArrayOrDef) {
	...
	if (typeof(cssFileOrArrayOrDef) == 'string') {
		...
		if (/[\\\/\.][^{}]*$/.test(cssFileOrArrayOrDef)) {
			...
			return this.AppendStyleSheet(domDocument, cssFileOrArrayOrDef
							.split(','));
		} else
			return [this.AppendStyleString(domDocument, FCKTools
							._GetUrlFixedCss(cssFileOrArrayOrDef))];
	} else {
		var styles = [];
		for (var i = 0; i < cssFileOrArrayOrDef.length; i++)
			styles.push(this._AppendStyleSheet(domDocument,
					cssFileOrArrayOrDef[i]));
		return styles;
	}
}
...
FCKTools.GetStyleHtml = (function() {
	var getStyle = function(styleDef, markTemp) {
		...
		return '<' + 'style type="text/css"' + temp + '>' + styleDef + '<'
				+ '/style>';
	}
	...
	var getLink = function(cssFileUrl, markTemp) {
		...
		return '<' + 'link href="' + cssFileUrl
				+ '" type="text/css" rel="stylesheet" ' + temp + '/>';
	}
	...
	return function(cssFileOrArrayOrDef, markTemp) {
		...
		if (typeof(cssFileOrArrayOrDef) == 'string') {
			...
			if (/[\\\/\.][^{}]*$/.test(cssFileOrArrayOrDef)) {
				...
				return getStyle(this._GetUrlFixedCss(cssFileOrArrayOrDef),
						markTemp);
		} else {
			var html = '';
...
FCKTools.GetDocumentWindow = function(document) {
	// With Safari, there is not way to retrieve the window from the document,
	// so we must fix it.
...
FCKTools._ProcessLineBreaksForPMode = function(oEditor, text, liState, node,
		strArray) {
	var closeState = 0;
	var blockStartTag = "<p>";
	var blockEndTag = "</p>";
...
FCKTools._ProcessLineBreaksForDivMode = function(oEditor, text, liState, node,
		strArray) {
	var closeState = 0;
	...
	for (var i = 0; i < text.length; i++) {
		...
		if (closeState) {
			if (strArray[strArray.length - 1] == blockStartTag) {
				// A div tag must have some contents inside for it to be
				// visible.
				strArray.push("&nbsp;");
			}
			strArray.push(blockEndTag);
...
FCKTools._ProcessLineBreaksForBrMode = function(oEditor, text, liState, node,
		strArray) {
	var closeState = 0;
...
FCKTools.ProcessLineBreaks = function(oEditor, oConfig, text) {
	...
	if (enterMode == 'p')
		...
		this._ProcessLineBreaksForDivMode(oEditor, text, liState, node,
				strArray);
	else if (enterMode == 'br')
		this
				._ProcessLineBreaksForBrMode(oEditor, text, liState, node,
						strArray);
...
FCKTools.AddSelectOption = function(selectElement, optionText, optionValue) {
	var oOption = FCKTools.GetElementDocument(selectElement)
...
FCKTools.SetTimeout = function(func, milliseconds, thisObject, paramsArray,
		timerWindow) {
...
FCKTools.SetInterval = function(func, milliseconds, thisObject, paramsArray,
		timerWindow) {
...
FCKTools.ConvertStyleSizetoView = function(size) {
...
// Amended to check the element itself before working back up through the parent
// hierarchy
...
FCKTools.IsStrictMode = function(document) {
	...
	return ('CSS1Compat' == (document.compatMode || (FCKBrowserInfo.IsSafari
			? 'CSS1Compat'
			: null)));
}
...
FCKTools.AppendBogusBr = function(element) {
	...
	if (!eLastChild
			|| (eLastChild.getAttribute('type', 2) != '_moz' && eLastChild
...
FCKTools.GetDocumentPosition = function(w, node) {
	...
	while (curNode
			&& !(curWindow == w && (curNode == w.document.body || curNode == w.document.documentElement))) {
	...
	// 2. It matters is when we're in IE and the element has no positioned
	// ancestor.
...
FCKTools.ProtectFormStyles = function(formNode) {
	if (!formNode || formNode.nodeType != 1
			|| formNode.tagName.toLowerCase() != 'form')
		return [];
	var hijackRecord = [];
...
/**
 * Check if the passed argument is a real Array. It may not working when calling
 * it cross windows.
 */
...
/**
 * Appends a "length" property to an object, containing the number of properties
 * available on it, excluded the append property itself.
 */
...
FCKTools.Bind = function(subject, func) {
	return function() {
		return func.apply(subject, arguments);
	};
}

/**
 * Retrieve the correct "empty iframe" URL for the current browser, which causes
 * the minimum fuzz (e.g. security warnings in HTTPS, DNS error in IE5.5, etc.)
 * for that browser, making the iframe ready to DOM use whithout having to
...
FCKTools.ResetStyles = function(element) {
	element.style.cssText = 'margin:0;' + 'padding:0;' + 'border:0;'
