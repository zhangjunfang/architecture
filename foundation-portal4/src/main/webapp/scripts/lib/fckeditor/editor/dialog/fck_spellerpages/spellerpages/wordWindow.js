...
function setText(textIndex, wordIndex, newText) {
	...
	if (word) {
		...
		endStr = this.textInputs[textIndex].substring(pos + oldText.length,
...
function _lastPos(txtid, idx) {
	if (idx > 0)
		return this.indexes[txtid][idx - 1]
...
function printForHtml(n) {
	return n; // by FredCK
	/*
	 * var htmlstr = n; if( htmlstr.length == 1 ) { // do simple case statement
	 * if it's just one character switch ( n ) { case "\n": htmlstr = '<br/>';
	 * break; case "<": htmlstr = '&lt;'; break; case ">": htmlstr = '&gt;';
	 * break; } return htmlstr; } else { htmlstr = htmlstr.replace( /</g, '&lt' );
	 * htmlstr = htmlstr.replace( />/g, '&gt' ); htmlstr = htmlstr.replace(
	 * /\n/g, '<br/>' ); return htmlstr; }
	 */
...
function _wordInputStr(word) {
	...
	str += 'class="blend" type="text" value="' + word + '" size="'
			+ word.length + '">';
	return str;
}
