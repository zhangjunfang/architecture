 ...
 * @constructor
 * @param {String}
 *            targetElement The element that will hold the editing area. Any
 *            child element present in the target will be deleted.
 */
...
/**
 * @param {String}
 *            html The complete HTML for the page, including DOCTYPE and the
 *            <html> tag.
 */
FCKEditingArea.prototype.Start = function(html, secondCall) {
	...
	if (this.Mode == FCK_EDITMODE_WYSIWYG) {
		...
		if (FCK_IS_CUSTOM_DOMAIN)
			html = '<script>document.domain="' + FCK_RUNTIME_DOMAIN
					+ '";</script>' + html;

		// IE has a bug with the <base> tag... it must have a </base> closer,
		// otherwise the all successive tags will be set as children nodes of
		// the <base>.
		if (FCKBrowserInfo.IsIE)
			html = html.replace(/(<base[^>]*?)\s*\/?>(?!\s*<\/base>)/gi,
					'$1></base>');
		else if (!secondCall) {
			...
			if (oMatchBefore && oMatchAfter) {
				var sBody = html.substr(oMatchBefore[1].length, html.length
								- oMatchBefore[1].length
								- oMatchAfter[1].length); // This is the BODY
				// tag contents.

				html = oMatchBefore[1] + // This is the HTML until the
						// <body...> tag, inclusive.
						'&nbsp;' + oMatchAfter[1]; // This is the HTML from the
				// </body> tag, inclusive.

				// If nothing in the body, place a BOGUS tag so the cursor will
				// appear.
				if (FCKBrowserInfo.IsGecko
						&& (sBody.length == 0 || FCKRegexLib.EmptyParagraph
								.test(sBody)))
					sBody = '<br type="_moz">';
		...
		// IE: Avoid JavaScript errors thrown by the editing are source (like
		// tags events).
		// See #1055.
		...
		if (FCK_IS_CUSTOM_DOMAIN && FCKBrowserInfo.IsIE) {
			window._FCKHtmlToLoad = html.replace(/<head>/i, '<head>'
							+ sOverrideError);
			oIFrame.src = 'javascript:void( (function(){' + 'document.open() ;'
					+ 'document.domain="' + document.domain + '" ;'
					+ 'document.write( window.parent._FCKHtmlToLoad );'
					+ 'document.close() ;'
					+ 'window.parent._FCKHtmlToLoad = null ;' + '})() )';
		} else if (!FCKBrowserInfo.IsGecko) {
			// Firefox will render the tables inside the body in Quirks mode if
			// the
		...
		// setting the "src", to avoid the "secure/unsecure" message under
		// HTTPS.
		eTargetElement.appendChild(oIFrame);

		// Get the window and document objects used to interact with the newly
		// created IFRAME.
		...
		if (oIFrame.readyState && oIFrame.readyState != 'completed') {
			...
			setTimeout(function() {
						try {
							editArea.Window.document.documentElement
									.doScroll("left");
						} catch (e) {
		...
		var eTextarea = this.Textarea = oTargetDocument
...
function FCKEditingArea_CompleteStart() {
	// On Firefox, the DOM takes a little to become available. So we must wait
	// for it in a loop.
...
FCKEditingArea.prototype.MakeEditable = function() {
	...
	if (FCKBrowserInfo.IsIE) {
		 ...
		 * oDoc.execCommand( 'AutoDetect', false, false ) ; oDoc.execCommand(
		 * 'KeepSelection', false, true ) ;
		 */
	} else {
		try {
			...
			if (this._BodyHTML) {
				...
				oDoc.body.offsetLeft; // Don't remove, this is a hack to fix
				// Opera 9.50, see #2264.
				this._BodyHTML = null;
			}
			...
			// Tell Gecko (Firefox 1.5+) to enable or not live resizing of
			// objects (by Alfonso Martinez)
			...
			// In Firefox if the iframe is initially hidden it can't be set to
			// designMode and it raises an exception
			// So we set up a DOM Mutation event Listener on the HTML, as it
			// will raise several events when the document is visible again
...
// This function processes the notifications of the DOM Mutation event on the
// document
// We use it to know that the document will be ready to be editable again (or we
// hope so)
function FCKEditingArea_Document_AttributeNodeModified(evt) {
	...
	editingArea._timer = FCKTools.SetTimeout(
			FCKEditingArea_MakeEditableByMutation, 1000, editingArea);
}

// This function ideally should be called after the document is visible, it does
// clean up of the
...
FCKEditingArea.prototype.Focus = function() {
	try {
		if (this.Mode == FCK_EDITMODE_WYSIWYG) {
			...
			if ((!oDoc.hasFocus || oDoc.hasFocus())
					&& oDoc.activeElement == this.Textarea)
				return;
