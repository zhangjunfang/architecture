...
var FCKJustifyCommand = function(alignValue) {
	...
	if (cssClassName && cssClassName.length > 0)
		this._CssClassRegex = new RegExp('(?:^|\\s+)' + cssClassName
				+ '(?=$|\\s)');
}
...
FCKJustifyCommand.prototype = {
	Execute : function() {
		...
		while ((block = iterator.GetNextParagraph())) {
			...
			if (cssClassName) {
				...
				var className = block.className.replace(FCKJustifyCommand
								._GetClassNameRegex(), '');
	...
	GetState : function() {
		...
		if (FCKBrowserInfo.IsIE)
			...
			currentAlign = FCK.EditorWindow.getComputedStyle(firstBlock, '')
					.getPropertyValue('text-align');
		currentAlign = currentAlign.replace(/(-moz-|-webkit-|start|auto)/i, '');
		if ((!currentAlign && this.IsDefaultAlign)
