...
FCKListCommand.prototype = {
	GetState : function() {
		...
		// We'll use the style system's convention to determine list state
		// here...
		// If the starting block is a descendant of an <ol> or <ul> node, then
		// we're in a list.
	...
	Execute : function() {
		...
		// Midas lists rule #1 says we can create a list even in an empty
		// document.
		// But FCKDomRangeIterator wouldn't run if the document is really empty.
		// So create a paragraph if the document is empty and we're going to
		// create a list.
		...
		// Group the blocks up because there are many cases where multiple lists
		// have to be created,
		...
		while (nextRangeExists) {
			while ((block = iterator.GetNextParagraph())) {
				...
				for (var i = path.Elements.length - 1; i >= 0; i--) {
					...
					if (el.nodeName.IEquals(['ol', 'ul'])) {
						...
						// The last group object of the block limit element
						// should
						// no longer be valid. Since paragraphs after the list
						// should belong to a different group of paragraphs
						// before
						// the list. (Bug #1309)
						...
						if (groupObj)
							...
							FCKDomTools.SetElementMarker(markerObj, el,
									'_FCK_ListGroupObject', groupObj);
						}
						processedFlag = true;
						break;
					}
				...
				if (root._FCK_ListGroupObject)
					...
					FCKDomTools.SetElementMarker(markerObj, root,
							'_FCK_ListGroupObject', groupObj);
					listGroups.push(groupObj);
				}
			}
			...
			if (FCKBrowserInfo.IsIE)
				...
				if (rangeQueue == null) {
					...
					for (var i = 1; selectionObject
							&& i < selectionObject.rangeCount; i++)
						rangeQueue.push(selectionObject.getRangeAt(i));
				}
				if (rangeQueue.length < 1)
					nextRangeExists = false;
				else {
					var internalRange = FCKW3CRange.CreateFromRange(doc,
							rangeQueue.shift());
		...
		// Now we have two kinds of list groups, groups rooted at a list, and
		// groups rooted at a block limit element.
		// We either have to build lists or remove lists, for removing a list
		// does not makes sense when we are looking
		// at the group that's not rooted at lists. So we have three cases to
		// handle.
		var listsCreated = [];
			...
			if (currentNode && currentNode.nodeName.IEquals(this.TagName)) {
				...
				while (currentNode.firstChild)
					listNode.appendChild(currentNode
							.removeChild(currentNode.firstChild));
			}
			...
			if (currentNode && currentNode.nodeName.IEquals(this.TagName)) {
				...
				while (currentNode.lastChild)
					listNode.insertBefore(currentNode
									.removeChild(currentNode.lastChild),
							listNode.firstChild);
			}
		}
	...
	_ChangeListType : function(groupObj, markerObj, listsCreated) {
		...
		for (var i = 0; i < groupObj.contents.length; i++) {
			...
			FCKDomTools.SetElementMarker(markerObj, itemNode,
					'_FCK_ListItem_Processed', true);
		}
		var fakeParent = FCKTools.GetElementDocument(groupObj.root)
	...
	_CreateList : function(groupObj, listsCreated) {
		...
		// It is possible to have the contents returned by DomRangeIterator to
		...
		if (contents.length == 1 && contents[0] == groupObj.root) {
			...
			while (contents[0].firstChild)
				divBlock.appendChild(contents[0]
						.removeChild(contents[0].firstChild));
		...
		for (var i = 0; i < contents.length; i++)
			commonParent = FCKDomTools.GetCommonParents(commonParent,
					contents[i].parentNode).pop();

		// We want to insert things that are in the same tree level only, so
		// calculate the contents again
		...
		while (listContents.length) {
			...
			while (contentBlock.firstChild)
				docFrag.appendChild(contentBlock
						.removeChild(contentBlock.firstChild));
	...
	_RemoveList : function(groupObj, markerObj) {
		...
		// Except that we're changing the selected items' indent to -1 in the
		// list array.
		var listArray = FCKDomTools.ListToArray(groupObj.root, markerObj);
		var selectedListItems = [];
		for (var i = 0; i < groupObj.contents.length; i++) {
		...
		// After cutting parts of the list out with indent=-1, we still have to
		// maintain the array list
		// model's nextItem.indent <= currentItem.indent + 1 invariant.
		// Otherwise the array model of the
		...
		var newList = FCKDomTools.ArrayToList(listArray, markerObj);
		// If groupObj.root is the last element in its parent, or its
		// nextSibling is a <br>, then we should
		// not add a <br> after the final item. So, check for the cases and trim
		// the <br>.
		if (groupObj.root.nextSibling == null
				|| groupObj.root.nextSibling.nodeName.IEquals('br')) {
