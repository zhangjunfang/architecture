...
/*
 * Constructor. @targetDocument : the target document. @enterMode : the behavior
 * for the <Enter> keystroke. May be "p", "div", "br". Default is "p".
 * @shiftEnterMode : the behavior for the <Shift>+<Enter> keystroke. May be
 * "p", "div", "br". Defaults to "br".
 */
var FCKEnterKey = function(targetWindow, enterMode, shiftEnterMode, tabSpaces) {
	...
	oKeystrokeHandler.SetKeystrokes([[13, 'Enter'], [SHIFT + 13, 'ShiftEnter'],
...
FCKEnterKey.prototype.DoEnter = function(mode, hasShift) {
	...
	if (sMode == 'br' || parentPath.Block
			&& parentPath.Block.tagName.toLowerCase() == 'pre')
...
FCKEnterKey.prototype.DoBackspace = function() {
	...
	if (FCKBrowserInfo.IsIE
			&& this._CheckIsAllContentsIncluded(oRange,
	...
	// by a <br> (#1383).
	if (FCKBrowserInfo.IsIE) {
		var previousElement = FCKDomTools.GetPreviousSourceElement(
				oRange.StartNode, true);
		...
		if (previousElement && previousElement.nodeName.toLowerCase() == 'br') {
			...
			// If that range is empty, we can proceed cleaning that <br>
			// manually.
	...
	if (oRange.StartBlockLimit == oRange.EndBlockLimit && oStartBlock
			&& oEndBlock) {
		if (!isCollapsed) {
		...
		if (oRange.CheckStartOfBlock()) {
			...
			var ePrevious = FCKDomTools.GetPreviousSourceElement(oCurrentBlock,
					true, ['BODY', oRange.StartBlockLimit.nodeName], ['UL',
							'OL']);
...
FCKEnterKey.prototype._ExecuteBackspace = function(range, previous,
		currentBlock) {
	var bCustom = false;
	...
	if (previous && currentBlock) {
		...
		// outdent it.
		if (currentBlock.nodeName.IEquals('LI')
				&& !previous.nodeName.IEquals('LI')) {
			this._OutdentWithSelection(currentBlock, range);
			return true;
		}
		...
		if (FCKListsLib.EmptyElements[sPreviousName] != null
				|| sPreviousName == 'table') {
			...
			// Move the contents of the block to the previous element and delete
			// it.
			// But for some block types (e.g. table), moving the children to the
			// previous block makes no sense.
...
FCKEnterKey.prototype.DoDelete = function() {
	...
	// results if we just move to the next block and apply the same <Backspace>
	// logic.
	...
	// There is just one special case for collapsed selections at the end of a
	// block.
	if (oRange.CheckIsCollapsed()
			&& oRange.CheckEndOfBlock(FCKBrowserInfo.IsGeckoLike)) {
		var oCurrentBlock = oRange.StartBlock;
		var eCurrentCell = FCKTools.GetElementAscensor(oCurrentBlock, 'td');

		var eNext = FCKDomTools.GetNextSourceElement(oCurrentBlock, true,
				[oRange.StartBlockLimit.nodeName], ['UL', 'OL', 'TR'], true);
...
FCKEnterKey.prototype.DoTab = function() {
	...
	// If the user pressed <tab> inside a table, we should give him the default
	// behavior ( moving between cells )
...
FCKEnterKey.prototype._ExecuteEnterBlock = function(blockTag, range) {
	...
	if (oSplitInfo) {
		...
		// If there is one block under a list item, modify the split so that the
		// list item gets split as well. (Bug #1647)
		if (eNextBlock) {
		...
		} else if (ePreviousBlock
				&& ePreviousBlock.parentNode.nodeName.IEquals('li')) {
			FCKDomTools.BreakParent(ePreviousBlock, ePreviousBlock.parentNode);
			oRange.MoveToElementEditStart(ePreviousBlock.nextSibling);
			FCKDomTools
					.MoveNode(ePreviousBlock, ePreviousBlock.previousSibling);
		}
		...
		if (!bIsStartOfBlock && !bIsEndOfBlock) {
			...
			// child
			// We'll need to append a placeholder or the list item wouldn't be
			// editable. (Bug #1420)
			if (eNextBlock.nodeName.IEquals('li') && eNextBlock.firstChild
					&& eNextBlock.firstChild.nodeName.IEquals(['ul', 'ol']))
				eNextBlock.insertBefore(FCKTools.GetElementDocument(eNextBlock)
								.createTextNode('\xa0'), eNextBlock.firstChild);
			// Move the selection to the end block.
			if (eNextBlock)
				oRange.MoveToElementEditStart(eNextBlock);
		} else {
			if (bIsStartOfBlock && bIsEndOfBlock
					&& ePreviousBlock.tagName.toUpperCase() == 'LI') {
			...
			if (elementPath) {
				for (var i = 0, len = elementPath.Elements.length; i < len; i++) {
					...
					if (FCKListsLib.InlineChildReqElements[element.nodeName
							.toLowerCase()]) {
			...
			// Move the selection to the new block.
			oRange.MoveToElementEditStart(bIsStartOfBlock && !bIsEndOfBlock
					? eNextBlock
					: eNewBlock);
		}
...
FCKEnterKey.prototype._ExecuteEnterBr = function(blockTag) {
	...
	if (oRange.StartBlockLimit == oRange.EndBlockLimit) {
		...
		var sStartBlockTag = oRange.StartBlock ? oRange.StartBlock.tagName
				.toUpperCase() : '';
		...
		if (!bHasShift && bIsEndOfBlock && (/^H[1-6]$/).test(sStartBlockTag)) {
			...
			FCKDomTools.InsertAfterNode(oRange.StartBlock, this.Window.document
							.createElement('br'));

			// The space is required by Gecko only to make the cursor blink.
			if (FCKBrowserInfo.IsGecko)
				FCKDomTools.InsertAfterNode(oRange.StartBlock,
						this.Window.document.createTextNode(''));

			// IE and Gecko have different behaviors regarding the position.
			oRange.SetStart(oRange.StartBlock.nextSibling, FCKBrowserInfo.IsIE
							? 3
							: 1);
		} else {
			var eLineBreak;
			...
			if (bIsPre)
				eLineBreak = this.Window.document
			...
			// The space is required by Gecko only to make the cursor blink.
			if (FCKBrowserInfo.IsGecko)
				FCKDomTools.InsertAfterNode(eLineBreak, this.Window.document
								.createTextNode(''));

			// If we are at the end of a block, we must be sure the bogus node
			// is available in that block.
			...
			if (!FCKBrowserInfo.IsIE) {
				...
				eLineBreak.parentNode.insertBefore(dummy,
						eLineBreak.nextSibling);
...
FCKEnterKey.prototype._CheckIsAllContentsIncluded = function(range, node) {
	 ...
	 * ',so='+range._Range.startOffset+ ',ec='+range.EndContainer.nodeName+
	...
	if (range.EndContainer == node || range.EndContainer == node.lastChild) {
		var nodeLength = range.EndContainer.nodeType == 3
