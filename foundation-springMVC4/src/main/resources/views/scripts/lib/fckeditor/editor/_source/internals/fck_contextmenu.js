 ...
 * 
 * Defines the FCK.ContextMenu object that is responsible for all Context Menu
 * operations in the editing area.
 */
...
function FCK_ContextMenu_Init() {
	var oInnerContextMenu = FCK.ContextMenu._InnerContextMenu = new FCKContextMenu(
...
function FCK_ContextMenu_GetListener(listenerName) {
	switch (listenerName) {
				...
				AddItems : function(menu, tag, tagName) {
					menu
							.AddItem(
									...
									FCKLang.Cut,
									7,
									FCKCommands.GetCommand('Cut').GetState() == FCK_TRISTATE_DISABLED);
					menu
							.AddItem(
									'Copy',
									FCKLang.Copy,
		...
		case 'Table' :
			return {
				AddItems : function(menu, tag, tagName) {
					...
					var bIsCell = (!bIsTable && FCKSelection
							.HasAncestorNode('TABLE'));

					if (bIsCell) {
						menu.AddSeparator();
						var oItem = menu.AddItem('Cell', FCKLang.CellCM);
						oItem.AddItem('TableInsertCellBefore',
								FCKLang.InsertCellBefore, 69);
						oItem.AddItem('TableInsertCellAfter',
								FCKLang.InsertCellAfter, 58);
						oItem.AddItem('TableDeleteCells', FCKLang.DeleteCells,
								59);
		...
		case 'Link' :
			return {
				AddItems : function(menu, tag, tagName) {
					var bInsideLink = (tagName == 'A' || FCKSelection
							.HasAncestorNode('A'));

					if (bInsideLink
							|| FCK.GetNamedCommandState('Unlink') != FCK_TRISTATE_DISABLED) {
						...
						// If it isn't a link then don't add the Link context
						// menu
						if (bIsAnchor)
							return;
...
function FCK_ContextMenu_OnBeforeOpen() {
	...
	// The extra () is to avoid a warning with strict error checking. This is
	// ok.
