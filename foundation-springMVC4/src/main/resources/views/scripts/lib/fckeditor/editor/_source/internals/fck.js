 ...
 * 
 * Creation and initialization of the "FCK" object. This is the main object that
...
var FCK = {
	...
	StartEditor : function() {
		...
		// In IE7, if the editor tries to access the clipboard by code, a dialog
		// is
		// shown to the user asking if the application is allowed to access or
		// not.
		// Due to the IE implementation of it, the KeystrokeHandler will not
		// work
		// well in this case, so we must leave the pasting keys to have their
		// default behavior.
	...
	FixBody : function() {
		...
		while (oNode) {
			...
			if (bMoveNode) {
				...
				if (!oNewBlock)
					oNewBlock = oParent.insertBefore(oDocument
									.createElement(sBlockTag), oNode);
	...
	GetData : function(format) {
		// We assume that if the user is in source editing, the editor value
		// must
		// represent the exact contents of the source, as the user wanted it to
		// be.
		...
		var data = FCK.DataProcessor.ConvertToDataFormat(isFullPage
						? oDoc.documentElement
						: oDoc.body, !isFullPage,
				FCKConfig.IgnoreEmptyParagraphValue, format);
	...
	OnDoubleClick : function(element) {
		var oCalls = FCK.RegisteredDoubleClickHandlers[element.tagName
				.toUpperCase()];
	...
	RegisterDoubleClickHandler : function(handlerFunction, tag) {
		...
		if (!(aTargets = FCK.RegisteredDoubleClickHandlers[nodeName]))
			...
			// Check that the event handler isn't already registered with the
			// same listener
			// It doesn't detect function pointers belonging to an object (at
			// least in Gecko)
	...
	// Saves URLs on links and images on special attributes, so they don't
	// change when
	// moving around.
	...
	ProtectEvents : function(html) {
		return html.replace(FCKRegexLib.TagsWithEvent,
				_FCK_ProtectEvents_ReplaceTags);
	},

	ProtectEventsRestore : function(html) {
		return html.replace(FCKRegexLib.ProtectedEvents,
	...
	SetData : function(data, resetIsDirty) {
		...
		// If there was an onSelectionChange listener in IE we must remove it to
		// avoid crashes #1498
		...
		if (FCK.EditMode == FCK_EDITMODE_WYSIWYG) {
			...
			// Insert the base tag (FCKConfig.BaseHref), if not exists in the
			// source.
			// The base must be the first tag in the HEAD, to get relative
			// links on styles, for example.
			if (FCK.TempBaseTag.length > 0
					&& !FCKRegexLib.HasBaseTag.test(data))
				data = data.replace(FCKRegexLib.HeadOpener, '$&'
								+ FCK.TempBaseTag);
			...
			if (FCKBrowserInfo.IsIE)
				...
				sHeadExtra += FCKTools.GetStyleHtml(FCK_ShowTableBordersCSS,
						true);
			...
			// Remove the references to the following elements, as the editing
			// area
	...
	ExecuteNamedCommand : function(commandName, commandParameter, noRedirect,
		...
		if (!noRedirect && FCK.RedirectNamedCommands[commandName] != null)
			...
			FCK.Focus();
			FCK.EditorDocument
					.execCommand(commandName, false, commandParameter);
			FCK.Events.FireEvent('OnSelectionChange');
		}
	...
	GetNamedCommandState : function(commandName) {
		try {
			...
			// Bug #50 : Safari never returns positive state for the Paste
			// command, override that.
			if (FCKBrowserInfo.IsSafari && FCK.EditorWindow
					&& commandName.IEquals('Paste'))
				return FCK_TRISTATE_OFF;

			if (!FCK.EditorDocument.queryCommandEnabled(commandName))
				return FCK_TRISTATE_DISABLED;
			else {
				return FCK.EditorDocument.queryCommandState(commandName)
						? FCK_TRISTATE_ON
						: FCK_TRISTATE_OFF;
			}
		} catch (e) {
	...
	Preview : function() {
		...
		if (FCKConfig.FullPage) {
			...
			sHTML = FCKConfig.DocType + '<html dir="'
		...
		if (FCK_IS_CUSTOM_DOMAIN && FCKBrowserInfo.IsIE) {
			...
			sOpenUrl = 'javascript:void( (function(){' + 'document.open() ;'
	...
	SwitchEditMode : function(noUndo) {
		...
		// Save the current IsDirty state, so we may restore it after the
		// switch.
	...
	InsertElement : function(element) {
		// The parameter may be a string (element name), so transform it in an
		// element.
		...
		if (FCKListsLib.BlockElements[elementName] != null) {
			...
			var next = FCKDomTools.GetNextSourceElement(element, false, null, [
							'hr', 'br', 'param', 'img', 'area', 'input'], true);

			// Be sure that we have something after the new element, so we can
			// move the cursor there.
			if (!next && FCKConfig.EnterMode != 'br') {
				next = this.EditorDocument.body.appendChild(this.EditorDocument
	...
	_KeyDownListener : function(evt) {
		...
		if (FCK.EditorWindow) {
			if (!FCK._IsFunctionKey(evt.keyCode) // do not capture function
					// key presses, like arrow
					// keys or shift/alt/ctrl
					&& !(evt.ctrlKey || evt.metaKey) // do not capture Ctrl
					// hotkeys, as they have
					// their snapshot
					// capture logic
					&& !(evt.keyCode == 46)) // do not capture Del, it has
				// its own capture logic in
				// fckenterkey.js
				FCK._KeyDownUndo();
		}
		return true;
	...
	_TabKeyHandler : function(evt) {
		...
		// Pressing <Tab> in source mode should produce a tab space in the text
		// area, not
...
// InsertElementAndGetIt and CreateElement are Deprecated : returns the same
// value as InsertElement.
...
function _FCK_ProtectEvents_ReplaceTags(tagMatch) {
	return tagMatch.replace(FCKRegexLib.EventAttributes,
...
function _FCK_ProtectEvents_ReplaceEvents(eventMatch, attName) {
	return ' ' + attName + '_fckprotectedatt="'
			+ encodeURIComponent(eventMatch) + '"';
}
...
function _FCK_PaddingNodeListener() {
	...
	if (!FCKBrowserInfo.IsIE && FCKDomTools.PaddingNode) {
		// Prevent the caret from going between the body and the padding node in
		// Firefox.
		// i.e. <body>|<p></p></body>
		...
		if (sel && sel.rangeCount == 1) {
			...
			if (range.collapsed
					&& range.startContainer == FCK.EditorDocument.body
					&& range.startOffset == 0) {
		...
		// Prevent the caret from going into an empty body but not into the
		// padding node in IE.
		// i.e. <body><p></p>|</body>
		...
		if (parentElement && parentElement.nodeName.IEquals('body')) {
			if (FCK.EditorDocument.body.childNodes.length == 1
				...
				/*
				 * Bug #1764: Don't move the selection if the current selection
				 * isn't in the editor document.
				 */
				if (FCKSelection
				...
				if (!paddingNode.childNodes.firstChild) {
					paddingNode.appendChild(FCKTools
							.GetElementDocument(paddingNode)
							.createTextNode('\ufeff'));
					clearContents = true;
				}
...
function _FCK_EditingArea_OnLoad() {
	...
	// Most of the CTRL key combos do not work under Safari for onkeydown and
	// onkeypress (See #1119)
...
function _FCK_KeystrokeHandler_OnKeystroke(keystroke, keystrokeValue) {
	...
	// be cancelled. Let's do that only if the Execute() call explicitly returns
	// "false".
	...
	return (oCommand.Execute.apply(oCommand, FCKTools.ArgumentsToArray(
					arguments, 2)) !== false);
}
...
(function() {
	...
	// name set to the ID you are looking for. So the best way in to get the
	// array
	...
	while (eLinkedField || i == 0) {
		if (eLinkedField
				&& eLinkedField.tagName.toLowerCase().Equals('input',
						'textarea')) {
		...
		for (var i = 0; i < this.Elements.length; i++) {
			if (this.Elements[i].isHtml) {
				...
				this.Elements[i] = node.firstChild
						.removeChild(node.firstChild.lastChild);
			}
		}
	}
};
...
var FCKFocusManager = FCK.FocusManager = {
	...
	AddWindow : function(win, sendToEditingArea) {
		...
		if (FCKBrowserInfo.IsIE)
			oTarget = win.nodeType == 1 ? win : win.frameElement
					? win.frameElement
					: win.document;
		...
		FCKTools.AddEventListener(oTarget, 'focus', sendToEditingArea
...
/*
 * #1633 : Protect the editor iframe from external styles. Notice that we can't
 * use FCKTools.ResetStyles here since FCKTools isn't loaded yet.
 */
(function() {
