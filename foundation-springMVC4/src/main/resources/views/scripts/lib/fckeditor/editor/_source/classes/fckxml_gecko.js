...
FCKXml.prototype = {
	LoadUrl : function(urlToCall) {
		...
		if (oXmlHttp.status == 200 || oXmlHttp.status == 304
			...
			if (!oXml)
				oXml = (new DOMParser()).parseFromString(oXmlHttp.responseText,
						'text/xml');
		} else
			oXml = null;
		...
		if (oXml) {
			...
			try {
				...
				// If document.domain has been changed (#123), we'll have a
				// security
				// error at this point. The workaround here is parsing the
				// responseText:
	...
	SelectNodes : function(xpath, contextNode) {
		...
		var xPathResult = this.DOMDocument.evaluate(xpath, contextNode
						? contextNode
						: this.DOMDocument, this.DOMDocument
