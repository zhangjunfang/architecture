...
FCKSelection.GetType = function() {
	// It is possible that we can still get a text range object even when
	// type=='None' is returned by IE.
	// So we'd better check the object returned by createRange() rather than by
	// looking at the type.
	try {
...
FCKSelection.Restore = function() {
	if (this.SelectionData) {
		...
		try {
			// Don't repeat the restore process if the editor document is
			// already selected.
