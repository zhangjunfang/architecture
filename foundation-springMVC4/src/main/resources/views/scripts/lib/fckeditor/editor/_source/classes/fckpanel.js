 ...
 * 
 * Component that creates floating panels. It is used by many other components,
 * like the toolbar items, context menu, etc...
 */
...
var FCKPanel = function(parentWindow) {
	...
	if (FCKBrowserInfo.IsIE) {
		...
		// The popup has to be created before playing with domain hacks, see
		// #1666.
		this._Popup = this._Window.createPopup();

		// this._Window cannot be accessed while playing with domain hacks, but
		// local variable is ok.
		// See #1666.
		...
		var oIFrame = this._IFrame = this._Window.document
...
FCKPanel.prototype.Show = function(x, y, relElement, width, height) {
	...
	if (this._Popup) {
		...
		// The following lines must be place after the above "show", otherwise
		// it
		...
		// Second call: Show the Popup at the specified location, with the
		// correct size.
		...
		// Do not unlock focus manager here because we're displaying another
		// floating panel
		...
		var oPos = FCKTools.GetDocumentPosition(this._Window,
				relElement.nodeType == 9 ? (FCKTools.IsStrictMode(relElement)
						? relElement.documentElement
						: relElement.body) : relElement);

		// Minus the offsets provided by any positioned parent element of the
		// panel iframe.
		var positionedAncestor = FCKDomTools
				.GetPositionedAncestor(this._IFrame.parentNode);
		if (positionedAncestor) {
			var nPos = FCKTools.GetDocumentPosition(FCKTools
							.GetElementWindow(positionedAncestor),
					positionedAncestor);
			oPos.x -= nPos.x;
			oPos.y -= nPos.y;
		}
		...
		this._resizeTimer = setTimeout(function() {
					var iWidth = eMainNode.offsetWidth
							|| eMainNode.firstChild.offsetWidth;
