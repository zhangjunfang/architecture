...
var FCKDomRangeIterator = function(range) {
	...
	/**
	 * Indicates that <br>
	 * elements must be used as paragraph boundaries.
	 */
	this.ForceBrBreak = false;

	/**
	 * Guarantees that the iterator will always return "real" block elements. If
	 * "false", elements like
	 * <li>,
	 * <th> and
	 * <td> are returned. If "true", a dedicated block element block element
	 * will be created inside those elements to hold the selected content.
	 */
...
FCKDomRangeIterator.prototype = {
	/**
	 * Get the next paragraph element. It automatically breaks the document when
	 * necessary to generate block elements for the paragraphs.
	 */
	GetNextParagraph : function() {
		...
		var boundarySet = this.ForceBrBreak
		...
		while (currentNode) {
			...
			if (!includeNode) {
				...
				if (boundarySet[nodeName]
						&& (!FCKBrowserInfo.IsIE || currentNode.scopeName == 'HTML')) {
					// <br> boundaries must be part of the range. It will
					// happen only if ForceBrBreak.
					if (nodeName == 'br')
						includeNode = true;
					else if (!range && currentNode.childNodes.length == 0
							&& nodeName != 'hr') {
					...
					if (range) {
						...
						// The found boundary must be set as the next one at
						// this
						// point. (#1717)
						if (nodeName != 'br')
							this._NextNode = FCKDomTools.GetNextSourceNode(
									currentNode, true, null, lastNode);
					}
			...
			// isLast = ( currentNode == lastNode && ( currentNode.nodeType != 1
			...
			// We have found a block boundary. Let's close the range and move
			// out of the
			// loop.
			if ((closeRange || isLast) && range) {
				...
				if (range.StartNode == range.EndNode
						...
						&& range.StartNode.getAttribute
						&& range.StartNode.getAttribute('_fck_bookmark'))
					range = null;
				else
					break;
			}
			...
			currentNode = FCKDomTools.GetNextSourceNode(currentNode,
					continueFromSibling, null, lastNode);
		}

		// Now, based on the processed range, look for (or create) the block to
		// be returned.
		if (!block) {
			...
			if (!block
					...
					&& range.StartBlockLimit.nodeName
							.IEquals('DIV', 'TH', 'TD')
					&& range.CheckStartOfBlock() && range.CheckEndOfBlock()) {
				block = range.StartBlockLimit;
			} else if (!block
					|| (this.EnforceRealBlocks && block.nodeName.toLowerCase() == 'li')) {
				// Create the fixed block.
				block = this.Range.Window.document
				...
				this._NextNode = block == lastNode ? null : FCKDomTools
						.GetNextSourceNode(range.EndNode, true, null, lastNode);
				return block;
			}
		}
		...
		if (removePreviousBr) {
			...
			if (previousSibling && previousSibling.nodeType == 1) {
				...
				else if (previousSibling.lastChild
						&& previousSibling.lastChild.nodeName.IEquals('br'))
		...
		// next interation.
		if (!this._NextNode)
			this._NextNode = (isLast || block == lastNode) ? null : FCKDomTools
					.GetNextSourceNode(block, true, null, lastNode);
