 ...
 * 
 * Defines the FCKXHtml object, responsible for the XHTML operations. IE
 * specific.
 */
...
FCKXHtml._AppendAttributes = function(xmlNode, htmlNode, node, nodeName) {
	var aAttributes = htmlNode.attributes, bHasStyle;
	...
	for (var n = 0; n < aAttributes.length; n++) {
		...
		if (oAttribute.specified) {
			...
			if (sAttName.StartsWith('_fck'))
				continue;
			// The following must be done because of a bug on IE regarding the
			// style
			...
			// - for events attributes (on IE only).
				...
				continue;
			// XHTML doens't support attribute minimization like "CHECKED". It
			// must be transformed to checked="checked".
			else if (oAttribute.nodeValue === true)
				sAttValue = sAttName;
			else {
				// We must use getAttribute to get it exactly as it is defined.
				// There are some rare cases that IE throws an error here, so we
				// must try/catch.
				try {
	...
	if (bHasStyle || htmlNode.style.cssText.length > 0) {
		...
		var sStyleValue = htmlNode.style.cssText.replace(
				FCKRegexLib.StyleProperties, FCKTools.ToLowerCase);
		FCKTools.RestoreFormStyles(htmlNode, data);
		this._AppendAttribute(node, 'style', sStyleValue);
	}
}

// On very rare cases, IE is loosing the "align" attribute for DIV. (right align
// and apply bulleted list)
...
FCKXHtml.TagProcessors['form'] = function(node, htmlNode) {
	if (htmlNode.acceptCharset && htmlNode.acceptCharset.length > 0
			&& htmlNode.acceptCharset != 'UNKNOWN')
