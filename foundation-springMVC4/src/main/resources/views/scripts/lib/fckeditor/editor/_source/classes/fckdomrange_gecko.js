 ...
 * is not intended to be an implementation of the W3C interface. (Gecko
 * Implementation)
 */
...
FCKDomRange.prototype.MoveToSelection = function() {
	...
	if (oSel && oSel.rangeCount > 0) {
		this._Range = FCKW3CRange.CreateFromRange(this.Window.document, oSel
						.getRangeAt(0));
...
FCKDomRange.prototype.Select = function() {
	...
	if (oRange) {
		...
		if (oRange.collapsed && startContainer.nodeType == 1
		...
		try {
			...
			// otherwise). The new start can't be after the end (W3C says it
			// can).
			// So, let's create a new range and collapse it to the desired
			// point.
