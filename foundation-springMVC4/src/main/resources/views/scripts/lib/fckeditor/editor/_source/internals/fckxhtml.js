...
FCKXHtml.GetXHTML = function(node, includeNode, format) {
	FCKDomTools.CheckAndRemovePaddingNode(FCKTools.GetElementDocument(node),
			FCKConfig.EnterMode);
	...
	FCKDomTools.EnforcePaddingNode(FCKTools.GetElementDocument(node),
			FCKConfig.EnterMode);
	return sXHTML;
}
...
FCKXHtml._AppendChildNodes = function(xmlNode, htmlNode, isBlockElement) {
	...
	if (isBlockElement && htmlNode.tagName
			&& htmlNode.tagName.toLowerCase() != 'pre') {
	...
	if (xmlNode.childNodes.length == 0) {
		...
		// Some inline elements are required to have something inside (span,
		// strong, etc...).
		if (FCKListsLib.InlineChildReqElements[sNodeName])
			return null;

		// We can't use short representation of empty elements that are not
		// marked
...
FCKXHtml._AppendNode = function(xmlNode, htmlNode) {
	...
	switch (htmlNode.nodeType) {
		// Element Node.
		case 1 :
			// If we detect a <br> inside a <pre> in Gecko, turn it into a line
			// break instead.
			// This is a workaround for the Gecko bug here:
			// https://bugzilla.mozilla.org/show_bug.cgi?id=92921
			if (FCKBrowserInfo.IsGecko
				...
				if (htmlNode == htmlNode.parentNode.firstChild)
					val += '\r';
				return FCKXHtml._AppendNode(xmlNode, this.XML
								.createTextNode(val));
			}

			// Here we found an element that is not the real element, but a
			// fake one (like the Flash placeholder image), so we must get the
			// real one.
			if (htmlNode.getAttribute('_fckfakelement'))
				return FCKXHtml._AppendNode(xmlNode, FCK
								.GetRealElement(htmlNode));

			// Ignore bogus BR nodes in the DOM.
			if (FCKBrowserInfo.IsGecko
					&& (htmlNode.hasAttribute('_moz_editor_bogus_node') || htmlNode
			...
			if (FCKBrowserInfo.IsIE) {
				// IE doens't include the scope name in the nodeName. So, add
				// the namespace.
				if (htmlNode.scopeName && htmlNode.scopeName != 'HTML'
						&& htmlNode.scopeName != 'FCK')
					sNodeName = htmlNode.scopeName.toLowerCase() + ':'
							+ sNodeName;
			} else {
			...
			// On some strange cases, the nodeName is empty, even if the node
			// exists.
			if (!FCKRegexLib.ElementName.test(sNodeName))
				return false;

			// The already processed nodes must be marked to avoid then to be
			// duplicated (bad formatted HTML).
			// So here, the "mark" is checked... if the element is Ok, then mark
			// it.
			if (htmlNode._fckxhtmljob
		...
		case 3 :
			if (htmlNode.parentNode
					&& htmlNode.parentNode.nodeName.IEquals('pre'))
		...
		default :
			xmlNode
					.appendChild(this.XML
							.createComment("Element not supported - Type: "
									+ htmlNode.nodeType + " Name: "
									+ htmlNode.nodeName));
			break;
	}
	return true;
...
FCKXHtml._AppendTextNode = function(targetNode, textValue) {
	...
	if (bHadText)
		targetNode.appendChild(this.XML.createTextNode(textValue.replace(
				FCKXHtmlEntities.EntitiesRegex, FCKXHtml_GetEntity)));
	return bHadText;
}
...
FCKXHtml.TagProcessors = {
	a : function(node, htmlNode) {
		// Firefox may create empty tags when deleting the selection in some
		// special cases (SF-BUG 1556878).
		...
		// Anchors with content has been marked with an additional class, now we
		// must remove it.
		if (FCKBrowserInfo.IsIE) {
	...
	area : function(node, htmlNode) {
		...
		// IE ignores the "COORDS" and "SHAPE" attribute so we must add it
		// manually.
		if (FCKBrowserInfo.IsIE) {
	...
	body : function(node, htmlNode) {
		node = FCKXHtml._AppendChildNodes(node, htmlNode, false);
		// Remove spellchecker attributes added for Firefox when converting to
		// HTML code (Bug #1351).
	...
	// Note: Opera does lose the content and doesn't provide it in the innerHTML
	// string
	iframe : function(node, htmlNode) {
		...
		node.appendChild(FCKXHtml.XML.createTextNode(FCKXHtml
				._AppendSpecialItem(sHtml)));
	...
	li : function(node, htmlNode, targetNode) {
		// If the XML parent node is already a <ul> or <ol>, then add the <li>
		// as usual.
		...
		do {
			...
			} while (htmlNode && htmlNode.nodeType == 3
					&& htmlNode.nodeValue.Trim().length == 0)
	...
	script : function(node, htmlNode) {
		...
		node.appendChild(FCKXHtml.XML.createTextNode(FCKXHtml
				._AppendSpecialItem(htmlNode.text)));
	...
	style : function(node, htmlNode) {
		...
		if (FCKBrowserInfo.IsIE) // Bug #403 : IE always appends a \r\n to
			// the beginning of StyleNode.innerHTML
			cssText = cssText.replace(/^(\r\n|\n|\r)/, '');

		node.appendChild(FCKXHtml.XML.createTextNode(FCKXHtml
				._AppendSpecialItem(cssText)));
