function loadData(id, url) {
	...
	if (_obj) {
		//增加等待效果
		 _obj.after("<div class='loadingImg' align='center' style='left: 55%;position: absolute;top: 40%;'><img title='加载信息'src='/styles/images/loading.gif'/></div>");
		//加载数据
...
function confirmWin(handler) {
	...
	var settings = $.extend({
				...
				w:350,//width;可选
				h:240,//height;可选
				okFn:function(){},//点击确定按钮回调方法
				cancelFn:function(){},//点击取消或关闭按钮回调方法;可选
				scope:null//作用域;可选
				},handler);
	...
	var winStr = '<div style="left:'+((parentWindow.window.innerWidth-settings.w)/2)+'px;top:'+((parentWindow.window.innerHeight-settings.h)/2)+'px;width:'+settings.w+'px;height:'+settings.h+'px; display: block; z-index: 1001;" class="x-window popUp popUpFrame" id="msgTipWin">'+
			    ...
			    '<div class="x-window-mr">'+
			        '<div class="x-window-mc">'+ 
			            '<div class="ws_pop_centerC_content">'+
			                '<div class="ws_confirm"> <b>'+settings.msg1+'</b>'+
			                    '<p>'+settings.msg2+'</p>'+
			                '</div>'+
			            '</div>'+
...
function flashObject(handler){
	if(!handler || !handler.obj)return;//json参数必须传递obj（闪烁对象），否则方法返回
	//以下参数都是可选
	var settings = $.extend({
		times:2,//闪烁次数
		showTime: 100,//闪烁显示时间
		flashTime: 100,//闪烁隐藏时间
		showCss: {'opacity':''},//闪烁显示css
		flashCss: {'opacity':'0.1'},//闪烁隐藏css，默认使用透明度闪烁
		beforeFlash: function(){},//闪烁前回调
		completeFlash: function(){},//闪烁完成回调
		scope:null
	},handler);
	
	settings.scope?settings.beforeFlash.call(settings.scope):settings.beforeFlash();
	var this_times = 0;
	setTimeout(function(){
		this_times++;
		if(this_times>settings.times){
			settings.scope?settings.completeFlash.call(settings.scope):settings.completeFlash();
			return;
		}
