...
function My97DP() {
	...
	function _() {
		var A = $d.getElementsByTagName("div"), $ = $d
				.getElementsByTagName("input"), B = $d
				.getElementsByTagName("button"), _ = $d
...
My97DP.prototype = {
	...
	splitDate : function(J, C, O, E, B, G, F, K, L) {
		...
		if (J && J.loadDate)
			...
			$ = new DPDate();
			C = C || $dp.dateFmt;
			var H, N = 0, A = /yyyy|yyy|yy|y|MM|M|dd|d|HH|H|mm|m|ss|s/g, M = J
					.split(/\W+/), _ = C.match(A);
			A.lastIndex = 0;
	...
	_fd : function() {
		var F, D, E, L, H = new sb(), G, A, I, C, K = "", $ = "", _ = new DPDate(
				$dt.y, $dt.M, $dt.d, 0, 0, 0), J = _.y, B = _.M;
			...
			for (E = 0; E < 7; E++) {
				...
				if (_.M == B) {
					...
					if (_.compareWith($sdt, "d") == 0)
						...
						K = (($dp.highLineWeekDay && (E == 0 || E == 6))
								? "Wwday"
								: "Wday");
					$ = (($dp.highLineWeekDay && (E == 0 || E == 6))
							? "WwdayOn"
							: "WdayOn")
	...
	testDate : function(_) {
		var $ = this.ddateRe
				&& this.ddateRe.test(this.getDateStr($dp.realFmt, _));
	...
	_fy : function(_) {
		...
		$
				.a(this.minDate.y < _
						? "class='menu' onmouseover=\"this.className='menuOn'\" onmouseout=\"this.className='menu'\" onmousedown='if(event.preventDefault)event.preventDefault();event.cancelBubble=true;$c._fy("
								+ (_ - 10) + ")';"
						: "class='invalidMenu'");
		$
	...
	_fillQS : function(A) {
		...
		$.a("<table class=WdayTable width=\"" + $d.dDiv.offsetWidth
				+ "px\" height=\"" + $d.dDiv.offsetHeight
				+ "px\" border=0 cellspacing=0 cellpadding=0>");
		$.a("<tr class=MTitle><td><div style=\"float:left\">" + $lang.quickStr
				+ "</div>");
		if (!A)
			$
		...
		for (var B = 0; B < this.QS.length; B++)
			if (this.QS[B]) {
				...
				$.a("hideSel();hide($d.qsDivSel);day_Click(" + this.QS[B].y
						+ ", " + this.QS[B].M + ", " + this.QS[B].d + ","
						+ this.QS[B].H + "," + this.QS[B].m + ","
						+ this.QS[B].s + ");\">");
	...
	initShowAndHide : function() {
		var $ = false;
		$dp.has.y
				? ($ = true, show($d.yI, $d.navLeftImg, $d.navRightImg))
				: hide($d.yI, $d.navLeftImg, $d.navRightImg);
		$dp.has.M ? ($ = true, show($d.MI, $d.leftImg, $d.rightImg)) : hide(
				$d.MI, $d.leftImg, $d.rightImg);
	...
	getP : function(C, _, $) {
		...
		for (E = 0; E < B.length; E++) {
			...
			D = B[E];
			if (_.indexOf(D) >= 0)
				_ = _.replace(D, doStr((C == "y" && D.length < 4)
										? (D.length < 3 ? $.y % 100 : ($.y
												+ 2000 - $dp.yearOffset)
												% 1000)
										: A, D.length))
	...
	initBtn : function() {
		$d.clearI.onclick = function() {
			...
			if (!$) {
				...
				else if ($c.oldValue != $dp.el[$dp.elProp] && $dp.el.onchange)
					fireEvent($dp.el, "change")
			}
		};
