...
public class ConstructorAccessor {
	...
	static {
		...
		Method _newInstance;
		try {
			_newInstance = ObjectStreamClass.class.getDeclaredMethod(
	...
	private static class ConstructorComparator implements
	...
	public final static <T> T newInstance(Class<T> type) {
		...
		if (ctor == null) {
			Constructor<T>[] ctors = (Constructor<T>[]) type
		...
		try {
			if (ctor == nullCtor) {
				return (T) newInstance.invoke(ObjectStreamClass.lookup(type),
						nullArgs);
			}
