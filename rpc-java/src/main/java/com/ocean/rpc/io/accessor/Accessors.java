...
public final class Accessors {
	...
	private static Map<String, MemberAccessor> getFields(Class<?> type) {
		...
		if (fields == null) {
			...
			for (Class<?> clazz = type; clazz != null; clazz = clazz
					.getSuperclass()) {
	...
	public final static Map<String, MemberAccessor> getMembers(Class<?> type,
			RpcMode mode) {
		return ((mode != RpcMode.MemberMode) && Serializable.class
				.isAssignableFrom(type)) ? (mode == RpcMode.FieldMode) ? getFields(type)
				: getProperties(type)
				: getMembers(type);
	}
