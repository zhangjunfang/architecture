...
final class StringBufferArrayUnserializer implements RpcUnserializer {
	...
	final static StringBuffer[] read(RpcReader reader, ByteBuffer buffer)
		...
		case TagRef:
			return (StringBuffer[]) reader.readRef(buffer);
		default:
	...
	final static StringBuffer[] read(RpcReader reader, InputStream stream)
		...
		case TagRef:
			return (StringBuffer[]) reader.readRef(stream);
		default:
			throw ValueReader.castError(reader.tagToString(tag), Array.class);
		}
	}
