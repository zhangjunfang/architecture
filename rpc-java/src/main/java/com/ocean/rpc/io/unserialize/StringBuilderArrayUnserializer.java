...
final class StringBuilderArrayUnserializer implements RpcUnserializer {
	...
	final static StringBuilder[] read(RpcReader reader, ByteBuffer buffer)
		...
		case TagRef:
			return (StringBuilder[]) reader.readRef(buffer);
		default:
	...
	final static StringBuilder[] read(RpcReader reader, InputStream stream)
		...
		case TagRef:
			return (StringBuilder[]) reader.readRef(stream);
		default:
			throw ValueReader.castError(reader.tagToString(tag), Array.class);
		}
	}
