...
public final class ValueReader implements RpcTags {

	final static RpcException badEncoding(int c) {
		return new RpcException("bad utf-8 encoding at "
				+ ((c < 0) ? "end of stream" : "0x"
						+ Integer.toHexString(c & 255)));
	}

	final static RpcException castError(String srctype, Type desttype) {
		return new RpcException(srctype + " can't change to "
				+ desttype.toString());
	}

	final static RpcException castError(Object obj, Type type) {
		return new RpcException(obj.getClass().toString() + " can't change to "
				+ type.toString());
	}
	...
	final static StringBuilder readUntil(ByteBuffer buffer, int tag)
			throws IOException {
	...
	final static StringBuilder readUntil(InputStream stream, int tag)
			throws IOException {
		StringBuilder sb = new StringBuilder();
		int i = stream.read();
		while ((i != tag) && (i != -1)) {
	...
	final static BigInteger readBigInteger(ByteBuffer buffer)
			throws IOException {
		return new BigInteger(readUntil(buffer, TagSemicolon).toString(), 10);
	}

	final static BigInteger readBigInteger(InputStream stream)
			throws IOException {
	...
	final static double readInfinity(ByteBuffer buffer) throws IOException {
		return (buffer.get() == TagNeg) ? Double.NEGATIVE_INFINITY
				: Double.POSITIVE_INFINITY;
	}

	final static double readInfinity(InputStream stream) throws IOException {
		return (stream.read() == TagNeg) ? Double.NEGATIVE_INFINITY
				: Double.POSITIVE_INFINITY;
	}
	...
	final static byte[] readBytes(ByteBuffer buffer) throws IOException {
		int len = readInt(buffer, RpcTags.TagQuote);
		byte[] b = new byte[len];
		buffer.get(b, 0, len);
		buffer.get();
		return b;
	...
	final static byte[] readBytes(InputStream stream) throws IOException {
		int len = readInt(stream, RpcTags.TagQuote);
		int off = 0;
		byte[] b = new byte[len];
		while (len > 0) {
	...
	final static int readTime(ByteBuffer buffer, DateTime dt)
			throws IOException {
	...
	final static int readTime(InputStream stream, DateTime dt)
			throws IOException {
