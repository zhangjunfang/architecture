...
public class DateTime {
	...
	public DateTime(int year, int month, int day, int hour, int minute,
			int second, int nanosecond, boolean utc) {
		this.year = year;
		this.month = month;
		this.day = day;
		this.hour = hour;
	...
	public String toString() {
		...
		if (year == 1970 && month == 1 && day == 1) {
			...
			s = String.format("%04d-%02d-%02dT%02d:%02d:%02d", year, month,
	...
	public StringBuilder toStringBuilder() {
		...
		if (year == 1970 && month == 1 && day == 1) {
			...
			s.append(String.format("%04d-%02d-%02dT%02d:%02d:%02d", year,
					month, day, hour, minute, second));
			if (nanosecond != 0)
				s.append(String.format(".%09d", nanosecond));
		}
		if (utc)
			s.append((char) TagUTC);
		return s;
	}
	...
	public Calendar toCalendar() {
		Calendar calendar = Calendar.getInstance(utc ? TimeZoneUtil.UTC
