...
public final class ClassUtil {
	...
	private static Class<?> getInnerClass(StringBuilder className, int[] pos,
			int i, char c) {
		if (i < pos.length) {
			int p = pos[i];
	...
	public final static String getClassAlias(Class<?> type) {
		String className = RpcClassManager.getClassAlias(type);
		if (className == null) {
			className = type.getName().replace('.', '_').replace('$', '_');
			RpcClassManager.register(type, className);
		}
		return className;
	}

	private static Class<?> getClass(StringBuilder className, int[] pos, int i,
			char c) {
		if (i < pos.length) {
			int p = pos[i];
			className.setCharAt(p, c);
	...
	public final static Class<?> getClass(String className) {
		Class<?> type = RpcClassManager.getClass(className);
		if (type == null) {
