...
public class CookieManager {
	...
	public synchronized String getCookie(String host, String path,
			boolean secure) {
		StringBuilder cookies = new StringBuilder();
		for (Entry<String, ConcurrentHashMap<String, ConcurrentHashMap<String, String>>> entry : container
				.entrySet()) {
			String domain = entry.getKey();
			ConcurrentHashMap<String, ConcurrentHashMap<String, String>> cookieList = entry
					.getValue();
			if (host.endsWith(domain)) {
				ArrayList<String> names = new ArrayList<String>(
						cookieList.size());
				for (Entry<String, ConcurrentHashMap<String, String>> entry2 : cookieList
						.entrySet()) {
					ConcurrentHashMap<String, String> cookie = entry2
							.getValue();
					if (cookie.containsKey("EXPIRES")
							&& Calendar.getInstance(utc).after(
									parseCalendar(cookie.get("EXPIRES")))) {
						names.add(entry2.getKey());
					} else if (path.startsWith(cookie.get("PATH"))) {
						if (((secure && cookie.containsKey("SECURE")) || !cookie
								.containsKey("SECURE"))
								&& !cookie.get("value").equals("")) {
							if (cookies.length() != 0) {
								cookies.append("; ");
							}
