...
import com.ocean.rpc.util.TcpUtil;
...
public class RpcTcpClient extends RpcClient {
	...
	public RpcTcpClient(String uri) {
		super(uri);
	}

	public RpcTcpClient(RpcMode mode) {
		super(mode);
	}

	public RpcTcpClient(String uri, RpcMode mode) {
		super(uri, mode);
	}
	...
	public static RpcClient create(String uri, RpcMode mode)
		...
		if (!scheme.equals("tcp") && !scheme.equals("tcp4")
				&& !scheme.equals("tcp6")) {
			throw new RpcException("This client doesn't support " + scheme
					+ " scheme.");
		}
		return new RpcTcpClient(uri, mode);
	}
