...
public class DbEngineServiceImpl implements DbEngineService, LogEnabled {
	...
	public List query(final String querySQL, final Class modelClass,
		...
		try {
			...
			while (rs.next()) {
				model.add(BeanUtils.resultSet(rs, modelClass, convertFieldName));
			}
							...
							public void execute(Connection connection)
								...
								try {
									...
									if (BaseException.class.isInstance(ex))
										...
										// query failed
										throw new BaseException("执行SQL失败！", ex);
								} finally {
									try {
										if (rs != null)
											rs.close();
							...
							public void execute(Connection conn)
								...
								try {
									...
									if (BaseException.class.isInstance(ex))
										...
										// query failed
										throw new BaseException("校验记录是否存在出错！",
												ex);
								} finally {
									try {
										if (rs != null)
